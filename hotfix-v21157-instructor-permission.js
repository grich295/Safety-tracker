...
'use strict';
(function(){
  if(window.__SAFETY_INSTRUCTOR_RESP_V21157)return;
  window.__SAFETY_INSTRUCTOR_RESP_V21157=true;

  let api=null,state=null,sb=null;
  let queue=[],eligibleManagers=[],myManagedDepartments=[],trainingInstructorAccess=false,trainingInstructorDepartments=[];
  let mode='ALL';
  ...
  const canUseInstructor=()=>isGlobalManager()||isDeptManager()||trainingInstructorAccess;
  const todayISO=()=>new Date().toISOString().slice(0,10);
  const fmtDate=d=>d?new Date(String(d).length===10?d+'T00:00:00':d).toLocaleDateString('en-GB'):'—';

  async function loadData(){
    if(!sb||!state?.user)return;
    const [q,m,d,cap]=await Promise.all([
    ...
    const capRow=cap.error?null:(Array.isArray(cap.data)?cap.data[0]:cap.data);
    trainingInstructorAccess=capRow?.can_carry_out_training===true &&
      !isGlobalManager() &&
      !isDeptManager();
    trainingInstructorDepartments=capRow?.department_names||[];

    if(isDeptManager()) mode='MINE';
    else if(trainingInstructorAccess) mode='AVAILABLE';
  ...
  function currentRows(){
    ...
    // Capability-based instructors only receive rows they are authorised to deliver
    // from instructor_queue_v21156(), so AVAILABLE simply shows those rows.
    return rows;
  }
  ...
  function ensureToolbar(){
    ...
    box.innerHTML=`
      ...
      ${trainingInstructorAccess?`<div class="hint-box"><strong>Training permission enabled:</strong> you can carry out instructor-led training for people in ${esc(trainingInstructorDepartments.join(', ')||'your Department')}. The Department Manager remains responsible for completion.</div>`:''}`;
  ...
  function addHelp(){
    ...
    card.innerHTML=`
      <div class="role-help-action-copy">
        ...
        <p>For whole-site instructor-led training, each employee defaults to their Department Manager. Managers/Admins can deliver, and any person with <strong>Can carry out instructor-led training</strong> ticked in their profile can also deliver training for their Department(s). Responsibility stays with the Department Manager unless manually overridden. The actual instructor is whoever records attendance.</p>
      </div>
  ...
  boot().catch(e=>console.warn('Instructor responsibility v2.11.57',e));
})();
