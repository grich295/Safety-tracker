...
function renderDocuments(){
  ...
  $('documentsList').innerHTML=docs.length?docs.map(d=>{
    ...
    return `<div class="item-card"><div class="row-between"><div><h3>${esc(d.title)}</h3><div class="meta"><span class="badge">${esc(docTypeLabel(d.doc_type))}</span><span>${esc(d.reference||'No reference')}</span>${v?`<span>Version ${esc(v.version_label)}</span>`:''}${reviewOverdue?'<span class="badge overdue">Review overdue</span>':''}${d.review_required?'<span class="badge overdue">Linked document changed - review required</span>':''}</div></div><span class="badge">${esc(d.status)}</span></div><div class="meta"><span>Review: ${fmtDate(v?.review_date)}</span><span>${assigns.length} assigned</span><span>${deliveryText(effectiveDocumentMethod(d,null,v))}</span><span>${renewalText(d.default_renewal_value,d.default_renewal_unit)}</span><span>${links.length} linked</span></div><div class="row">${v?btn('View file','secondary',`data-open-doc="${v.id}"`):''}${btn('Links','ghost',`data-doc-links="${d.id}"`)}${isManager()?btn('Assign','secondary',`data-assign-doc="${d.id}"`)+btn('New version','secondary',`data-new-version="${d.id}"`)+btn('Details','ghost',`data-doc-details="${d.id}"`):btn('Details','ghost',`data-doc-details="${d.id}"`)}</div></div>`;
...
function addSignatureEvidencePages(doc,title,rows){
  if(!rows.length){
    doc.addPage('a4','landscape');
    doc.setFontSize(16);doc.text(title,14,16);
    doc.setFontSize(10);doc.text('No signed records.',14,26);
    return;
  }
  ...
  rows.forEach((r,idx)=>{
    ...
    if(r.signature_data){
      try{
        doc.addImage(r.signature_data,'PNG',sigX+1,sigY+1,signatureW-2,signatureH-2,undefined,'FAST');
      }catch(e){
        doc.text('Signature image could not be rendered',sigX+3,sigY+13);
      }
    }else{
      doc.text('No digital signature image stored',sigX+3,sigY+13);
    }
...
function downloadSignoffPDF(kind){
  ...
  // Remove the initially-created blank page because signature helper creates pages itself.
  doc.deletePage(1);
  addSignatureEvidencePages(doc,kind==='DOCUMENT'?'Document Sign-offs — Signature Evidence':'Training Sign-offs — Signature Evidence',rows);
