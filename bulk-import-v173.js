/* Safety Tracker v1.7.3 - bulk importer with automatic content change detection */
(() => {
'use strict';
...
const oneYearFromToday=()=>{const d=new Date();d.setFullYear(d.getFullYear()+1);return localISODate(d)};
...
function bulkDefaultTbtRenewal(){
  const p=$b('bulkTbtDefault')?.value||'';
  if(!p)return {value:null,unit:null};
  const [v,u]=p.split('|');return {value:Number(v),unit:u};
}
function bulkRenewalPreset(item){
  if(!item.renewalValue||!item.renewalUnit)return '';
  const v=`${item.renewalValue}|${item.renewalUnit}`;
  return ['3|MONTHS','6|MONTHS','12|MONTHS','24|MONTHS'].includes(v)?v:'CUSTOM';
}
function bulkRenewalOptions(item){
  const p=bulkRenewalPreset(item);
  return `<option value="" ${p===''?'selected':''}>One-off only</option>
...
function titleBeforeSds(lines){
  const tidy=s=>clean(s).replace(/\s+:\s*$/,'');
  // Prefer an explicit Product name field.
  for(let i=0;i<Math.min(lines.length,80);i++){
    const l=tidy(lines[i]);
    const same=l.match(/^Product name\s*:?\s*(.+)$/i);
    if(same&&same[1]&&!/^product name$/i.test(same[1])){
      const v=tidy(same[1]);if(v.length>2)return v;
    }
    if(/^Product name\s*:?\s*$/i.test(l)){
      for(let j=i+1;j<Math.min(lines.length,i+5);j++){
        const v=tidy(lines[j]).replace(/^:\s*/,'');
        if(v&&!/^1\.\d|^SECTION|supplier|identifier/i.test(v)&&v.length>2)return v;
      }
    }
  }

  const i=lines.findIndex(x=>/^\s*SAFETY DATA SHEET\s*$/i.test(clean(x)));
  if(i>=0){
    for(let j=i-1;j>=Math.max(0,i-6);j--){
      const l=tidy(lines[j]);
      if(!l)continue;
      if(/SHIELD SAFETY|SECTION 3\.4|MAINTENANCE|SUPPORTING NON-COSHH/i.test(l))continue;
      return l;
    }
    for(let j=i+1;j<Math.min(lines.length,i+12);j++){
      const l=tidy(lines[j]);
      if(!l||/Product name|SECTION 1|Identification|Conforms to|Regulation|Date of|Version/i.test(l))continue;
...
function titleFromRaPage(lines,ref){
  const stop=lines.findIndex(x=>/^Marriott Portsmouth$/i.test(clean(x))||/^Adopted on\b/i.test(clean(x)));
  const end=stop>=0?stop:Math.min(lines.length,6);
  const out=[];
  for(let i=0;i<end;i++){
    const l=clean(lines[i]);
    if(!l)continue;
    if(/SHIELD SAFETY|RISK ASSESSMENT|MAINTENANCE SECTION|^MAINTENANCE$|^SECTION\s+2\.2$/i.test(l))continue;
    if(new RegExp(`\\b${ref.replace('-','\\-')}\\b`,'i').test(l))continue;
    out.push(l);
  }
  return clean(out.join(' '))||ref;
}
function detectStart(text,lines){
  let m=text.match(/\bTBT\s+(?:Reference|Ref)\s+(TBT-\d{3})\b/i);
  if(m&&/TOOLBOX TALK/i.test(text)&&!/TRAINING\s*&\s*SIGN-OFF/i.test(lines.slice(0,4).join(' '))){
    return {kind:'TOOLBOX_TALK',ref:m[1].toUpperCase(),title:titleAfter(lines,'TOOLBOX TALK',/\bTBT\s+(Reference|Ref)\b/i)||m[1].toUpperCase()}
  }

  m=text.match(/\bSSW\s+Reference\s+(SSW-\d{3})\b/i);
  if(m){
    return {kind:'SSW',ref:m[1].toUpperCase(),title:titleAfter(lines,'SAFE SYSTEM OF WORK (SSW)',/\bSSW\s+Reference\b/i)||m[1].toUpperCase()}
  }

  // Standard RA reference field.
  m=text.match(/\b(?:RA\s+Reference|Risk Assessment\s+(?:Reference|Ref))\s*:?\s*(RA-\d{3})\b/i);
  if(m){
    const ref=m[1].toUpperCase();
    return {kind:'RISK_ASSESSMENT',ref,title:titleFromRaPage(lines,ref)||titleFromRefLine(lines,ref)}
  }

  // Shield Safety RA packs identify each RA in the page footer rather than a "RA Reference" field.
  // Only Page 1 starts a new assessment; Page 2 remains attached to that same assessment.
  m=text.match(/\bRISK ASSESSMENT\b[\s\S]*?\b(RA-\d{3})\b\s+Page\s+1\b/i);
  if(m){
    const ref=m[1].toUpperCase();
    return {kind:'RISK_ASSESSMENT',ref,title:titleFromRaPage(lines,ref)}
  }

  m=text.match(/\bCOSHH\s+(?:Reference|Ref)\s*:?\s*(COSHH-\d{3})\b/i);
  if(m){
    return {kind:'COSHH',ref:m[1].toUpperCase(),title:titleAfter(lines,'COSHH RISK ASSESSMENT',/\bCOSHH\s+(Reference|Ref)\b/i)||titleAfter(lines,'COSHH ASSESSMENT',/\bCOSHH\s+(Reference|Ref)\b/i)||titleFromRefLine(lines,m[1].toUpperCase())}
  }
  ...
  if(starts.length){
    for(let n=0;n<starts.length;n++){
      ...
      items.push({
        ...
        issueDate:meta.issue||today(),reviewDate:s.kind==='SDS'?null:(meta.review||oneYearFromToday()),relatedRefs:related,contentHash,
        renewalValue:s.kind==='TOOLBOX_TALK'?bulkDefaultTbtRenewal().value:null,renewalUnit:s.kind==='TOOLBOX_TALK'?bulkDefaultTbtRenewal().unit:null,
    ...
    items.push({
      ...
      issueDate:meta.issue||today(),reviewDate:single.kind==='SDS'?null:(meta.review||oneYearFromToday()),relatedRefs:meta.refs.filter(r=>!r.startsWith('TBT-')),
      contentHash,renewalValue:single.kind==='TOOLBOX_TALK'?bulkDefaultTbtRenewal().value:null,renewalUnit:single.kind==='TOOLBOX_TALK'?bulkDefaultTbtRenewal().unit:null,compareStatus:'NEW',selected:true,mode:'CREATE',existingId:null
    });
  }
  return items;
}
...
async function storedHashForDocument(doc){
  const v=state.versions.filter(x=>x.document_id===doc.id).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0];
  if(!v)return null;
...
async function compareItem(item){
  ...
  item.pendingConflict=false;
  if(!x){
    item.compareStatus='NEW';item.mode='CREATE';item.selected=true;return;
  }
  if(item.kind==='TOOLBOX_TALK'&&item.renewalValue==null&&item.renewalUnit==null){
    item.renewalValue=x.renewal_value||null;item.renewalUnit=x.renewal_unit||null;
  }
  if(item.kind!=='TOOLBOX_TALK'&&state.versions.some(v=>v.document_id===x.id&&v.approval_status==='PENDING')){
    const latest=state.versions.filter(v=>v.document_id===x.id).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0];
    const oldHash=latest?.content_text_sha256||await storedHashForDocument(x);
    if(oldHash&&item.contentHash&&oldHash===item.contentHash){item.compareStatus='UNCHANGED';item.mode='SKIP';item.selected=false;return;}
    item.pendingConflict=true;item.compareStatus='UNKNOWN';item.mode='SKIP';item.selected=false;return;
  }
  ...
  if(oldHash&&item.contentHash){
    if(oldHash===item.contentHash){
      ...
      if(item.kind==='TOOLBOX_TALK'){
        ...
        item.renewalValue=x.renewal_value||item.renewalValue||null;
        item.renewalUnit=x.renewal_unit||item.renewalUnit||null;
      }else{
...
function renderPreview(){
  ...
  box.innerHTML=B.items.map((i,idx)=>`<div class="item-card bulk-preview-card" data-kind="${i.kind}" data-bulk-index="${idx}">
    ...
    <div class="meta">
      ...
      ${i.pendingConflict?'<span>A different version is already pending approval</span>':''}
    </div>
    ...
    <div class="bulk-grid">
      ...
      ${i.kind==='TOOLBOX_TALK'?`<label>Training frequency<select class="bulk-renewal">${bulkRenewalOptions(i)}</select></label><div class="bulk-custom-renewal full" ${bulkRenewalPreset(i)==='CUSTOM'?'':'hidden'}><div class="bulk-grid"><label>Every<input class="bulk-renewal-value" type="number" min="1" value="${i.renewalValue||''}"></label><label>Unit<select class="bulk-renewal-unit"><option ${i.renewalUnit==='DAYS'?'selected':''}>DAYS</option><option ${i.renewalUnit==='MONTHS'?'selected':''}>MONTHS</option><option ${i.renewalUnit==='YEARS'?'selected':''}>YEARS</option></select></label></div></div>`:''}
    </div>
  </div>`).join('');
...
function bindPreviewEdits(){
  document.querySelectorAll('[data-bulk-index]').forEach(card=>{
    ...
    card.querySelector('.bulk-renewal')?.addEventListener('change',e=>{
      const p=e.target.value,custom=card.querySelector('.bulk-custom-renewal');
      if(p==='CUSTOM'){if(custom)custom.hidden=false;i.renewalValue=Number(card.querySelector('.bulk-renewal-value')?.value)||null;i.renewalUnit=card.querySelector('.bulk-renewal-unit')?.value||'MONTHS';}
      else{if(custom)custom.hidden=true;if(!p){i.renewalValue=null;i.renewalUnit=null}else{const [v,u]=p.split('|');i.renewalValue=Number(v);i.renewalUnit=u;}}
    });
    card.querySelector('.bulk-renewal-value')?.addEventListener('change',e=>i.renewalValue=Number(e.target.value)||null);
    card.querySelector('.bulk-renewal-unit')?.addEventListener('change',e=>i.renewalUnit=e.target.value);
  });
}
...
async function createDocument(item,blob){
  const type=item.kind,isSds=type==='SDS',delivery=type==='SSW'?'INSTRUCTOR_LED':'SELF_TRAINING';
  const {data:d,error}=await sb.from('documents').insert({title:item.title,reference:item.reference||null,doc_type:type,delivery_method:delivery,status:'ACTIVE',default_renewal_value:null,default_renewal_unit:null,resign_on_new_version:!isSds,created_by:state.user.id}).select().single();if(error)throw error;
  ...
  const notes=`Bulk imported from ${item.sourceName}, pages ${item.startPage}-${item.endPage}.${isSds?' SDS/MSDS reference-only; no signed approval required.':' Awaiting signed approval before controlled use.'}`;
  const {error:ve}=await sb.from('document_versions').insert({document_id:d.id,version_label:ver,issue_date:item.issueDate||null,review_date:isSds?null:(item.reviewDate||null),delivery_method:delivery,storage_path:path,file_name:name,notes,status:isSds?'CURRENT':'SUPERSEDED',approval_status:isSds?'NOT_REQUIRED':'PENDING',content_text_sha256:item.contentHash||null,created_by:state.user.id});if(ve){await sb.storage.from('safety-files').remove([path]);await sb.from('documents').delete().eq('id',d.id);throw ve}
  item.dbDocId=d.id;
}
async function publishVersion(item,blob){
  ...
  if(!isSds&&state.versions.some(v=>v.document_id===d.id&&v.approval_status==='PENDING'))throw new Error('A document version is already awaiting approval.');
  ...
  if(isSds&&old){const {error}=await sb.from('document_versions').update({status:'SUPERSEDED'}).eq('id',old.id);if(error)throw error}
  const {data:nv,error}=await sb.from('document_versions').insert({document_id:d.id,version_label:ver,issue_date:item.issueDate||null,review_date:isSds?null:(item.reviewDate||null),delivery_method:method,storage_path:path,file_name:name,notes:`Bulk imported changed version from ${item.sourceName}, pages ${item.startPage}-${item.endPage}.${isSds?' Published as SDS/MSDS reference revision.':' Awaiting signed approval.'}`,status:isSds?'CURRENT':'SUPERSEDED',approval_status:isSds?'NOT_REQUIRED':'PENDING',reassign_on_approval:!isSds&&!!d.resign_on_new_version,reassign_due_date:!isSds?daysFromNow(14):null,content_text_sha256:item.contentHash||null,created_by:state.user.id}).select().single();if(error)throw error;
...
async function createTraining(item,blob){
  ...
  const isTbt=item.kind==='TOOLBOX_TALK';
  const {data:t,error}=await sb.from('training_sessions').insert({
    name,session_type:'TOOLBOX_TALK',delivery_method:'INSTRUCTOR_LED',description:desc,delivered_date:null,trainer_name:null,trainer_user_id:state.user.id,
    review_date:item.reviewDate||null,default_due_date:daysFromNow(14),renewal_value:item.renewalValue||null,renewal_unit:item.renewalUnit||null,status:'ACTIVE',
    approval_status:isTbt?'PENDING':'NOT_REQUIRED',replaces_training_session_id:item.replacesTrainingId||null,created_by:state.user.id
...
async function replaceTraining(item,blob){
  ...
  item.replacesTrainingId=old.id;
  if(item.renewalValue==null&&item.renewalUnit==null){item.renewalValue=old.renewal_value||null;item.renewalUnit=old.renewal_unit||null;}
...
async function autoLink(){
  ...
  for(const item of B.items.filter(i=>i.dbDocId)){
    ...
    for(const ref of item.relatedRefs){
      const other=state.documents.find(x=>String(x.reference||'').toUpperCase()===ref.toUpperCase());
      if(other)await addAutoLink(d,other);
    }
  }
  // Re-check the full document library after every batch so newly uploaded upstream
  // or downstream documents can complete missing SDS→COSHH→SSW and RA→SSW chains.
  if(typeof window.autoDiscoverAllDocumentLinks==='function'){
    await window.autoDiscoverAllDocumentLinks();
  }else{
    const sds=state.documents.filter(d=>d.doc_type==='SDS'),coshh=state.documents.filter(d=>d.doc_type==='COSHH');
    for(const s of sds){
      const scored=coshh.map(c=>({c,score:similarity(s.title,c.title)})).sort((a,b)=>b.score-a.score);
      if(scored[0]?.score>=0.60&&(scored.length===1||scored[0].score-scored[1].score>=0.15))await addAutoLink(s,scored[0].c);
    }
  }
}
async function importSelected(){
  ...
  try{
    ...
    setStatus(`Bulk import finished: ${done} imported${failed?`, ${failed} failed`:''}. ${failed?'Failed items remain in the preview.':'Files are now available in Documents / Training. New controlled documents and Toolbox Talks are pending signed approval before assignment.'}`);toast(failed?'Bulk import finished with some errors.':'Bulk import complete.');
