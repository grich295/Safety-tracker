...
async function saveDocument(){
  ...
  const {error:ve}=await sb.from('document_versions').insert({document_id:doc.id,version_label:$('docVersion').value.trim()||'1',issue_date:$('docIssue').value||null,review_date:$('docReview').value||null,delivery_method:$('docDelivery').value,storage_path:path,file_name:file.name,notes:$('docNotes').value.trim()||null,status:'CURRENT',created_by:state.user.id});if(ve)return toast(ve.message);closeModal();await refresh('Document created.');
}
