...
async function createDocumentRecord(){
  ...
  try{
    ...
    closeModal();
    await refresh(isSds?'SDS/MSDS saved as reference-only.':'Document uploaded and is awaiting signed approval before it becomes current or can be assigned.');
  }catch(err){
...
async function publishVersion(docId){
  ...
  closeModal();
  await refresh(isSds?'New SDS/MSDS version published. No approval signature required.':'New version uploaded. The existing approved version remains current until this upload is digitally signed and approved.');
}
