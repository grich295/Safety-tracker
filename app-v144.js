...
function setupSignaturePad(canvasId,clearButtonId){
  ...
  // Match the backing canvas to its displayed size so drawing is accurate on phones/high-DPI screens.
  ...
  if(window.PointerEvent){
    ...
    // Fallback for older Android/WebView browsers.
...
function signatureBlock(prefix){
  ...
  return `<div class="full signature-wrap"><label>Digital signature name<input id="${prefix}SignatureName" value="${name}"></label><div><div class="signature-label">Sign below with your finger or mouse</div><canvas id="${prefix}SignaturePad" class="signature-pad" width="700" height="210" aria-label="Digital signature pad"></canvas></div><div class="row">${btn('Clear signature','ghost',`id="${prefix}ClearSignature"`)}</div><div class="signature-note">Draw your signature in the white box. It is stored with your authenticated user account and the sign-off timestamp.</div></div>`;
}
