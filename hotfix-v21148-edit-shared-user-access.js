...
'use strict';
(function(){
  if(window.__SAFETY_V21148)return;
  window.__SAFETY_V21148=true;

  const ParentObserver=window.MutationObserver;
  if(typeof ParentObserver==='function' && !window.__SAFETY_V21148_OBSERVER_GUARD){
    window.__SAFETY_V21148_OBSERVER_GUARD=true;
    class StableObserverV21148{
    ...
    window.MutationObserver=StableObserverV21148;
  }
  ...
  document.addEventListener('click',e=>{
    const s=e.target.closest?.('[data-v21148-sites]');
    if(s){
      e.preventDefault();e.stopImmediatePropagation();
      pending='sites'; if(booted)openSites(); return;
    }
    const p=e.target.closest?.('[data-v21148-people]');
    if(p){
      e.preventDefault();e.stopImmediatePropagation();
  ...
  function installStyles(){
    if($('safetyV21148Styles'))return;
    const s=document.createElement('style');
    s.id='safetyV21148Styles';
    s.textContent=`
      ...
      .admin-shortcuts-v21148{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:0 0 14px}
      .admin-shortcut-v21148{border:1px solid var(--border,#475569);border-radius:14px;background:var(--card,#1f1f1f);color:inherit;padding:14px;text-align:left;min-height:92px}
      .admin-shortcut-v21148 strong{display:block;font-size:1.05rem;margin-bottom:4px}
      .admin-shortcut-v21148 small{color:var(--muted,#a6b1c2);line-height:1.35}
      .standalone-back-v21148{margin:0 0 14px}
      .site-title-v21148{display:flex;gap:7px;align-items:center;flex-wrap:wrap}
      .site-card-v21148 p{margin:7px 0 0}
      .app-badges-v21148{display:flex;gap:5px;flex-wrap:wrap;margin-top:7px}
      .site-assigned-v21148{margin-top:7px}
      .setup-grid-v21148{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0}
      .setup-grid-v21148>div{display:flex;gap:9px}
      .setup-grid-v21148 b{flex:0 0 30px;width:30px;height:30px;border-radius:50%;display:grid;place-items:center;background:#243b53}
      .setup-grid-v21148 span{display:flex;flex-direction:column}
      .setup-grid-v21148 small{color:var(--muted,#94a3b8)}
      .site-user-row-v21148{display:flex;gap:10px;align-items:flex-start;padding:9px 0;border-bottom:1px solid var(--border,#334155)}
      .site-user-row-v21148:last-child{border-bottom:0}
      .people-card-row-v21148{gap:10px;align-items:flex-start}
      .people-card-row-v21148>button{flex:0 0 auto}
      .pa-username-fields-v21148[hidden]{display:none!important}

      @media(max-width:700px){
        .admin-shortcuts-v21148{grid-template-columns:1fr 1fr}
        .setup-grid-v21148{grid-template-columns:1fr}
        .row-between.site-row-v21148,.row-between.people-head-v21148{display:block}
        .site-row-v21148>button,.people-head-v21148>button{width:100%;margin-top:9px}
      }
    `;
  ...
  function injectAdminShortcuts(){
    ...
    let box=$('adminShortcutsV21148');
    if(!box){
      box=document.createElement('div');
      box.id='adminShortcutsV21148';
      box.className='admin-shortcuts-v21148';
      box.innerHTML=`
        <button class="admin-shortcut-v21148" type="button" data-v21148-sites>
          <strong>Sites</strong>
          <small>Create and manage sites. New sites start with the setup Admin only.</small>
        </button>
        <button class="admin-shortcut-v21148" type="button" data-v21148-people>
  ...
  function ensureSitesView(){
    let v=$('sitesV21148View');
    if(v)return v;
    v=document.createElement('section');
    v.id='sitesV21148View';
    v.className='view';
    v.innerHTML=`
      <div class="page-heading">
        <div><h2>Sites</h2><p class="muted">Each site has its own people and Safety records.</p></div>
        <button class="primary" type="button" data-v21148-create-site>Create site</button>
      </div>
      <button class="secondary standalone-back-v21148" type="button" data-v21148-admin-home>← Admin sections</button>
      <div id="sitesV21148Content"></div>`;
    ($('appView')||document.querySelector('main')||document.body).appendChild(v);
    return v;
  }

  function ensurePeopleView(){
    let v=$('peopleV21148View');
    if(v)return v;
    v=document.createElement('section');
    v.id='peopleV21148View';
    v.className='view';
    v.innerHTML=`
      <div class="page-heading">
        <div><h2>People & Access</h2><p class="muted">All app users. Safety and site access are assigned separately.</p></div>
        <button class="primary" type="button" data-v21148-create-safety>Create new person</button>
      </div>
      <button class="secondary standalone-back-v21148" type="button" data-v21148-admin-home>← Admin sections</button>
      <div id="peopleV21148Content"></div>`;
    ($('appView')||document.querySelector('main')||document.body).appendChild(v);
    return v;
  }
  ...
  function renderSites(){
    const box=$('sitesV21148Content'); if(!box)return;
    ...
    box.innerHTML=`
      ...
      <div class="card-list">
        ${active.map(site=>{
          ...
          return `<div class="item-card site-card-v21148">
            <div class="row-between site-row-v21148">
              <div>
                <div class="site-title-v21148">
                ...
                <div class="meta site-assigned-v21148">
              ...
              <div class="actions">
                <button class="secondary" type="button" data-v21148-manage-site-users="${esc(site.id)}">Manage people</button>
                ${status==='READY'&&site.id!==currentSiteId?`<button class="secondary" type="button" data-v21148-open-site="${esc(site.id)}">Open site</button>`:''}
              </div>
            </div>
          </div>`;
  ...
  async function openSites(){
    ...
    await loadSites();
    renderSites();
    showStandalone('sitesV21148View');
  }
  ...
  function createSiteModal(){
    ...
    body.innerHTML=`
      ...
      <div class="form-grid">
        <label>Site name<input id="newSiteNameV21148" autocomplete="off" placeholder="e.g. Southampton Hotel"></label>
        <label>Timezone<select id="newSiteTimezoneV21148"><option value="Europe/London">Europe/London</option></select></label>
      </div>
      <label class="check-row"><input id="newSiteConfirmV21148" type="checkbox"> Start this site with only my Admin account and clear records.</label>
      <div class="actions">
        <button class="ghost" type="button" data-close-modal>Cancel</button>
        <button class="primary" type="button" data-v21148-save-site>Create site</button>
      </div>`;
    if(!modal.open)modal.showModal();
    setTimeout(()=>$('newSiteNameV21148')?.focus(),50);
  }

  async function saveSite(btn){
    const name=clean($('newSiteNameV21148')?.value);
    if(!name)return toast('Enter a site name.');
    if(!$('newSiteConfirmV21148')?.checked)return toast('Confirm the clean site setup.');
    ...
    const {error}=await sb.rpc('create_clean_safety_site_v21138',{
      p_name:name,p_timezone:$('newSiteTimezoneV21148')?.value||'Europe/London'
    });
    ...
    body.innerHTML=`
      ...
      <div class="section-card">
        ${local.map(p=>{
          ...
          return `<label class="site-user-row-v21148">
            <input type="checkbox" class="site-user-check-v21148" value="${esc(p.id)}"
      ...
      <div class="actions">
        <button class="ghost" type="button" data-close-modal>Cancel</button>
        <button class="primary" type="button" data-v21148-save-site-users="${esc(siteId)}">Save site access</button>
      </div>`;
    if(!modal.open)modal.showModal();
  }
  ...
  async function saveSiteUsers(siteId,btn){
    ...
    const selected=new Set([...document.querySelectorAll('.site-user-check-v21148:checked')].map(x=>x.value));
  ...
  function renderPeople(){
    const box=$('peopleV21148Content'); if(!box)return;
    ...
    box.innerHTML=`
      ...
      <div class="section-card">
        <div class="row-between people-head-v21148">
          <div>
            <h3>All app users</h3>
            <p class="muted">Global Inventory/Energy directory. Safety and site access are granted separately.</p>
          </div>
          <button class="secondary" type="button" data-v21148-refresh-users>Refresh users</button>
        </div>
        ...
        <div class="card-list">
          ${sharedUsers.map(u=>{
            ...
            return `<div class="item-card compact">
              <div class="row-between people-card-row-v21148">
                <div>
                ...
                ${isAdmin()?`<button class="primary small" type="button" data-v21148-edit-shared="${esc(u.source_user_id)}">${s?'Edit access':'Give Safety access'}</button>`:''}
              </div>
            </div>`;
          ...
          ${local.map(p=>{
            ...
            return `<div class="item-card compact">
              <div class="row-between people-card-row-v21148">
                ...
                ${isAdmin()?`<button class="secondary small" type="button" data-v21148-edit-safety="${esc(p.id)}">Edit access</button>`:''}
              </div>
            </div>`;
  ...
  function siteChecksHtml(userId=null){
    ...
    return sites.filter(x=>x.active!==false).map(s=>{
      ...
      return `<label class="site-user-row-v21148">
        <input type="checkbox" class="pa-site-v21148" value="${esc(s.id)}" ${assigned.has(s.id)||creatorLocked?'checked':''} ${creatorLocked?'disabled':''}>
  ...
  async function openSharedAccessEditor(sourceId){
    ...
    body.innerHTML=`
      ...
      <div class="form-grid">
        ...
        <label>Home site
          <select id="paNewHomeV21148">${homeSiteOptions()}</select>
        </label>
      </div>
      ...
      <div class="section-card">
        <h4>Login method</h4>
        ${hasEmail?`<label class="check-row"><input type="radio" name="paLoginMethodV21148" value="email" checked> Email invitation to ${esc(u.email)}</label>`:''}
        <label class="check-row"><input type="radio" name="paLoginMethodV21148" value="username" ${hasEmail?'':'checked'}> Username + temporary password</label>
        <div class="form-grid pa-username-fields-v21148" ${hasEmail?'hidden':''}>
          <label>Username<input id="paNewUsernameV21148" value="${esc(username)}" autocomplete="off"></label>
          <label>Temporary password<input id="paNewPasswordV21148" value="${esc(temp)}" autocomplete="off"></label>
        </div>
      ...
      <div class="actions">
        <button class="ghost" type="button" data-close-modal>Cancel</button>
        <button class="primary" type="button" data-v21148-create-access="${esc(sourceId)}">Create Safety access</button>
      </div>`;
    if(!modal.open)modal.showModal();
  }
  ...
  async function createSharedSafetyAccess(sourceId,btn){
    const u=sharedUsers.find(x=>x.source_user_id===sourceId);
    if(!u)return;
    const role=$('paNewRoleV21148')?.value||'user';
    const view=$('paNewViewV21148')?.value||'user';
    const home=$('paNewHomeV21148')?.value||null;
    const selected=new Set([...document.querySelectorAll('.pa-site-v21148:checked')].map(x=>x.value));
    if(home)selected.add(home);

    const method=document.querySelector('input[name="paLoginMethodV21148"]:checked')?.value||(u.email?'email':'username');
    const username=clean($('paNewUsernameV21148')?.value)||suggestedUsername(u);
    const password=$('paNewPasswordV21148')?.value||generatedPassword();
    ...
    try{
      ...
      const link=await sb.rpc('link_shared_user_to_safety_v21148',{p_source_user_id:sourceId,p_safety_user_id:userId});
      ...
      if(method==='username'){
        bodyCredentialResultV21148(u,username,password);
  ...
  function bodyCredentialResultV21148(u,username,password){
  ...
  async function openSafetyAccessEditor(userId,sourceId=null){
    ...
    body.innerHTML=`
      ...
      <div class="form-grid">
        <label class="check-row"><input id="paEnabledV21148" type="checkbox" ${ma.enabled!==false?'checked':''}> Safety Tracker access</label>
        <label>Safety role
          <select id="paRoleV21148">
        ...
        <label>Home site<select id="paHomeV21148">${homeSiteOptions(userId)}</select></label>
      </div>
      ...
      <div class="actions">
        ${p.login_username?`<button class="secondary" type="button" data-v21148-reset-password="${esc(userId)}">Reset temporary password</button>`:''}
        <button class="ghost" type="button" data-close-modal>Cancel</button>
        <button class="primary" type="button" data-v21148-save-access="${esc(userId)}" data-source-id="${esc(sourceId||'')}">Save access</button>
      </div>`;
    if(!modal.open)modal.showModal();
  }
  ...
  async function saveSafetyAccess(userId,btn){
    ...
    const enabled=!!$('paEnabledV21148')?.checked;
    const role=$('paRoleV21148')?.value||'user';
    const view=$('paViewV21148')?.value||'user';
    const home=$('paHomeV21148')?.value||null;
    const selected=new Set([...document.querySelectorAll('.pa-site-v21148:checked')].map(x=>x.value));
    if(home)selected.add(home);
  ...
  async function openPeople(){
    ...
    await loadSharedUsers();
    renderPeople();
    showStandalone('peopleV21148View');
  }
  ...
  function installEvents(){
    document.addEventListener('click',e=>{
      const editShared=e.target.closest?.('[data-v21148-edit-shared]');
      if(editShared){e.preventDefault();e.stopImmediatePropagation();openSharedAccessEditor(editShared.dataset.v21148EditShared);return}
      const editSafety=e.target.closest?.('[data-v21148-edit-safety]');
      if(editSafety){e.preventDefault();e.stopImmediatePropagation();openSafetyAccessEditor(editSafety.dataset.v21148EditSafety);return}
      const createAccess=e.target.closest?.('[data-v21148-create-access]');
      if(createAccess){e.preventDefault();e.stopImmediatePropagation();createSharedSafetyAccess(createAccess.dataset.v21148CreateAccess,createAccess);return}
      const saveAccess=e.target.closest?.('[data-v21148-save-access]');
      if(saveAccess){e.preventDefault();e.stopImmediatePropagation();saveSafetyAccess(saveAccess.dataset.v21148SaveAccess,saveAccess);return}
      const resetPw=e.target.closest?.('[data-v21148-reset-password]');
      if(resetPw){e.preventDefault();e.stopImmediatePropagation();resetTemporaryPassword(resetPw.dataset.v21148ResetPassword,resetPw);return}

      if(e.target.closest?.('[data-v21148-admin-home]')){
        e.preventDefault();e.stopImmediatePropagation();adminHome();return;
      }
      if(e.target.closest?.('[data-v21148-create-site]')){
        e.preventDefault();e.stopImmediatePropagation();createSiteModal();return;
      }
      const save=e.target.closest?.('[data-v21148-save-site]');
      if(save){e.preventDefault();e.stopImmediatePropagation();saveSite(save);return}
      const open=e.target.closest?.('[data-v21148-open-site]');
      if(open){e.preventDefault();e.stopImmediatePropagation();switchSite(open.dataset.v21148OpenSite);return}
      const manage=e.target.closest?.('[data-v21148-manage-site-users]');
      if(manage){e.preventDefault();e.stopImmediatePropagation();manageSiteUsersModal(manage.dataset.v21148ManageSiteUsers);return}
      const saveUsers=e.target.closest?.('[data-v21148-save-site-users]');
      if(saveUsers){e.preventDefault();e.stopImmediatePropagation();saveSiteUsers(saveUsers.dataset.v21148SaveSiteUsers,saveUsers);return}
      const refresh=e.target.closest?.('[data-v21148-refresh-users]');
      if(refresh){e.preventDefault();e.stopImmediatePropagation();refreshUsers(refresh);return}
      if(e.target.closest?.('[data-v21148-create-safety]')){
        e.preventDefault();e.stopImmediatePropagation();
    ...
    document.addEventListener('change',e=>{
      if(e.target?.name==='paLoginMethodV21148'){
        const fields=document.querySelector('.pa-username-fields-v21148');
  ...
  async function boot(){
    ...
    window.SafetyPeopleSitesV21148={openSites,openPeople};
    window.SafetyPeopleSitesV21146=window.SafetyPeopleSitesV21148;
  }
