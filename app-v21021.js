/* Safety Tracker v2.10.18 CLEAN - training evidence accuracy and outstanding fixes. */
'use strict';

const APP_VERSION='2.10.20';
const BUILD_ID='v21020-auth-startup-repair-20260915';
...
const state={
  ...
  departments:[],userDepartments:[],userViewPreferences:[],documentAudiences:[],trainingAudiences:[],awarenessAudiences:[],ppeAudiences:[],
...
function userEnabledViews(uid=state.user?.id){const rows=(state.userViewPreferences||[]).filter(x=>x.user_id===uid);if(!rows.length)return STANDARD_USER_VIEWS;return new Set(rows.filter(x=>x.enabled!==false).map(x=>x.view_key).concat(['mySafety','hsTraining','help']))}
function canAccessView(name){if(isReportViewer())return name==='reports';if(isAdmin())return ADMIN_VIEWS.has(name);if(isManager())return MANAGER_VIEWS.has(name);const allowed=userEnabledViews();if(name==='asbestos')return allowed.has('asbestos')&&isMaintenanceUser();return allowed.has(name)}
...
const userDepartmentRows=userId=>state.userDepartments.filter(x=>x.user_id===userId);
const userDepartmentRow=userId=>userDepartmentRows(userId).find(x=>x.is_primary)||userDepartmentRows(userId)[0]||null;
const userDepartmentId=userId=>userDepartmentRow(userId)?.department_id||null;
const userDepartmentIds=userId=>userDepartmentRows(userId).map(x=>x.department_id);
const userDepartmentName=userId=>{const rows=userDepartmentRows(userId);return rows.length?rows.map(x=>departmentName(x.department_id)+(x.is_primary?' (primary)':'')).join(' + '):'No department'};
...
function documentAudiencePeople(documentId){const a=audienceTargetsForDocument(documentId),people=activePeople();if(a.everyone)return people;return people.filter(p=>a.userIds.has(p.id)||userDepartmentIds(p.id).some(id=>a.departmentIds.has(id)))}
...
function wireGenericAudience(prefix,showDue=true){const section=$(prefix+'AudienceSection');if(!section)return;const update=()=>{const a=genericAudienceSelection(prefix,showDue),ids=new Set();document.querySelectorAll('.'+prefix+'-department-choice,.'+prefix+'-person-choice').forEach(x=>x.disabled=a.everyone);if(a.everyone)activePeople().forEach(p=>ids.add(p.id));else activePeople().forEach(p=>{const deps=userDepartmentIds(p.id);if(deps.some(dep=>a.departmentIds.includes(dep))||a.userIds.includes(p.id))ids.add(p.id)});const box=$(prefix+'AudienceSummary');if(box)box.innerHTML=ids.size?`<strong>${ids.size} active user${ids.size===1?'':'s'} matched.</strong> Department membership will keep this audience current automatically.`:'<strong>No audience selected.</strong> Choose Everyone, a Department, or a specific person before saving.'};section.addEventListener('change',update);update()}
...
async function refresh(msg){if(isReportViewer()){await loadReportViewerData();renderReports();if(msg)toast(msg);return}await loadAll();renderMySafety();renderHsTraining();if(isManager())renderDocuments();if(isManager())renderCreator();if(isManager())renderTraining();renderAwareness();renderPpe();renderOnSite();if(isMaintenanceUser())renderAsbestosLookup();if(isManager()){renderPeople();renderCompliance();renderInstructor();renderReports()}if(isAdmin())renderAdmin();if(msg)toast(msg)}
...
function installActionRouter(){
  ...
  window.__SAFETY_ACTION_ROUTER='v2.10.21-capture';
}
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',async()=>{
    try{
      ...
      for(const r of regs){
        ...
        if(url&&url.includes('/Safety-tracker/')&&!url.endsWith('/sw-v21020.js'))await r.unregister();
      }
      const r=await navigator.serviceWorker.register('./sw-v21020.js',{updateViaCache:'none'});
...
function hsCategory(t){return mySafetyCategory(t)}
function hsCategoryLabel(k){return ({ALL:'All H&S Training',COSHH:'COSHH RA',RA:'Risk Assessments',SSW:'SSW',TBT:'Toolbox Talks',SDS:'SDS / MSDS',AWARENESS:'Safety Awareness',OTHER:'Other Training'})[k]||k}
let hsTrainingCategory='ALL';
function renderMySafety(){
  renderMySafetyOnSiteShortcut();renderMySafetyKeyShortcut();
  const rows=myActiveAssignments().map(x=>({...x,status:assignmentStatus(x.a,x.t)})).map(x=>({...x,traffic:assignmentTraffic(x.status,trainingDependencyState(x.t).ready)}));
  const completed=rows.filter(x=>x.status.code==='COMPLETED').length,overdue=rows.filter(x=>x.status.code==='OVERDUE'||x.traffic==='red').length,waiting=rows.filter(x=>x.status.code==='AWAITING_INSTRUCTOR').length,action=rows.filter(x=>x.status.code!=='COMPLETED').length;
  ...
  $('mySafetyStats').innerHTML=stats.map(x=>`<div class="stat traffic-${x[2]}"><span class="traffic-dot"></span><strong>${x[1]}</strong><span>${x[0]}</span></div>`).join('');
  const cats=[['COSHH','COSHH RA'],['RA','Risk Assessments'],['SSW','SSW'],['TBT','Toolbox Talks'],['AWARENESS','Safety Awareness'],['OTHER','Other Training']];
  const tile=$('mySafetyCategoryTiles');if(tile)tile.innerHTML=cats.map(([k,label])=>{const r=rows.filter(x=>hsCategory(x.t)===k),bad=r.filter(x=>x.traffic==='red').length,pending=r.filter(x=>x.status.code!=='COMPLETED').length,tr=bad?'red':pending?'amber':'green';return `<button class="my-safety-category-button traffic-${tr}" data-open-hs-training="${k}"><strong>${label}</strong><span>${pending?pending+' action required':'Up to date'}</span></button>`}).join('');
  renderMyAwareness();renderMyPpe();
}
function renderHsTraining(){
  ...
  const cats=[['ALL','All'],['COSHH','COSHH RA'],['RA','Risk Assessments'],['SSW','SSW'],['TBT','Toolbox Talks'],['AWARENESS','Safety Awareness'],['OTHER','Other Training']];
  const tiles=$('hsTrainingTiles');if(tiles)tiles.innerHTML=cats.map(([k,l])=>`<button class="my-safety-category-button${hsTrainingCategory===k?' active':''}" data-hs-category="${k}"><strong>${l}</strong><span>${k==='ALL'?rows.length:rows.filter(x=>hsCategory(x.t)===k).length} item(s)</span></button>`).join('');
  const filtered=rows.filter(x=>hsTrainingCategory==='ALL'||hsCategory(x.t)===hsTrainingCategory), list=$('hsTrainingList');if(!list)return;
  list.innerHTML=filtered.length?filtered.map(({a,t,status,traffic})=>{const src=sourceForTraining(t),deps=trainingDependencyState(t),materials=requiredTrainingMaterials(a,t),opened=requiredTrainingMaterialOpened(a,t);const req=materials.length?btn(opened?'Safety document opened ✓':'Open safety document',opened?'training-doc-opened':'training-doc-required',`data-open-required-training="${a.id}"`):'';const action=deps.ready&&status.code!=='COMPLETED'&&status.method==='SELF_TRAINING'&&opened?btn('Confirm and complete','primary',`data-sign-training="${a.id}"`):deps.ready&&status.code==='READY_TO_SIGN'?btn('Confirm attendance','primary',`data-sign-training="${a.id}"`):'';return `<div class="item-card training-status-card traffic-${traffic}"><h3>${esc(trainingReference(t)&&!t.name.toUpperCase().includes(trainingReference(t))?trainingReference(t)+' - '+t.name:t.name)}</h3><div class="meta"><span class="badge">${esc(kindLabel(trainingKind(t)))}</span><span>${esc(status.label)}</span>${status.due?`<span>Due ${fmtDate(status.due)}</span>`:''}</div>${src?`<div class="muted">Safety source: ${esc(src.reference||'')} ${esc(src.title)}</div>`:''}<div class="row action-bar">${req}${action}${btn(status.code==='COMPLETED'?'Training record':'View training','ghost',`data-view-training="${t.id}"`)}</div></div>`}).join(''):'<div class="success-note">No assigned training in this section.</div>';
}
...
function renderDocumentApprovalOverview(){
  ...
  if(overviewHead&&!overviewHead.querySelector('[data-open-chemical-linking]')){
    ...
    actions.className='document-control-quick-actions';
    actions.innerHTML='<button type="button" class="secondary document-control-link-button" data-open-chemical-linking>Chemical / COSHH Linking</button>';
    overviewHead.appendChild(actions);
  }
...
function registerSectionHtml(kind,label,q=''){
  ...
  return `<section class="register-section"><div class="register-section-heading"><h3>${esc(label)}</h3><span class="badge complete">${rows.length}</span></div>${rows.length?`<div class="register-list">${rows.map(d=>{const v=approvedCurrentVersion(d.id),title=documentDisplayTitle(d);return `<div class="register-row traffic-register-green" data-register-document="${d.id}"><div class="register-main"><strong>${esc(d.reference||'—')}</strong><span>${esc(title)}</span></div><div class="register-meta"><span class="badge complete">${esc(versionApprovalLabel(v,d))}</span><span>v${esc(v?.version_label||'—')}</span><span>Issue ${fmtDate(v?.issue_date)}</span>${kind==='SDS'?'':`<span>Review ${fmtDate(v?.review_date)}</span>`}</div>${isManager()?`<div class="row">${btn('Links','ghost',`data-doc-links="${d.id}"`)}${btn('Edit','ghost',`data-edit-doc-controls="${d.id}"`)}</div>`:''}</div>`}).join('')}</div>`:'<div class="empty compact-empty">No approved/current items registered.</div>'}</section>`;
}
...
function trainingMaterialOpenedSince(m,a){
  ...
  return state.documentActivity.some(x=>x.user_id===uid&&x.action==='OPENED'&&x.source_context==='TRAINING_ASSIGNMENT'&&x.training_session_id===a.training_session_id&&new Date(x.occurred_at||0).getTime()>cutoff&&((m.kind==='DOCUMENT'&&m.version&&x.document_version_id===m.version.id)||(m.kind==='TRAINING_FILE'&&x.training_file_id===m.file.id)));
}
...
async function openRequiredTrainingMaterial(assignmentId){
  ...
  if(m.kind==='DOCUMENT'){if(!m.version?.storage_path)return toast('Stored PDF not found.');await openDocument(m.version.id,{source_context:'TRAINING_ASSIGNMENT',training_session_id:t.id,training_assignment_id:a.id})}else{if(!m.file?.storage_path)return toast('Stored training file not found.');await openTrainingFile(m.file.id,{source_context:'TRAINING_ASSIGNMENT',training_assignment_id:a.id})}
  renderMySafety();
}
...
async function openDocument(versionId,activityContext=null){const v=state.versions.find(x=>x.id===versionId);if(!v?.storage_path)return toast('Stored PDF not found.');if(!isManager()&&(!isVersionApproved(v)||v.status!=='CURRENT'))return toast('Only the approved current version is available for use.');let popup=null;try{popup=window.open('about:blank','_blank')}catch{}try{let blob=state.offline||!navigator.onLine?await cachedSafetyBlob('document',v.id):null;let edgeError='';if(!blob&&!state.offline&&navigator.onLine){if(isManager()){const {data:{session}}=await sb.auth.getSession();const token=session?.access_token;if(token){const r=await fetch(`${CFG.supabaseUrl}/functions/v1/safety-file-open`,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`,'apikey':CFG.supabaseKey},body:JSON.stringify({version_id:v.id})});if(r.ok){blob=await r.blob()}else{try{const j=await r.json();edgeError=j?.error||''}catch{edgeError=await r.text()}}}}if(!blob){const r=await sb.storage.from('safety-files').download(v.storage_path);if(r.error||!r.data)throw new Error(r.error?.message||edgeError||'Stored file could not be opened.');blob=r.data}await cacheSafetyBlob('document',v.id,blob)}if(!blob)throw new Error('This safety PDF has not yet been saved for offline use. Open it once while connected, then it will be available offline.');const url=URL.createObjectURL(blob);try{if(popup&&!popup.closed){popup.opener=null;popup.location.href=url}else{window.location.href=url}}catch{window.location.href=url}setTimeout(()=>URL.revokeObjectURL(url),120000);if(!state.offline&&navigator.onLine){await logDocumentActivity('OPENED',{...activityVersionSnapshot(v),training_session_id:activityContext?.training_session_id||null,source_context:activityContext?.source_context||(versionApprovalStatus(v)==='PENDING'?'VERSION_APPROVAL':'DOCUMENT_LIBRARY')},activityContext?.training_assignment_id?{training_assignment_id:activityContext.training_assignment_id}:null);if($('approvalOpenStatus')&&$('approvalOpenStatus').dataset.versionId===v.id){$('approvalOpenStatus').textContent='Opened ✓';$('approvalOpenStatus').className='badge complete';}}}catch(e){try{popup?.close()}catch{}console.error('openDocument failed',e);toast(e?.message||'Could not open this PDF.')}}
function showDocDetails(id){const d=state.documents.find(x=>x.id===id),versions=state.versions.filter(v=>v.document_id===id).sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0));openModal('Document details',`<p><strong>${esc(d.reference?d.reference+' - '+documentDisplayTitle(d):documentDisplayTitle(d))}</strong></p><div class="meta"><span class="badge">${esc(docTypeLabel(d.doc_type))}</span><span>Status ${esc(d.status)}</span><span>Training ${esc(renewalText(d.default_renewal_value,d.default_renewal_unit,sourceDocTypes.has(d.doc_type)))}</span></div>${isManager()?`<div class="row action-bar">${btn('Edit controls','secondary',`data-edit-doc-controls="${d.id}"`)}${btn('Manage direct links','ghost',`data-doc-links="${d.id}"`)}${btn('Create New Version','ghost',`data-new-version="${d.id}"`)}</div>`:''}<h4>Version history</h4><div class="card-list">${versions.map(v=>{const approved=isVersionApproved(v),pending=versionApprovalStatus(v)==='PENDING',rejected=versionApprovalStatus(v)==='REJECTED',usable=approved&&v.status==='CURRENT',traffic=pending?'amber':rejected?'red':usable?'green':'neutral',canOpen=isManager()||usable;return `<div class="item-card compact document-status-card traffic-${traffic}"><div class="row-between"><span><strong>v${esc(v.version_label)}</strong> · ${esc(v.status)}</span><span class="badge ${pending?'due':rejected?'overdue':approved?'complete':''}">${esc(versionApprovalLabel(v,d))}</span></div><div class="meta"><span>Issue ${fmtDate(v.issue_date)}</span><span>Review ${fmtDate(v.review_date)}</span><span>${esc(v.file_name||'')}</span>${v.approval_at?`<span>Decision ${fmtDateTime(v.approval_at)}</span>`:''}${v.approval_signature_name?`<span>Confirmed by ${esc(v.approval_signature_name)}</span>`:''}</div>${v.approval_note?`<div class="request-note">${esc(v.approval_note)}</div>`:''}<div class="row">${v.storage_path&&canOpen?btn('Open','ghost',`data-open-doc="${v.id}"`)+btn('Download','ghost',`data-download-doc="${v.id}"`)+(usable&&!documentUsesFormalTraining(d)?btn('Mark read / reviewed','ghost',`data-mark-doc-reviewed="${v.id}"`):''):''}${isManager()&&pending?btn(d.doc_type==='SDS'?'Review & accept':'Review & approve','primary',`data-approve-version="${v.id}"`):''}</div></div>`}).join('')}</div>`)}

function showEditDocumentDetails(id){
  const d=state.documents.find(x=>x.id===id),v=approvedCurrentVersion(id)||currentVersion(id);if(!d||!isManager())return;
  const audience=audienceRowsForDocument(id);
  openModal('Edit document controls',`<p><strong>${esc(d.reference?d.reference+' - '+documentDisplayTitle(d):documentDisplayTitle(d))}</strong></p><div class="hint-box">Use this for controlled metadata such as title, reference, training method, refresher frequency, review date and audience. Changing the PDF content still requires <strong>Create New Version</strong> so the approved file history is preserved.</div><div class="form-grid"><label>Title<input id="editDocTitle" value="${esc(d.title||'')}"></label><label>Reference<input id="editDocRef" value="${esc(d.reference||'')}"></label>${sourceDocTypes.has(d.doc_type)?`<label>Training method<select id="editDocMethod"><option value="SELF_TRAINING" ${(d.delivery_method||'SELF_TRAINING')==='SELF_TRAINING'?'selected':''}>Self-training</option><option value="INSTRUCTOR_LED" ${d.delivery_method==='INSTRUCTOR_LED'?'selected':''}>Instructor-led</option></select></label><div class="full"><div class="form-grid">${renewalFields('editDoc',d.default_renewal_value,d.default_renewal_unit,true)}</div></div>`:''}${d.doc_type==='SDS'?'':`<label>Current review date<input id="editDocReviewDate" type="date" value="${esc(v?.review_date||'')}"></label>`}</div>${sourceDocTypes.has(d.doc_type)?genericAudienceHtml('editDocAudience',audience,{heading:'Training audience',help:'Changing this updates who requires the controlled document. Existing evidence/history is retained.'}):''}<div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Save changes','primary',`data-save-doc-controls="${id}"`)}</div>`);
  if(sourceDocTypes.has(d.doc_type)){wireRenewal('editDoc');wireGenericAudience('editDocAudience',true)}
}
async function saveDocumentControls(id){
  const d=state.documents.find(x=>x.id===id),v=approvedCurrentVersion(id)||currentVersion(id);if(!d||!isManager())return;
  const title=clean($('editDocTitle')?.value),reference=clean($('editDocRef')?.value).toUpperCase()||null;if(!title)return toast('Title is required.');
  const payload={title,reference};let audience=null;
  if(sourceDocTypes.has(d.doc_type)){const r=getRenewal('editDoc');payload.delivery_method=$('editDocMethod')?.value||d.delivery_method||'SELF_TRAINING';payload.default_renewal_value=r.value;payload.default_renewal_unit=r.unit;audience=genericAudienceSelection('editDocAudience',true);if(!audienceSelectionValid(audience))return toast('Choose Everyone, at least one Department, or at least one specific person.');}
  const up=await sb.from('documents').update(payload).eq('id',id).select().maybeSingle();if(up.error)return toast(up.error.message);
  if(v&&d.doc_type!=='SDS'&&$('editDocReviewDate')){const rv=await sb.from('document_versions').update({review_date:$('editDocReviewDate').value||null}).eq('id',v.id);if(rv.error)return toast(rv.error.message)}
  if(audience){const ar=await sb.rpc('set_document_training_audience_v230',{p_document_id:id,p_everyone:audience.everyone,p_department_ids:audience.departmentIds,p_user_ids:audience.userIds,p_due_days:audience.dueDays});if(ar.error)return toast(ar.error.message)}
  closeModal();await loadAll();await syncSourceTrainings();await refresh('Document controls updated. Approved PDF history was not changed.');showDocDetails(id)
}
...
function chemicalLinkingRows(q=''){
  const search=clean(q).toLowerCase();
  return state.documents
    .filter(d=>d.status!=='ARCHIVED'&&d.doc_type==='COSHH')
    .filter(d=>!search||`${d.reference||''} ${documentDisplayTitle(d)}`.toLowerCase().includes(search))
    .sort((a,b)=>String(a.reference||documentDisplayTitle(a)).localeCompare(String(b.reference||documentDisplayTitle(b)),undefined,{numeric:true}));
}
function chemicalLinkSummary(d){
  const links=linksForDocument(d.id);
  const linkedDocs=links.map(l=>otherDocForLink(l,d.id)).filter(Boolean);
  return {
    links,
    linkedDocs,
    sds:linkedDocs.filter(x=>x.doc_type==='SDS'),
    ssw:linkedDocs.filter(x=>x.doc_type==='SSW'),
    other:linkedDocs.filter(x=>!['SDS','SSW'].includes(x.doc_type))
  };
}
function showChemicalLinking(){
  if(!isManager())return;
  const rows=chemicalLinkingRows();
  const cards=rows.map(d=>{
    const s=chemicalLinkSummary(d),st=documentApprovalSummary(d);
    const sdsText=s.sds.length?s.sds.map(x=>`${x.reference||'SDS'} ${documentDisplayTitle(x)}`).join(' · '):'No SDS/MSDS linked';
    const sswText=s.ssw.length?s.ssw.map(x=>`${x.reference||'SSW'} ${documentDisplayTitle(x)}`).join(' · '):'No SSW linked';
    return `<div class="item-card chemical-link-card traffic-${st.traffic}" data-chemical-link-search="${esc(`${d.reference||''} ${documentDisplayTitle(d)} ${sdsText} ${sswText}`.toLowerCase())}">
      <div class="row-between">
        <div>
          <h4>${esc(d.reference?d.reference+' - '+documentDisplayTitle(d):documentDisplayTitle(d))}</h4>
          <div class="meta">
            <span class="badge ${st.ready?'complete':'due'}">${esc(st.label)}</span>
            <span class="badge ${s.sds.length?'complete':'due'}">SDS/MSDS ${s.sds.length}</span>
            <span class="badge ${s.ssw.length?'complete':'due'}">SSW ${s.ssw.length}</span>
          </div>
        </div>
        ${btn('Manage links','primary',`data-doc-links="${d.id}"`)}
      </div>
      <div class="chemical-link-summary">
        <button type="button" class="chemical-linked-reference ${s.sds.length?'has-link':'missing-link'}" data-doc-links="${d.id}">
          <strong>SDS/MSDS</strong><span>${esc(sdsText)}</span>
        </button>
        <button type="button" class="chemical-linked-reference ${s.ssw.length?'has-link':'missing-link'}" data-doc-links="${d.id}">
          <strong>Safe System of Work</strong><span>${esc(sswText)}</span>
        </button>
      </div>
    </div>`;
  }).join('');
  openModal('Chemical / COSHH Linking',`
    <div class="hint-box"><strong>Link each chemical in one place.</strong> Select a COSHH Risk Assessment and connect its SDS/MSDS, Safe System of Work and any other directly related controlled document. Links remain direct pairwise relationships only.</div>
    <label>Search chemical, COSHH reference, SDS/MSDS or SSW
      <input id="chemicalLinkSearch" placeholder="e.g. WD-40, COSHH-003, SSW-019">
    </label>
    <div class="chemical-linking-stats">
      <span class="badge">${rows.length} COSHH item${rows.length===1?'':'s'}</span>
      <span class="badge complete">${rows.filter(d=>chemicalLinkSummary(d).sds.length).length} with SDS/MSDS</span>
      <span class="badge complete">${rows.filter(d=>chemicalLinkSummary(d).ssw.length).length} with SSW</span>
    </div>
    <div id="chemicalLinkingList" class="card-list chemical-linking-list">${cards||'<div class="empty">No COSHH Risk Assessments found.</div>'}</div>
  `);
  const input=$('chemicalLinkSearch');
  input?.addEventListener('input',()=>{
    const q=clean(input.value).toLowerCase();
    document.querySelectorAll('[data-chemical-link-search]').forEach(el=>el.hidden=!!q&&!el.dataset.chemicalLinkSearch.includes(q));
  });
}
...
function signTraining(id){
  ...
  const trainee=esc(state.profile?.display_name||state.user?.email||'Authenticated user');openModal('Training confirmation',`<p><strong>${esc(t.name)}</strong></p><div class="meta"><span>${esc(kindLabel(trainingKind(t)))}</span><span>${esc(deliveryText(status.method))}</span></div><div class="success-note">✓ Required training file opened and recorded.</div><p>I confirm I completed/attended this training, understood the relevant controls, and had the opportunity to ask questions. If anything remains unclear I will ask my manager before carrying out the task.</p><div class="review-confirm-box"><strong>Trainee</strong><div>${trainee}</div><p class="muted">Your acknowledgement will be recorded with the date and time. A digital signature is not required for standard training.</p></div><label class="check-row"><input id="trainAckCheck" type="checkbox"> I confirm this training acknowledgement.</label><div class="hint-box">Digital signatures are only required for Permit to Work (PTW) and Keys sign-off.</div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Confirm and complete','primary',`data-confirm-training-sign="${id}"`)}</div>`);
}
async function confirmTrainingSign(id){
  const a=state.trainingAssignments.find(x=>x.id===id),t=state.training.find(x=>x.id===a?.training_session_id);if(!a||!t)return;const deps=trainingDependencyState(t);if(!deps.ready)return toast(trainingDependencyMessage(t));
  if(!requiredTrainingMaterialOpened(a,t))return toast(`Open the required current training file before signing off: ${requiredTrainingMaterialLabel(a,t)}`);
  if(!$('trainAckCheck').checked)return toast('Tick the confirmation first.');
  const sigName=clean(state.profile?.display_name||state.user?.email||'Authenticated user');
  ...
  closeModal();await refresh('Training acknowledgement recorded.')
}
...
function showSetRole(id){
  const p=state.people.find(x=>x.id===id),selected=p?.report_only===true?'report_viewer':p?.role,currentDept=userDepartmentId(id)||'',own=id===state.user?.id,allDeps=new Set(userDepartmentIds(id));
  const deptOpts=activeDepartments().map(d=>`<option value="${d.id}" ${currentDept===d.id?'selected':''}>${esc(d.name)}</option>`).join('');
  const extras=activeDepartments().map(d=>`<label class="check-row"><input type="checkbox" class="role-extra-dept" value="${d.id}" ${allDeps.has(d.id)&&d.id!==currentDept?'checked':''}>${esc(d.name)}</label>`).join('');
  const defaults=new Set(['mySafety','hsTraining','awareness','ppe','onsite','help']);const saved=(state.userViewPreferences||[]).filter(x=>x.user_id===id);const enabled=saved.length?new Set(saved.filter(x=>x.enabled!==false).map(x=>x.view_key)):defaults;
  const views=[['mySafety','My Safety'],['hsTraining','H&S Training'],['awareness','Safety Awareness'],['ppe','PPE Checks'],['onsite',"Who's On Site"],['documents','Documents'],['asbestos','Asbestos Lookup'],['help','Help']];
  const viewChecks=views.map(([k,l])=>`<label class="check-row"><input type="checkbox" class="role-view-choice" value="${k}" ${enabled.has(k)?'checked':''} ${['mySafety','hsTraining','help'].includes(k)?'disabled':''}>${esc(l)}${['mySafety','hsTraining','help'].includes(k)?' <span class="muted">default</span>':''}</label>`).join('');
  openModal('Edit user',`<div class="form-grid"><label>Name<input id="editUserName" value="${esc(p?.display_name||'')}" maxlength="120"></label><label>Email<input id="editUserEmail" type="email" value="${esc(p?.email||'')}"></label><label>Role / access<select id="roleSelect"><option value="user" ${selected==='user'?'selected':''}>User</option><option value="manager" ${selected==='manager'?'selected':''}>Manager</option><option value="admin" ${selected==='admin'?'selected':''}>Admin</option><option value="report_viewer" ${selected==='report_viewer'?'selected':''}>Report Viewer — reports/download only</option></select></label><label>Primary department<select id="roleDepartment"><option value="">No department</option>${deptOpts}</select></label></div><div class="section-card"><h4>Additional departments</h4><p class="muted">Use these when someone covers another department. They automatically receive training targeted to any department they belong to.</p><div class="checkbox-list">${extras||'No departments configured.'}</div></div><div class="section-card"><h4>User Mode preferences</h4><p class="muted">My Safety, H&S Training and Help are standard. Add only the extra areas this user needs.</p><div class="checkbox-list">${viewChecks}</div></div><div class="hint-box">${own?'<strong>Your own account:</strong> you cannot change your own role/access level. ':''}Changing department membership recalculates department-based training. Existing sign-offs and evidence are retained.</div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Save changes','primary',`data-save-role="${id}"`)}</div>`)
}
async function saveRole(id){
  ...
  try{
    ...
    if(requested==='report_viewer'){
      ...
      const dr=await sb.rpc('set_user_departments_v21021',{p_user_id:id,p_primary_department_id:null,p_additional_department_ids:[]});if(dr.error)throw dr.error
    }else{
      ...
      const extras=[...document.querySelectorAll('.role-extra-dept:checked')].map(x=>x.value).filter(x=>x!==departmentId);const dr=await sb.rpc('set_user_departments_v21021',{p_user_id:id,p_primary_department_id:departmentId||null,p_additional_department_ids:extras});if(dr.error)throw dr.error;const views=[...document.querySelectorAll('.role-view-choice:checked')].map(x=>x.value);for(const v of ['mySafety','hsTraining','help'])if(!views.includes(v))views.push(v);const vr=await sb.rpc('set_user_view_preferences_v21021',{p_user_id:id,p_enabled_views:views});if(vr.error)throw vr.error
    }
...
function renderSiteLocations(){const box=$('siteLocationList');if(!box)return;const rows=[...state.siteLocations].sort((a,b)=>(a.active!==false?0:1)-(b.active!==false?0:1)||(a.sort_order||0)-(b.sort_order||0)||locationPath(a.id).localeCompare(locationPath(b.id)));box.innerHTML=rows.length?rows.map(x=>`<div class="item-card compact"><div class="row-between"><div><strong>${esc(locationPath(x.id))}</strong><div class="meta"><span>${esc((x.location_type||'AREA').replaceAll('_',' '))}</span><span class="badge ${x.active===false?'neutral':'complete'}">${x.active===false?'Archived':'Active'}</span></div></div><div class="row">${btn('Edit','secondary',`data-edit-site-location="${x.id}"`)}${btn(x.active===false?'Restore':'Archive',x.active===false?'primary':'ghost',`data-toggle-site-location="${x.id}"`)}</div></div></div>`).join(''):'<div class="empty">No site locations.</div>'}
function showSiteLocationEditor(id=''){if(!isAdmin())return;const x=id?locationById(id):null;const parents=state.siteLocations.filter(p=>p.id!==id&&p.active!==false).map(p=>`<option value="${p.id}" ${x?.parent_id===p.id?'selected':''}>${esc(locationPath(p.id))}</option>`).join('');openModal(x?'Edit site location':'New site location',`<div class="form-grid"><label>Name<input id="siteLocationName" maxlength="120" value="${esc(x?.name||'')}"></label><label>Type<select id="siteLocationType"><option value="AREA">Area</option><option value="GUEST_ROOMS">Guest rooms group</option><option value="FLOOR">Floor</option><option value="ROOM">Room</option><option value="PLANT">Plant</option><option value="EXTERNAL">External</option><option value="OTHER">Other</option></select></label><label>Parent location<select id="siteLocationParent"><option value="">Top level</option>${parents}</select></label><label>Sort order<input id="siteLocationSort" type="number" value="${Number(x?.sort_order||0)}"></label><label>Floor label<input id="siteLocationFloor" value="${esc(x?.floor_label||'')}"></label><label>Room label<input id="siteLocationRoom" value="${esc(x?.room_label||'')}"></label></div><div class="hint-box">For bedrooms, create a Floor beneath Guest Rooms, then create individual Room locations beneath that floor. Selecting Guest Rooms, a floor or a room will then drive the permit/asbestos check at that level.</div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Save location','primary',`data-save-site-location="${id}"`)}</div>`);$('siteLocationType').value=x?.location_type||'AREA'}
async function saveSiteLocation(id=''){if(!isAdmin())return;const name=clean($('siteLocationName').value);if(!name)return toast('Location name is required.');const payload={name,location_type:$('siteLocationType').value,parent_id:$('siteLocationParent').value||null,sort_order:Number($('siteLocationSort').value||0),floor_label:clean($('siteLocationFloor').value)||null,room_label:clean($('siteLocationRoom').value)||null,active:true,created_by:state.user.id};let r;if(id){delete payload.created_by;r=await sb.from('site_locations_v280').update(payload).eq('id',id)}else r=await sb.from('site_locations_v280').insert(payload);if(r.error)return toast(r.error.message);closeModal();await refresh(id?'Site location updated.':'Site location created.')}
async function toggleSiteLocation(id){if(!isAdmin())return;const x=locationById(id);if(!x)return;const r=await sb.from('site_locations_v280').update({active:x.active===false}).eq('id',id);if(r.error)return toast(r.error.message);await refresh(x.active===false?'Site location restored.':'Site location archived.')}
...
function updateApprovalAudienceSummary(){
  ...
  if(a.everyone)people.forEach(p=>ids.add(p.id));else{people.forEach(p=>{const deps=userDepartmentIds(p.id);if(deps.some(dep=>a.departmentIds.includes(dep))||a.userIds.includes(p.id))ids.add(p.id)})}
...
async function saveVersionApproval(versionId){
  ...
  const decision=$('approvalDecision')?.value,context=$('approvalContext')?.value,note=clean($('approvalNote')?.value),audience=documentUsesFormalTraining(d)?approvalAudienceSelection():null;
  ...
  if(decision==='APPROVED'&&audience){
    ...
    r=await sb.rpc('decide_document_version_with_training_schedule_ack_v281',{p_document_version_id:v.id,p_decision:decision,p_context:context,p_note:note||null,p_everyone:audience.everyone,p_department_ids:audience.departmentIds,p_user_ids:audience.userIds,p_due_days:audience.dueDays,p_delivery_method:trainingDelivery,p_renewal_value:trainingRenewal.value,p_renewal_unit:trainingRenewal.unit});
  }else{
    r=await sb.rpc('decide_document_version_with_ack_v281',{p_document_version_id:v.id,p_decision:decision,p_context:context,p_note:note||null});
  }
  if(r.error)return toast(r.error.message);
...
async function saveDocumentReview(id){if(!$('reviewAck').checked)return toast('Tick the confirmation first.');const d=state.documents.find(x=>x.id===id),v=approvedCurrentVersion(id),outcome=$('reviewOutcome').value,reason=$('reviewReason').value,note=clean($('reviewNote').value),next=$('nextReviewDate').value||null,sig=`ACK:${state.user?.id||'user'}:${new Date().toISOString()}`,sigName=clean(state.profile?.display_name||state.user?.email||'Authenticated user');if(outcome==='NO_CHANGE'&&!next)return toast('Choose the next review date.');const storedNote=`Review reason: ${controlledReviewReasonLabel(reason)}${note?`
...
function evidenceDocGroup(d){return d.doc_type==='SDS'?'SDS / MSDS':d.doc_type==='COSHH'?'COSHH RA':d.doc_type==='RISK_ASSESSMENT'?'Risk Assessments':d.doc_type==='SSW'?'Safe Systems of Work':'Other controlled documents'}
function evidenceDocumentPickerRows(preselected=[]){
  const sel=new Set(preselected),groups=['SDS / MSDS','COSHH RA','Risk Assessments','Safe Systems of Work','Other controlled documents'];
  const rows=state.documents.filter(d=>d.status!=='ARCHIVED').sort((a,b)=>String(a.reference||documentDisplayTitle(a)).localeCompare(String(b.reference||documentDisplayTitle(b)),undefined,{numeric:true}));
  return groups.map((group,idx)=>{
    const items=rows.filter(d=>evidenceDocGroup(d)===group);if(!items.length)return '';
    const selectedCount=items.filter(d=>sel.has(d.id)).length;
    return `<details class="evidence-group" data-evidence-group="${esc(group)}" ${selectedCount||idx===0?'open':''}><summary class="evidence-group-title"><span>${esc(group)}</span><span class="evidence-group-count">${items.length}${selectedCount?` · ${selectedCount} selected`:''}</span></summary><div class="evidence-group-body">${items.map(d=>{const v=approvedCurrentVersion(d.id)||pendingApprovalVersion(d.id)||latestVersion(d.id),status=v?versionApprovalLabel(v,d):'No version';return `<label class="check-row evidence-choice" data-evidence-search="${esc(`${d.reference||''} ${documentDisplayTitle(d)} ${docTypeLabel(d.doc_type)} ${status}`.toLowerCase())}"><input type="checkbox" class="evidence-doc-choice" value="${d.id}" ${sel.has(d.id)?'checked':''}><span class="evidence-choice-text"><strong>${esc(d.reference||docTypeLabel(d.doc_type))}</strong> · ${esc(documentDisplayTitle(d))}<br><span class="muted">${esc(docTypeLabel(d.doc_type))} · ${esc(status)}</span><span class="evidence-auto-note" hidden></span></span></label>`}).join('')}</div></details>`
  }).join('')
}
function evidenceTrainingPickerRows(preselected=[]){
  const sel=new Set(preselected);
  return state.training.filter(t=>t.status!=='ARCHIVED'&&!t.source_document_id).sort((a,b)=>String(trainingReference(a)||a.name).localeCompare(String(trainingReference(b)||b.name),undefined,{numeric:true})).map(t=>`<label class="check-row evidence-choice" data-evidence-search="${esc(`${trainingReference(t)||''} ${t.name} ${kindLabel(trainingKind(t))}`.toLowerCase())}"><input type="checkbox" class="evidence-training-choice" value="${t.id}" ${sel.has(t.id)?'checked':''}><span class="evidence-choice-text"><strong>${esc(trainingReference(t)||kindLabel(trainingKind(t)))}</strong> · ${esc(t.name)}<br><span class="muted">Standalone ${esc(kindLabel(trainingKind(t)))}</span><span class="evidence-auto-note" hidden></span></span></label>`).join('')
}
let evidencePickerMode='ALL';
function evidenceDocLabel(id){const d=state.documents.find(x=>x.id===id);return d?String(d.reference||documentDisplayTitle(d)||'linked document'):'linked document'}
function evidenceExpectedMissing(docId){
  const d=state.documents.find(x=>x.id===docId);if(!d)return [];
  const linked=evidenceDirectDocIds(docId).map(id=>state.documents.find(x=>x.id===id)).filter(Boolean);
  const types=new Set(linked.map(x=>x.doc_type));
  const missing=[];
  if(d.doc_type==='SDS'&&!types.has('COSHH'))missing.push('No COSHH RA linked');
  if(d.doc_type==='COSHH'){if(!types.has('SDS'))missing.push('No SDS/MSDS linked');if(!types.has('SSW'))missing.push('No SSW linked')}
  if(d.doc_type==='RISK_ASSESSMENT'&&!types.has('SSW'))missing.push('No SSW linked');
  return missing;
}
function updateEvidenceMissingLinks(){
  const box=$('evidenceMissingLinks');if(!box)return;
  const selected=[...document.querySelectorAll('.evidence-doc-choice:checked')].map(x=>x.value);
  const rows=selected.flatMap(id=>evidenceExpectedMissing(id).map(msg=>({id,msg})));
  box.hidden=!rows.length;
  box.innerHTML=rows.length?`<strong>${rows.length} possible missing link${rows.length===1?'':'s'}.</strong><div class="evidence-missing-list">${rows.map(x=>`<div><span class="badge due">${esc(evidenceDocLabel(x.id))}</span> ${esc(x.msg)}</div>`).join('')}</div>`:'';
}
function applyEvidencePickerFilters(){
  const q=clean($('evidenceItemSearch')?.value||'').toLowerCase(),mode=evidencePickerMode||'ALL';
  document.querySelectorAll('.evidence-choice').forEach(x=>{
    const input=x.querySelector('input');
    const searchOk=!q||String(x.dataset.evidenceSearch||'').includes(q);
    const modeOk=mode==='ALL'||(mode==='SELECTED'&&!!input?.checked)||(mode==='MISSING'&&input?.classList.contains('evidence-doc-choice')&&evidenceExpectedMissing(input.value).length>0);
    x.hidden=!(searchOk&&modeOk);
  });
  document.querySelectorAll('.evidence-group').forEach(g=>{
    const choices=[...g.querySelectorAll('.evidence-choice')],visible=choices.filter(x=>!x.hidden);
    g.hidden=!visible.length;
    if(q||mode!=='ALL')g.open=!!visible.length;
  });
  document.querySelectorAll('[data-evidence-mode]').forEach(b=>b.classList.toggle('active',b.dataset.evidenceMode===mode));
}
function wireEvidenceSearch(){
  const q=$('evidenceItemSearch');if(q)q.addEventListener('input',applyEvidencePickerFilters);
  wireEvidenceLinkedSelection();applyEvidencePickerFilters();updateEvidenceSelectedSummary();updateEvidenceMissingLinks()
}
function setEvidenceChoices(selector,checked){
  document.querySelectorAll(selector).forEach(x=>{if(!x.closest('[hidden]')){x.checked=checked;markEvidenceAutoSelected(x,false)}});
  updateEvidenceLinkedSummary();updateEvidenceSelectedSummary();updateEvidenceMissingLinks();applyEvidencePickerFilters()
}
function markEvidenceAutoSelected(input,on,reason=''){
  if(!input)return;
  input.dataset.autoSelected=on?'true':'';
  const note=input.closest('.evidence-choice')?.querySelector('.evidence-auto-note');
  if(note){note.hidden=!on;note.textContent=on?`Auto-selected because linked to ${reason||'your selection'}`:''}
  input.closest('.evidence-choice')?.classList.toggle('auto-linked',!!on)
}
function evidenceDirectDocIds(docId){const out=new Set();for(const l of state.documentLinks||[]){if(l.source_document_id===docId)out.add(l.target_document_id);else if(l.target_document_id===docId)out.add(l.source_document_id)}return [...out].filter(id=>state.documents.some(d=>d.id===id&&d.status!=='ARCHIVED'))}
function evidenceLinkedTrainingIds(docIds){const ids=new Set(docIds),out=new Set();for(const l of state.trainingDocumentLinks||[]){if(!ids.has(l.document_id))continue;const t=state.training.find(x=>x.id===l.training_session_id);if(t&&t.status!=='ARCHIVED'&&!t.source_document_id)out.add(t.id)}return [...out]}
function expandEvidenceLinkedSelection(rootId){
  const maxDepth=3,seen=new Set([rootId]),queue=[{id:rootId,depth:0}],addedDocs=[],parent=new Map();
  while(queue.length){const cur=queue.shift();if(cur.depth>=maxDepth)continue;for(const id of evidenceDirectDocIds(cur.id)){if(seen.has(id))continue;seen.add(id);parent.set(id,cur.id);addedDocs.push(id);queue.push({id,depth:cur.depth+1})}}
  for(const id of addedDocs){const box=document.querySelector(`.evidence-doc-choice[value="${CSS.escape(id)}"]`);if(box&&!box.checked){box.checked=true;markEvidenceAutoSelected(box,true,evidenceDocLabel(parent.get(id)||rootId))}}
  const linkedTraining=evidenceLinkedTrainingIds([...seen]);
  for(const tid of linkedTraining){const box=document.querySelector(`.evidence-training-choice[value="${CSS.escape(tid)}"]`);if(box&&!box.checked){box.checked=true;markEvidenceAutoSelected(box,true,evidenceDocLabel(rootId))}}
  updateEvidenceLinkedSummary();updateEvidenceSelectedSummary();updateEvidenceMissingLinks();applyEvidencePickerFilters()
}
function updateEvidenceLinkedSummary(){
  const box=$('evidenceLinkedSummary');if(!box)return;
  const auto=[...document.querySelectorAll('.evidence-doc-choice[data-auto-selected="true"],.evidence-training-choice[data-auto-selected="true"]')].filter(x=>x.checked);
  box.hidden=!auto.length;box.innerHTML=auto.length?`<strong>${auto.length} linked item${auto.length===1?'':'s'} auto-selected.</strong> They are suggestions from the saved document links; untick anything you do not need.`:''
}
function updateEvidenceSelectedSummary(){
  const box=$('evidenceSelectedSummary');if(!box)return;
  const docs=[...document.querySelectorAll('.evidence-doc-choice:checked')],training=[...document.querySelectorAll('.evidence-training-choice:checked')];
  box.innerHTML=`<strong>${docs.length+training.length}</strong><span>selected</span><small>${docs.length} documents · ${training.length} training / talks</small>`
}
function wireEvidenceLinkedSelection(){
  document.querySelectorAll('.evidence-doc-choice').forEach(box=>box.addEventListener('change',()=>{if(box.checked){markEvidenceAutoSelected(box,false);expandEvidenceLinkedSelection(box.value)}else markEvidenceAutoSelected(box,false);updateEvidenceLinkedSummary();updateEvidenceSelectedSummary();updateEvidenceMissingLinks();applyEvidencePickerFilters()}));
  document.querySelectorAll('.evidence-training-choice').forEach(box=>box.addEventListener('change',()=>{markEvidenceAutoSelected(box,false);updateEvidenceLinkedSummary();updateEvidenceSelectedSummary();applyEvidencePickerFilters()}));
  const initial=[...document.querySelectorAll('.evidence-doc-choice:checked')].map(x=>x.value);for(const id of initial)expandEvidenceLinkedSelection(id)
}
function showEvidencePackPicker(preselectedDocs=[],preselectedTraining=[]){
  if(!isManager())return;evidencePickerMode='ALL';
  openModal('Person evidence pack',`<div class="hint-box"><strong>Start with the document you need.</strong> Search or open a document group, tick an SDS/MSDS, COSHH RA, Risk Assessment, SSW or other controlled document, and Safety Tracker will automatically suggest its saved linked documents and linked Toolbox Talks/training. Direct document relationships are not changed.</div><div class="evidence-picker-toolbar"><div id="evidenceSelectedSummary" class="evidence-selected-summary"></div><div class="evidence-mode-buttons"><button type="button" class="secondary small active" data-evidence-mode="ALL">All</button><button type="button" class="secondary small" data-evidence-mode="SELECTED">Selected only</button><button type="button" class="secondary small" data-evidence-mode="MISSING">Missing links</button></div></div><div id="evidenceLinkedSummary" class="success-note" hidden></div><div id="evidenceMissingLinks" class="request-note" hidden></div><div class="form-grid"><label class="full">Person<select id="evidencePerson"><option value="">Select person…</option>${evidencePersonOptions()}</select></label><label>History<select id="evidenceScope"><option value="CURRENT">Current approved version only</option><option value="FULL">Full version history</option></select></label><label>From date (optional)<input id="evidenceStart" type="date"></label><label>To date (optional)<input id="evidenceEnd" type="date"></label><label class="full evidence-search-label">Find a document or product<input id="evidenceItemSearch" placeholder="Search reference, chemical/product, title or type"></label></div><div class="section-card evidence-compact-section"><div class="row-between"><div><h4>Controlled documents</h4><p class="muted">Groups stay collapsed so this remains usable as the register grows.</p></div><div class="row"><button class="secondary small" type="button" data-evidence-select-docs="1">Select visible</button><button class="ghost small" type="button" data-evidence-clear-docs="1">Clear visible</button></div></div><div class="checkbox-list evidence-picker-list evidence-grouped-list">${evidenceDocumentPickerRows(preselectedDocs)}</div></div><div class="section-card evidence-compact-section"><details class="evidence-training-details" ${preselectedTraining.length?'open':''}><summary><strong>Toolbox Talks / standalone training</strong> <span class="muted">(${state.training.filter(t=>t.status!=='ARCHIVED'&&!t.source_document_id).length})</span></summary><div class="row evidence-training-actions"><button class="secondary small" type="button" data-evidence-select-training="1">Select visible</button><button class="ghost small" type="button" data-evidence-clear-training="1">Clear visible</button></div><div class="checkbox-list evidence-picker-list">${evidenceTrainingPickerRows(preselectedTraining)||'<span class="muted">No standalone training items.</span>'}</div></details></div><div class="actions"><button class="secondary" type="button" data-evidence-select-all="1">Select all visible</button>${btn('Cancel','ghost','data-close-modal')}${btn('Create evidence pack','primary','data-generate-evidence-selected')}</div>`);
  wireEvidenceSearch()
}
