...
function renderMyAction(x){
  ...
  if(x.kind==='document'){
    const method=x.d.delivery_method||defaultDocDelivery(x.d.doc_type),c=latestDocConfirmation(x.a),event=latestDocEvent(x.a),ready=freshConfirmation(c,x.s),needs=!x.s||overdue;
  ...
  return `<div class="item-card"><div class="row-between"><div><h3>${esc(x.t.name)}</h3><div class="meta"><span>${esc(x.t.session_type.replaceAll('_',' '))}</span><span>${deliveryText(method)}</span><span>Trainer: ${esc(x.t.trainer_name||'—')}</span></div></div><span class="badge ${overdue?'overdue':x.s&&!overdue?'complete':'due'}">${status}</span></div><div class="meta"><span>Due: ${x.due?fmtDate(x.due):'One-off complete'}</span><span>${renewalText(x.a.renewal_value,x.a.renewal_unit)}</span>${c?.delivery_date?`<span>Training date ${fmtDate(c.delivery_date)}</span>`:''}</div><div class="row">${btn('View training','secondary',`data-view-training="${x.t.id}"`)}${needs&&(method!=='INSTRUCTOR_LED'||ready)?btn('Sign off','primary',`data-sign-training="${x.a.id}"`):''}${creatorException?btn('Sign off as exception','secondary',`data-training-exception="${x.a.id}"`):''}</div></div>`;
}
...
async function signDocument(assignmentId){const a=state.docAssignments.find(x=>x.id===assignmentId),v=state.versions.find(x=>x.id===a.document_version_id),d=state.documents.find(x=>x.id===v.document_id);const prev=latestDocSignoff(a),method=d.delivery_method||defaultDocDelivery(d.doc_type);if(method==='INSTRUCTOR_LED'&&!freshConfirmation(latestDocConfirmation(a),prev))return toast('Instructor confirmation is required before you can sign off.');openModal('Acknowledge document',`<p><strong>${esc(d.title)}</strong> · ${esc(d.reference||'')} · Version ${esc(v.version_label)}</p><p>I confirm I have read the assigned document and understand that I should ask my manager if anything is unclear.</p><label class="check-row"><input id="docAckCheck" type="checkbox"> I confirm this acknowledgement.</label><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Acknowledge','primary',`data-confirm-sign-doc="${assignmentId}"`)}</div>`)}
...
function showTrainingDetails(id){
  const t=state.training.find(x=>x.id===id),files=state.trainingFiles.filter(f=>f.training_session_id===id),assigns=state.trainingAssignments.filter(a=>a.training_session_id===id&&a.active!==false),method=t.delivery_method||defaultTrainingDelivery(t.session_type);
  openModal('Training details',`<div class="stack"><div><strong>${esc(t.name)}</strong><div class="meta"><span>${esc(t.session_type.replaceAll('_',' '))}</span><span>${deliveryText(method)}</span><span>Trainer ${esc(t.trainer_name||'—')}</span><span>${renewalText(t.renewal_value,t.renewal_unit)}</span></div></div>${t.description?`<p>${esc(t.description)}</p>`:''}<div><strong>Files</strong><div class="row">${files.length?files.map(f=>btn(f.file_name,'secondary',`data-open-training-file="${f.id}"`)).join(''):'<span class="muted">No supporting files.</span>'}</div></div>${isManager()?`<div><strong>Attendance / sign-off</strong>${method==='INSTRUCTOR_LED'?`<p class="muted">Tick only the people you are recording now. Anyone left unticked remains awaiting training and can be completed later.</p>`:''}<div class="card-list">${assigns.length?assigns.map(a=>{const s=latestTrainingSignoff(a),c=latestTrainingConfirmation(a),event=latestTrainingEvent(a),ready=freshConfirmation(c,s),due=trainingAssignmentDue(a,s),done=!!s&&!(a.renewal_value&&new Date(due)<=new Date());let label=done?'Completed':method==='INSTRUCTOR_LED'?(ready?'Ready for trainee sign-off':event?.attendance_status==='ABSENT'?'Absent / not attended':'Awaiting instructor confirmation'):'Outstanding';return `<div class="item-card compact"><div class="row-between"><label class="check-row">${method==='INSTRUCTOR_LED'&&!done?`<input type="checkbox" class="attendance-person" value="${a.id}">`:''}<span>${esc(personName(a.user_id))}</span></label><span class="badge ${done?'complete':'due'}">${label}</span></div><div class="meta">${event?.delivery_date?`<span>Last attendance date ${fmtDate(event.delivery_date)}</span>`:''}${s?`<span>Signed ${fmtDateTime(s.signed_at)}</span>`:''}</div></div>`}).join(''):'<span class="muted">No one assigned.</span>'}</div>${method==='INSTRUCTOR_LED'&&assigns.length?`<div class="actions">${btn('Record selected attendance','primary',`data-bulk-training-attendance="${id}"`)}</div>`:''}</div>`:''}</div>`);
}
...
async function confirmSignTraining(id){
  ...
  const {error}=await sb.from('training_signoffs').insert({training_assignment_id:id,training_session_id:t.id,user_id:state.user.id,statement_snapshot:statement,training_name_snapshot:t.name,trainer_snapshot:t.trainer_name,delivered_date_snapshot:c?.delivery_date||t.delivered_date});
...
async function sendInvite(){
  ...
  const res=await fetch(`${CFG.supabaseUrl}/functions/v1/invite-user`,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${session.access_token}`,'apikey':CFG.supabaseKey},body:JSON.stringify({action:'invite',email:$('inviteEmail').value.trim(),display_name:$('inviteName').value.trim(),role:$('inviteRole').value,redirect_to:redirect})});
  const out=await res.json().catch(()=>({}));if(!res.ok)return toast(out.error||'Invite failed');closeModal();toast('Invitation sent. The user must set their own password.');
}
...
function complianceRows(){
  ...
  state.docAssignments.filter(a=>a.active!==false).forEach(a=>{
    ...
    const s=latestDocSignoff(a),due=documentAssignmentDue(a,s),method=d.delivery_method||defaultDocDelivery(d.doc_type),c=latestDocConfirmation(a),event=latestDocEvent(a),ready=freshConfirmation(c,s),needs=!s||(a.renewal_value&&new Date(due)<=now);
  ...
  state.trainingAssignments.filter(a=>a.active!==false).forEach(a=>{
    ...
    const s=latestTrainingSignoff(a),due=trainingAssignmentDue(a,s),method=t.delivery_method||defaultTrainingDelivery(t.session_type),c=latestTrainingConfirmation(a),event=latestTrainingEvent(a),ready=freshConfirmation(c,s),needs=!s||(a.renewal_value&&new Date(due)<=now);
    let code='COMPLETED',label='Completed';
    if(needs){if(due&&new Date(due)<now){code='OVERDUE';label='Overdue';}else if(method==='INSTRUCTOR_LED'){if(ready){code='READY_TO_SIGN';label='Ready for trainee sign-off';}else if(event?.attendance_status==='ABSENT'){code='ABSENT';label='Absent / not attended';}else{code='AWAITING_INSTRUCTOR';label='Awaiting instructor confirmation';}}else{code='OUTSTANDING';label=s?'Renewal due':'Not started';}}
    else if(due&&withinDays(due,30)){code='DUE_SOON';label='Due within 30 days';}
