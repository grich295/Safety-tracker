...
const BUILD_ID='v21013-chemical-coshh-linking-20260914';
...
function updateDocumentCreationUi(){
  ...
  if(nav){nav.textContent=`Create Safety Doc · ${enabled?'ON':'OFF'}`;nav.classList.toggle('creator-disabled',!enabled);nav.title=enabled?'Document creation is enabled.':'Document creation is currently disabled. Existing Documents, Register, links and Training remain available.';if(!enabled&&!isManager())nav.hidden=true;}
...
async function init(){
  ...
  $('loginForm').addEventListener('submit',login);$('contractorSignInBtn')?.addEventListener('click',()=>showContractorPortal('PUBLIC'));$('contractorBackBtn')?.addEventListener('click',closeContractorPortal);$('forgotPasswordBtn').addEventListener('click',forgotPassword);$('signOutBtn')?.addEventListener('click',forceLocalSignOut);$('adminUserModeBtn')?.addEventListener('click',toggleAdminUserMode);
  ...
  if('serviceWorker'in navigator)window.addEventListener('load',async()=>{
    try{
      ...
      const r=await navigator.serviceWorker.register('./sw-v21013.js',{updateViaCache:'none'});
...
async function showContractorPortal(route='PUBLIC'){$('authView').hidden=true;$('appView').hidden=true;$('demoView').hidden=true;$('contractorView').hidden=false;const box=$('contractorPortalContent');box.innerHTML='<div class="loading-note">Loading permit locations…</div>';try{const cfg=await invokePermit('public_config');state.siteLocations=cfg.locations||state.siteLocations;renderContractorForm(route,cfg.asbestosLoaded)}catch(e){box.innerHTML=`<div class="danger-note">${esc(e.message)}</div>`}}
