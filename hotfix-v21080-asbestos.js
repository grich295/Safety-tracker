/* Safety Tracker v2.10.81 - asbestos parser hardening */
'use strict';
(function(){
  ...
  function install(core){
    ...
    async function readPdf(file){
      ...
      for(var n=1;n<=pdf.numPages;n++){
        ...
        rows.sort(function(a,b){return b.y-a.y;});var lineObjs=rows.map(function(r){var a=r.a.sort(function(a,b){return a.x-b.x;});return {text:a.map(function(i){return i.s;}).join(' '),x:a.length?a[0].x:0,y:r.y};}),lines=lineObjs.map(function(r){return r.text;});
    ...
    var aliases=[
      ...
      ['extent',['extent / quantity','quantity','extent']],
      ...
      ['action_due_text',['action due date','due date','review due']],['sample_reference',['sample / evidence','sample reference','sample ref','sample no','sample number']],
      ['accessibility',['accessibility','access']],['applies_to_descendants',['applies to descendants','applies throughout','applies to sublocations']],['precise_location',['precise location','position within area','position']],['notes',['notes / comments','comments','notes']]
    ];
    var aliasRows=[];aliases.forEach(function(row){row[1].forEach(function(a){aliasRows.push({k:row[0],a:a});});});aliasRows.sort(function(a,b){return b.a.length-a.a.length;});
    function field(line){var s=clean(line);for(var i=0;i<aliasRows.length;i++){var a=aliasRows[i],rx=new RegExp('^'+a.a.replace(/[.*+?^$(){}|[\]\\]/g,'\\$&')+'(?=\\s|:|\\-|$)\\s*[:\\-]?\\s*','i');if(rx.test(s))return {k:a.k,v:s.replace(rx,'').trim()};}return null;}
    ...
    function isoDate(v){var s=clean(v);if(!s)return null;var m=s.match(/(\d{1,2})\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})/i);if(m){var d=new Date(m[2]+' '+m[1]+', '+m[3]+' 12:00:00');if(!isNaN(d))return d.toISOString().slice(0,10);}var d2=new Date(s);return isNaN(d2)?null:d2.toISOString().slice(0,10);}
    ...
    function parsePage(p,type){
      ...
      for(var i=0;i<objs.length;i++){var line=clean(objs[i].text),z=field(line);if(z)anchors.push({idx:i,k:z.k,x:Number(objs[i].x||0),y:Number(objs[i].y||0),parts:z.v?[{y:Number(objs[i].y||0),idx:i,text:z.v}]:[]});}
      if(!anchors.length)return null;
      for(var j=0;j<objs.length;j++){
        var obj=objs[j],txt=clean(obj.text);if(!txt||field(txt)||stopLine(txt))continue;
        var best=null,bestDist=1e9;
        for(var a=0;a<anchors.length;a++){
          var an=anchors[a];if(Number(obj.x||0)<=an.x+55)continue;var dist=Math.abs(Number(obj.y||0)-an.y);if(dist>34)continue;
          if(dist<bestDist){best=an;bestDist=dist;}
        }
        if(best)best.parts.push({y:Number(obj.y||0),idx:j,text:txt});
      }
      anchors.forEach(function(an){if(!an.parts.length)return;an.parts.sort(function(a,b){if(a.y===b.y)return a.idx-b.idx;return b.y-a.y;});var value=clean(an.parts.map(function(x){return x.text;}).join(' '));if(value)f[an.k]=clean((f[an.k]?f[an.k]+' ':'')+value);});
      if(!f.material||!f.raw_location)return null;var status=normStatus(f.identification_status||f.asbestos_type),no=status==='NOT_DETECTED',na=/not accessed|no access|inaccessible/i.test((f.material||'')+' '+(f.notes||'')+' '+(f.accessibility||'')),path=pathFor(f.raw_location),dueDate=isoDate(f.action_due_text);
      return {external_ref:f.external_ref||null,source_page:String(p.n),record_kind:na?'NO_ACCESS':no?'NON_ACM':'ACM',lifecycle_event:na?'NO_ACCESS':no?'NEGATIVE':type==='REINSPECTION'?'REINSPECT':type==='LAB_RESULT'?'CONFIRM':type==='REMOVAL'?'REMOVE':type==='CLEARANCE'?'CLEAR':'ADD_PRESENT',raw_location:f.raw_location,location_path:path.a,proposed_location_types:path.a.map(function(x,i){return i===0?'AREA':/floor/i.test(x)?'FLOOR':/^(room|bedroom|suite)/i.test(x)?'ROOM':'AREA';}),location_confidence:path.c,precise_location:f.precise_location||null,material:f.material,asbestos_type:f.asbestos_type||null,identification_status:na?'PRESUMED':status,extent:f.extent||null,condition:f.condition||null,surface_treatment:f.surface_treatment||null,material_assessment:f.material_assessment||null,priority_assessment:f.priority_assessment||null,risk_rating:f.risk_rating||null,management_action:f.management_action||null,action_due_date:dueDate,action_due_text:f.action_due_text||null,accessibility:f.accessibility||null,applies_to_descendants:/\byes\b|throughout|all/i.test(f.applies_to_descendants||''),sample_reference:f.sample_reference||null,notes:f.notes||null,confidence:.95,review_status:path.c>=.75?'READY':'REVIEW',excerpt:p.text.slice(0,6500)};
    }
