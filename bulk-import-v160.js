/* Safety Tracker v1.6.0 - client-side section PDF bulk importer */
(() => {
'use strict';
...
function metaFromText(text,kind){
  ...
  let version='1';const vm=text.match(/\bVersion\s*:?[\s]*([A-Za-z0-9._-]+)/i);if(vm)version=vm[1];
  ...
  return {refs,version,issue,review};
}
async function analyseOne(file,sourceIndex){
  ...
  if(starts.length){
    for(let n=0;n<starts.length;n++){
      const s=starts[n],end=(n+1<starts.length?starts[n+1].index-1:pages.length-1),segment=pages.slice(s.index,end+1),text=segment.map(x=>x.text).join(' '),meta=metaFromText(text,s.kind);
      const related=meta.refs.filter(r=>r!==s.ref && !r.startsWith('TBT-'));
      items.push({sourceIndex,sourceName:file.name,startPage:s.index+1,endPage:end+1,kind:s.kind,reference:s.ref,title:s.title||s.ref,version:meta.version||'1',issueDate:meta.issue,reviewDate:meta.review,relatedRefs:related,selected:true,mode:'CREATE',existingId:null});
    }
  }else{
    const allText=pages.map(x=>x.text).join(' '),single=inferSingle(allText,pages[0]?.lines||[],file.name),meta=metaFromText(allText,single.kind);
    items.push({sourceIndex,sourceName:file.name,startPage:1,endPage:pages.length,kind:single.kind,reference:single.ref,title:single.title,version:meta.version||'1',issueDate:meta.issue,reviewDate:meta.review,relatedRefs:meta.refs.filter(r=>!r.startsWith('TBT-')),selected:true,mode:'CREATE',existingId:null});
  }
  return items;
}
function existingFor(item){
  if(item.kind==='TOOLBOX_TALK'){
    const ref=item.reference.toUpperCase();return state.training.find(t=>ref?String(t.name||'').toUpperCase().startsWith(ref):clean(t.name).toLowerCase()===clean(item.title).toLowerCase())||null;
  }
  if(item.reference){const ref=item.reference.toUpperCase();const hit=state.documents.find(d=>String(d.reference||'').toUpperCase()===ref);if(hit)return hit}
  if(item.kind==='SDS'){const key=productWords(item.title).join(' ');return state.documents.find(d=>d.doc_type==='SDS'&&productWords(d.title).join(' ')===key)||null}
  return null;
}
function prepareExistingFlags(){
  for(const item of B.items){const x=existingFor(item);item.existingId=x?.id||null;if(x){item.mode='SKIP';item.selected=false}else{item.mode='CREATE';item.selected=true}}
}
...
function modeOptions(item){
  if(item.kind==='TOOLBOX_TALK')return `<option value="CREATE" ${item.mode==='CREATE'?'selected':''}>Create training</option><option value="SKIP" ${item.mode==='SKIP'?'selected':''}>Skip</option>`;
  if(item.existingId)return `<option value="SKIP" ${item.mode==='SKIP'?'selected':''}>Skip existing</option><option value="NEW_VERSION" ${item.mode==='NEW_VERSION'?'selected':''}>Publish as new version</option>`;
...
function renderPreview(){
  ...
  sum.innerHTML=Object.entries(counts).map(([k,n])=>`<div class="stat"><strong>${n}</strong><span>${escB(kindLabel(k))}${n===1?'':'s'}</span></div>`).join('');
  box.innerHTML=B.items.map((i,idx)=>`<div class="item-card bulk-preview-card" data-kind="${i.kind}" data-bulk-index="${idx}"><div class="row-between"><label class="check-row"><input class="bulk-select" type="checkbox" ${i.selected?'checked':''}> <strong>${escB(i.reference?`${i.reference} · ${i.title}`:i.title)}</strong></label><span class="badge ${i.existingId?'due':'complete'}">${i.existingId?'Already in tracker':'New'}</span></div><div class="meta"><span>${escB(i.sourceName)}</span><span>Pages ${i.startPage}${i.endPage!==i.startPage?`–${i.endPage}`:''}</span><span>${escB(methodLabel(i))}</span></div>${i.relatedRefs.length?`<div class="bulk-ref-list">Relevant refs: ${escB(i.relatedRefs.join(', '))}</div>`:''}<div class="bulk-grid"><label>Reference<input class="bulk-ref" value="${escB(i.reference)}" placeholder="Optional"></label><label class="wide">Title<input class="bulk-title" value="${escB(i.title)}"></label><label>Section<select class="bulk-kind">${typeOptions(i.kind)}</select></label><label>Action<select class="bulk-mode">${modeOptions(i)}</select></label>${i.kind!=='TOOLBOX_TALK'?`<label>Version<input class="bulk-version" value="${escB(i.version||'1')}"></label><label>Issue date<input class="bulk-issue" type="date" value="${escB(i.issueDate||'')}"></label>${i.kind!=='SDS'?`<label>Review date<input class="bulk-review" type="date" value="${escB(i.reviewDate||'')}"></label>`:''}`:''}</div></div>`).join('');
...
function bindPreviewEdits(){
  document.querySelectorAll('[data-bulk-index]').forEach(card=>{
    ...
    card.querySelector('.bulk-select').onchange=e=>{i.selected=e.target.checked;if(!i.selected)i.mode='SKIP';else if(i.mode==='SKIP')i.mode=i.existingId&&i.kind!=='TOOLBOX_TALK'?'NEW_VERSION':'CREATE';renderPreview()};
    card.querySelector('.bulk-ref').onchange=e=>i.reference=clean(e.target.value).toUpperCase();
    card.querySelector('.bulk-title').onchange=e=>i.title=clean(e.target.value);
    card.querySelector('.bulk-kind').onchange=e=>{i.kind=e.target.value;const x=existingFor(i);i.existingId=x?.id||null;i.mode=x?'SKIP':'CREATE';i.selected=!x;renderPreview()};
...
async function analyse(){
  ...
  try{
    ...
    prepareExistingFlags();renderPreview();setStatus(`Analysis complete: ${B.items.length} record${B.items.length===1?'':'s'} detected. Check the preview before importing.`);
...
async function createDocument(item,blob){
  ...
  const {error:ve}=await sb.from('document_versions').insert({document_id:d.id,version_label:ver,issue_date:item.issueDate||null,review_date:type==='SDS'?null:(item.reviewDate||null),delivery_method:delivery,storage_path:path,file_name:name,notes,status:'CURRENT',created_by:state.user.id});if(ve){await sb.storage.from('safety-files').remove([path]);await sb.from('documents').delete().eq('id',d.id);throw ve}
  item.dbDocId=d.id;
}
...
async function createTraining(item,blob){
  ...
  const {error:fe}=await sb.from('training_files').insert({training_session_id:t.id,file_name:fname,storage_path:path,uploaded_by:state.user.id});if(fe)throw fe;item.dbTrainingId=t.id;
}
...
async function importSelected(){
  ...
  try{
    for(const item of items){
      try{setStatus(`Importing ${done+1} of ${items.length}: ${item.reference||item.title}…`);const blob=await splitPdf(item);if(item.kind==='TOOLBOX_TALK')await createTraining(item,blob);else if(item.mode==='NEW_VERSION')await publishVersion(item,blob);else await createDocument(item,blob);done++}
      catch(e){failed++;item.error=e.message||String(e);console.error('Bulk import item failed',item,e)}
    }
    setStatus('Creating document links…');await autoLink();await refresh();prepareExistingFlags();renderPreview();
