...
const STANDARD_USER_VIEWS=new Set(['mySafety','awareness','ppe','onsite','help']);
const MANAGER_VIEWS=new Set(['mySafety','documents','creator','training','awareness','ppe','onsite','asbestos','people','compliance','instructor','reports','help']);
...
function renderMySafety(){
  renderMySafetyOnSiteShortcut();
  renderMySafetyKeyShortcut();
  ...
  const stats=[
    {key:'COMPLETE',label:'Complete',value:completed,traffic:'green'},
    {key:'ACTION',label:'Action required',value:action,traffic:overdue?'red':action?'amber':'green'},
    {key:'OVERDUE',label:'Overdue / blocked',value:overdue,traffic:overdue?'red':'green'},
    {key:'WAITING',label:'Awaiting instructor',value:waiting,traffic:waiting?'amber':'green'}
  ];
  $('mySafetyStats').innerHTML=stats.map(x=>`<button type="button" class="stat my-safety-stat-button traffic-${x.traffic}${mySafetyStatusView===x.key?' active':''}" data-my-safety-status="${x.key}" aria-pressed="${mySafetyStatusView===x.key?'true':'false'}"><span class="traffic-dot" aria-hidden="true"></span><strong>${x.value}</strong><span>${x.label}</span></button>`).join('');

  const list=$('mySafetyList');
  if(!mySafetyStatusView){
    list.innerHTML='';
    list.hidden=true;
  }else{
    list.hidden=false;
    const selectedRows=rows.filter(r=>mySafetyStatusMatch(r,mySafetyStatusView));
    const counts={ALL:selectedRows.length,COSHH:0,RA:0,SSW:0,TBT:0,AWARENESS:0,OTHER:0};
    selectedRows.forEach(r=>counts[mySafetyCategory(r.t)]++);
    const categories=[
      ['ALL','All'],
      ['COSHH','COSHH RA'],
      ['RA','Risk Assessments'],
    ...
    if(mySafetyCategoryView!=='ALL'&&!counts[mySafetyCategoryView])mySafetyCategoryView='ALL';
    const filtered=selectedRows.filter(r=>mySafetyCategoryView==='ALL'||mySafetyCategory(r.t)===mySafetyCategoryView);
    const statusTitle={COMPLETE:'Complete',ACTION:'Action required',OVERDUE:'Overdue / blocked',WAITING:'Awaiting instructor'}[mySafetyStatusView]||'Training';
    const categoryButtons=categories.map(([key,label])=>`<button type="button" class="my-safety-category-button${mySafetyCategoryView===key?' active':''}" data-my-safety-category="${key}"><strong>${esc(label)}</strong><span>${counts[key]} item${counts[key]===1?'':'s'}</span></button>`).join('');
    const cards=filtered.length?filtered.map(({a,t,status,traffic})=>{
    ...
    }).join(''):'<div class="success-note">No training items in this section.</div>';
    list.innerHTML=`<div class="section-card my-safety-drilldown"><div class="row-between my-safety-drilldown-head"><div><h3>${esc(statusTitle)}</h3><p class="muted">Choose a training type to narrow the list.</p></div><button type="button" class="secondary" data-my-safety-back>Back to summary</button></div><div class="my-safety-category-grid">${categoryButtons}</div></div>${cards}`;
  }
  renderMyAwareness();
  renderMyPpe();
}
