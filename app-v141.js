...
function showNewDocument(){
  ...
  // v1.4.1: bind this modal action directly rather than relying only on delegated clicks.
  $('saveDocument').addEventListener('click',e=>{e.stopPropagation();saveDocument();});
}
...
async function saveDocument(){
  ...
  try{
    ...
    if(!$('docTitle')?.value.trim()||!file)return toast('Title and file are required.');
    ...
    toast(err?.message||'Could not create document.');
    if(saveBtn){
