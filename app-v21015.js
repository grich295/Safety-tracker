/* Safety Tracker v2.9.14 - unified contractor access/key control on PTW and non-PTW sign-in. */
'use strict';

const APP_VERSION='2.10.15';
const BUILD_ID='v21015-evidence-ui-cleanup-20260914';
...
function updateDocumentCreationUi(){
  const enabled=documentCreationEnabled(),nav=document.querySelector('#mainNav button[data-view="creator"]'),toggle=$('documentCreationToggleBtn'),note=$('documentCreationStateNote');
  ...
  if(toggle){toggle.hidden=!isManager();toggle.textContent=`Document Creation: ${enabled?'ON':'OFF'}`;toggle.classList.toggle('danger',enabled);toggle.classList.toggle('primary',!enabled);}
  if(note){note.className=enabled?'success-note':'pending-use-warning';note.innerHTML=enabled?'<strong>Document creation is enabled.</strong> Managers/Admins can create new RA, COSHH RA, SSW and Toolbox Talk drafts.':'<strong>Document creation is OFF.</strong> Existing documents, approvals, Register, links and Training continue normally. Turn creation back on to start or generate new safety documents.';}
...
function evidenceDocumentPickerRows(preselected=[]){const sel=new Set(preselected),groups=['SDS / MSDS','COSHH RA','Risk Assessments','Safe Systems of Work','Other controlled documents'],rows=state.documents.filter(d=>d.status!=='ARCHIVED').sort((a,b)=>String(a.reference||documentDisplayTitle(a)).localeCompare(String(b.reference||documentDisplayTitle(b)),undefined,{numeric:true}));return groups.map(group=>{const items=rows.filter(d=>evidenceDocGroup(d)===group);if(!items.length)return '';return `<div class="evidence-group" data-evidence-group="${esc(group)}"><div class="evidence-group-title">${esc(group)} <span class="muted">${items.length}</span></div>${items.map(d=>{const v=approvedCurrentVersion(d.id)||pendingApprovalVersion(d.id)||latestVersion(d.id),status=v?versionApprovalLabel(v,d):'No version';return `<label class="check-row evidence-choice" data-evidence-search="${esc(`${d.reference||''} ${documentDisplayTitle(d)} ${docTypeLabel(d.doc_type)} ${status}`.toLowerCase())}"><input type="checkbox" class="evidence-doc-choice" value="${d.id}" ${sel.has(d.id)?'checked':''}><span class="evidence-choice-text"><strong>${esc(d.reference||docTypeLabel(d.doc_type))}</strong> · ${esc(documentDisplayTitle(d))}<br><span class="muted">${esc(docTypeLabel(d.doc_type))} · ${esc(status)}</span><span class="evidence-auto-note" hidden>Auto-selected because it is linked to your selection</span></span></label>`}).join('')}</div>`}).join('')}
function evidenceTrainingPickerRows(preselected=[]){const sel=new Set(preselected);return state.training.filter(t=>t.status!=='ARCHIVED'&&!t.source_document_id).sort((a,b)=>String(trainingReference(a)||a.name).localeCompare(String(trainingReference(b)||b.name),undefined,{numeric:true})).map(t=>`<label class="check-row evidence-choice" data-evidence-search="${esc(`${trainingReference(t)||''} ${t.name} ${kindLabel(trainingKind(t))}`.toLowerCase())}"><input type="checkbox" class="evidence-training-choice" value="${t.id}" ${sel.has(t.id)?'checked':''}><span class="evidence-choice-text"><strong>${esc(trainingReference(t)||kindLabel(trainingKind(t)))}</strong> · ${esc(t.name)}<br><span class="muted">Standalone ${esc(kindLabel(trainingKind(t)))}</span><span class="evidence-auto-note" hidden>Auto-selected because it is linked to your selection</span></span></label>`).join('')}
function wireEvidenceSearch(){const q=$('evidenceItemSearch');if(q)q.addEventListener('input',()=>{const s=clean(q.value).toLowerCase();document.querySelectorAll('.evidence-choice').forEach(x=>x.hidden=!!s&&!String(x.dataset.evidenceSearch||'').includes(s));document.querySelectorAll('.evidence-group').forEach(g=>{const choices=[...g.querySelectorAll('.evidence-choice')];g.hidden=choices.length>0&&choices.every(x=>x.hidden)})});wireEvidenceLinkedSelection()}
function setEvidenceChoices(selector,checked){document.querySelectorAll(selector).forEach(x=>{if(!x.closest('[hidden]')){x.checked=checked;markEvidenceAutoSelected(x,false)}});updateEvidenceLinkedSummary()}
function markEvidenceAutoSelected(input,on){if(!input)return;input.dataset.autoSelected=on?'true':'';const note=input.closest('.evidence-choice')?.querySelector('.evidence-auto-note');if(note)note.hidden=!on;input.closest('.evidence-choice')?.classList.toggle('auto-linked',!!on)}
...
function expandEvidenceLinkedSelection(rootId){const maxDepth=3,seen=new Set([rootId]),queue=[{id:rootId,depth:0}],addedDocs=[];while(queue.length){const cur=queue.shift();if(cur.depth>=maxDepth)continue;for(const id of evidenceDirectDocIds(cur.id)){if(seen.has(id))continue;seen.add(id);addedDocs.push(id);queue.push({id,depth:cur.depth+1})}}for(const id of addedDocs){const box=document.querySelector(`.evidence-doc-choice[value="${CSS.escape(id)}"]`);if(box&&!box.checked){box.checked=true;markEvidenceAutoSelected(box,true)}}for(const tid of evidenceLinkedTrainingIds([...seen])){const box=document.querySelector(`.evidence-training-choice[value="${CSS.escape(tid)}"]`);if(box&&!box.checked){box.checked=true;markEvidenceAutoSelected(box,true)}}const count=addedDocs.length+evidenceLinkedTrainingIds([...seen]).length;if(count)updateEvidenceLinkedSummary()}
function updateEvidenceLinkedSummary(){const box=$('evidenceLinkedSummary');if(!box)return;const auto=[...document.querySelectorAll('.evidence-doc-choice[data-auto-selected="true"],.evidence-training-choice[data-auto-selected="true"]')].filter(x=>x.checked);box.hidden=!auto.length;box.innerHTML=auto.length?`<strong>${auto.length} linked item${auto.length===1?'':'s'} auto-selected.</strong> You can untick any item you do not want in this evidence pack.`:''}
function wireEvidenceLinkedSelection(){document.querySelectorAll('.evidence-doc-choice').forEach(box=>box.addEventListener('change',()=>{if(box.checked){markEvidenceAutoSelected(box,false);expandEvidenceLinkedSelection(box.value)}else markEvidenceAutoSelected(box,false);updateEvidenceLinkedSummary()}));document.querySelectorAll('.evidence-training-choice').forEach(box=>box.addEventListener('change',()=>{if(!box.checked)markEvidenceAutoSelected(box,false);else markEvidenceAutoSelected(box,false);updateEvidenceLinkedSummary()}));const initial=[...document.querySelectorAll('.evidence-doc-choice:checked')].map(x=>x.value);for(const id of initial)expandEvidenceLinkedSelection(id);updateEvidenceLinkedSummary()}
function showEvidencePackPicker(preselectedDocs=[],preselectedTraining=[]){if(!isManager())return;openModal('Person evidence pack',`<div class="hint-box"><strong>Linked evidence is selected for you.</strong> Select an SDS/MSDS, COSHH RA, RA or other controlled document and Safety Tracker will also tick relevant documents connected through the controlled-link chain. Direct document relationships are not changed; this only helps build the report. You can untick anything you do not want.</div><div id="evidenceLinkedSummary" class="success-note" hidden></div><div class="form-grid"><label class="full">Person<select id="evidencePerson"><option value="">Select person…</option>${evidencePersonOptions()}</select></label><label>History<select id="evidenceScope"><option value="CURRENT">Current approved version only</option><option value="FULL">Full version history</option></select></label><label>From date (optional)<input id="evidenceStart" type="date"></label><label>To date (optional)<input id="evidenceEnd" type="date"></label><label class="full">Search documents / items<input id="evidenceItemSearch" placeholder="Search reference, product, title or type"></label></div><div class="section-card"><div class="row-between"><h4>Controlled documents</h4><div class="row"><button class="secondary small" type="button" data-evidence-select-docs="1">Select all</button><button class="ghost small" type="button" data-evidence-clear-docs="1">Clear</button></div></div><div class="checkbox-list evidence-picker-list evidence-grouped-list">${evidenceDocumentPickerRows(preselectedDocs)}</div></div><div class="section-card"><div class="row-between"><h4>Toolbox Talks / standalone training</h4><div class="row"><button class="secondary small" type="button" data-evidence-select-training="1">Select all</button><button class="ghost small" type="button" data-evidence-clear-training="1">Clear</button></div></div><div class="checkbox-list evidence-picker-list">${evidenceTrainingPickerRows(preselectedTraining)||'<span class="muted">No standalone training items.</span>'}</div></div><div class="actions"><button class="secondary" type="button" data-evidence-select-all="1">Select all sections</button>${btn('Cancel','ghost','data-close-modal')}${btn('Create evidence pack','primary','data-generate-evidence-selected')}</div>`);wireEvidenceSearch()}
...
function downloadReport(kind){
  try{
    ...
    if(kind==='outstanding'){
      ...
      const rows=(state.trainingSignoffs||[]).slice().sort((a,b)=>new Date(b.signed_at||0)-new Date(a.signed_at||0));doc=reportPdfBase('Safety Tracker - Training Sign-offs');doc.autoTable({head:[['Person','Training','Acknowledged by','Completed','Method']],body:rows.length?rows.map(r=>{const a=state.trainingAssignments.find(x=>x.id===r.training_assignment_id),t=state.training.find(x=>x.id===a?.training_session_id);return [personName(a?.user_id),t?(trainingReference(t)?`${trainingReference(t)} - ${t.name}`:t.name):'Training record',r.signature_name||r.signed_name||'',fmtDateTime(r.signed_at),deliveryText(effectiveTrainingMethod(t,a))]}):[['No training sign-offs','','','','']],startY:28,styles:{fontSize:7},margin:{left:14,right:14}});name=`Safety-Tracker-Training-Signoffs-${todayISO()}.pdf`;
...
async function downloadTrainingExcel(){
  ...
  try{
    ...
    ws.columns=[
      ...
      {header:'Version',key:'version',width:14},{header:'Signed Name',key:'signed',width:24},{header:'Instructor',key:'instructor',width:24}
    ];
