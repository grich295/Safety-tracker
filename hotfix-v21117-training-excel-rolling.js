/* Safety Tracker v2.11.17 - existing Training Excel Report becomes rolling tracker */
'use strict';
(function(){
  if(window.__SAFETY_TRAINING_EXCEL_ROLLING_V21117)return;
  window.__SAFETY_TRAINING_EXCEL_ROLLING_V21117=true;
  ...
  function install(api){
    ...
    async function buildWorkbook(){
      ...
      wb.title='Safety Tracker Training Tracker';
      ...
      wb.description=`Rolling Training Tracker. ${hist.length} historical completion record${hist.length===1?'':'s'} through ${new Date().toLocaleDateString('en-GB')}.`;
    ...
    async function downloadRollingTrainingExcel(){
      ...
      if(b){b.disabled=true;b.textContent='Creating Training Tracker…'}
      try{
        const {wb,historyCount,currentCount}=await buildWorkbook();
        const bytes=await wb.xlsx.writeBuffer();
        downloadBlob(
          new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),
          `Safety-Tracker-Training-Tracker-${todayISO()}.xlsx`
        );
        api.toast?.(`Training Tracker downloaded — ${historyCount} historical completion${historyCount===1?'':'s'} and ${currentCount} current assignment${currentCount===1?'':'s'}.`);
      }catch(e){
        console.error('v2.11.17 training tracker',e);
        api.toast?.(`Training Excel failed: ${e?.message||e}`);
      }finally{
        if(b){b.disabled=false;b.textContent='Training Excel Tracker'}
      }
    }

    // Replace the existing report; do not add a second Excel report.
    const b=$('trainingExcelBtn');
    if(b)b.textContent='Training Excel Tracker';
    ...
    window.SafetyTrainingExcelV21117={
      buildWorkbook,
