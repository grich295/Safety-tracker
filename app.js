/* Safety Tracker v2.3.8 - login bootstrap recovery + simplified user access. */
'use strict';

const APP_VERSION='2.3.8';
const BUILD_ID='v238-login-recovery-20260911';
...
const isStandardUser=()=>state.profile?.role==='user'&&!isReportViewer();
...
async function init(){
  ...
  try{const {data:{session},error}=await sb.auth.getSession();if(error)throw error;if(session)await enterApp(session.user);else showLogin();if(boot&&!session)boot.textContent='Ready to sign in.';}catch(e){if(boot)boot.textContent='Sign-in service error.';showLogin();showAuthMessage(e?.message||'Could not initialise sign-in. Refresh and try again.');}
...
async function enterApp(user){state.user=user;if(user?.user_metadata?.must_set_password===true)return showMandatoryPasswordSetup(user);const {data,error}=await sb.from('profiles').select('*').eq('id',user.id).single();if(error||!data){showAuthMessage('Profile not found. Ask an administrator to check the Safety Tracker profile.');await sb.auth.signOut();return}if(data.active===false){showAuthMessage('This Safety Tracker account is disabled.');await sb.auth.signOut();return}state.profile=data;if(isReportViewer())await loadReportViewerData();else await loadAll();$('authView').hidden=true;$('appView').hidden=false;$('currentUserName').textContent=data.display_name||user.email;$('currentUserRole').textContent=isReportViewer()?'Report Viewer':isAdmin()?'Admin':isManager()?'Manager':'User';document.querySelectorAll('.manager-only').forEach(el=>el.hidden=!isManager());document.querySelectorAll('.admin-only').forEach(el=>el.hidden=!isAdmin());document.querySelectorAll('#mainNav button[data-view]').forEach(b=>b.hidden=!canAccessView(b.dataset.view));const docIntro=$('documentsIntro');if(docIntro)docIntro.textContent='Controlled source library. Training and sign-off are managed in Training.';const docStatus=$('documentStatusFilter');if(docStatus&&isStandardUser()){docStatus.innerHTML='<option value="APPROVED">Approved/current</option>';docStatus.value='APPROVED';}const initialView=isReportViewer()?'reports':'mySafety';seedSafetyNavigation(initialView);showView(initialView,{push:false})}
...
function renderMySafety(){
  ...
  $('mySafetyList').innerHTML=rows.length?rows.map(({a,t,status})=>{
    ...
    const requiredBtn=materials.length?btn(status.code==='COMPLETED'?'Open safety document':(materialOpened?'Source file opened ✓':'Open source file'),'secondary',`data-open-required-training="${a.id}"`):'';
...
async function openTrainingFile(fileId){const f=state.trainingFiles.find(x=>x.id===fileId);if(!f?.storage_path)return toast('Stored Toolbox Talk PDF not found.');const t=state.training.find(x=>x.id===f.training_session_id);if(!isManager()&&(!t||t.status==='ARCHIVED'||!['APPROVED','LEGACY'].includes(trainingApprovalStatus(t))))return toast('Only approved/current Toolbox Talks are available.');const r=await sb.storage.from('safety-files').createSignedUrl(f.storage_path,300);if(r.error)return toast(r.error.message);window.open(r.data.signedUrl,'_blank','noopener');await logDocumentActivity('OPENED',{...activityTrainingFileSnapshot(f),source_context:'TOOLBOX_TALK_APPROVAL'});renderDocuments();renderTraining();}
...
function showTrainingApproval(id){if(!isManager())return;const t=state.training.find(x=>x.id===id);if(!t||trainingKind(t)!=='TOOLBOX_TALK')return;const st=trainingApprovalStatus(t);if(st!=='PENDING')return toast(st==='APPROVED'?'This Toolbox Talk is already approved.':'This Toolbox Talk is not pending approval.');const f=latestTrainingFile(t.id);if(!f)return toast('No Toolbox Talk PDF is attached.');const opened=trainingFileHasBeenOpened(f.id);openModal('Review & approve Toolbox Talk',`<div class="section-card training-catalogue-card traffic-amber"><h3>${esc(trainingReference(t)?trainingReference(t)+' - '+t.name:t.name)}</h3><div class="meta"><span class="badge due">Pending approval</span><span>${esc(deliveryText(t.delivery_method||'INSTRUCTOR_LED'))}</span><span>Review ${fmtDate(t.review_date)}</span></div><div class="${opened?'success-note':'pending-use-warning'}">${opened?'Exact Toolbox Talk PDF opened ✓':'Open the exact Toolbox Talk PDF before approval.'}</div><div class="row">${btn('Open Toolbox Talk PDF','secondary',`data-open-training-file="${f.id}"`)}</div></div><label>Approval note (optional)<textarea id="trainingApprovalNote" placeholder="Record any checks or conditions."></textarea></label>${signatureBlock('trainingApproval')}<label class="check-row"><input id="trainingApprovalAck" type="checkbox"> I confirm I have reviewed this Toolbox Talk and approve it for controlled use and training assignment.</label><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Approve Toolbox Talk','primary',`data-confirm-training-approval="${t.id}"`)}</div>`);setupSignaturePad('trainingApprovalSignaturePad','trainingApprovalClearSignature')}
async function confirmTrainingApproval(id){if(!isManager())return;const t=state.training.find(x=>x.id===id),f=latestTrainingFile(id);if(!t||!f)return;if(trainingApprovalStatus(t)!=='PENDING')return toast('This Toolbox Talk is no longer pending approval.');if(!trainingFileHasBeenOpened(f.id))return toast('Open the exact Toolbox Talk PDF before approving it.');if(!$('trainingApprovalAck')?.checked)return toast('Tick the approval confirmation first.');const sig=signatureData('trainingApprovalSignaturePad'),name=clean($('trainingApprovalSignatureName')?.value),note=clean($('trainingApprovalNote')?.value);if(!sig||!name)return toast('Digital signature and signature name are required.');const r=await sb.rpc('approve_training_session',{p_training_session_id:id,p_signature_data:sig,p_signature_name:name,p_approval_note:note||null});if(r.error)return toast(r.error.message);closeModal();await refresh('Toolbox Talk approved. It can now be assigned and used for training.')}
...
function renderToolboxTalkDocumentIndex(q,status){
  ...
  $('documentsList').innerHTML=rows.length?rows.map(t=>{
    ...
    const msg=!active?'Archived Toolbox Talk — retained for history and not available for new assignments.':pending?'Pending management approval — open and review the exact PDF, then digitally approve it before assignment.':approved?'Approved Toolbox Talk — available for assignment, attendance and sign-off in Training.':'Toolbox Talk is not approved for training use.';
    return `<div class="item-card training-catalogue-card traffic-${traffic}"><div class="row-between"><div><h3>${esc(trainingReference(t)&&!t.name.toUpperCase().includes(trainingReference(t))?trainingReference(t)+' - '+t.name:t.name)}</h3><div class="meta"><span class="badge">Toolbox Talk</span>${approvalBadge}<span class="badge ${active?'complete':''}">${active?'Active':'Archived'}</span><span>Review ${fmtDate(t.review_date)}</span><span>${esc(deliveryText(t.delivery_method||'INSTRUCTOR_LED'))}</span></div></div><div class="meta"><span class="badge">${assigns.length} assigned</span></div></div><div class="muted">${msg}</div>${t.review_required&&t.review_reason?`<div class="request-note">${esc(t.review_reason)}</div>`:''}<div class="row">${f?btn('Open file','secondary',`data-open-training-file="${f.id}"`)+btn('Download','secondary',`data-download-training-file="${f.id}"`):''}${btn('Activity','ghost',`data-training-file-activity="${t.id}"`)}${btn('View training','secondary',`data-view-training="${t.id}"`)}${isManager()&&active&&pending?btn('Review & approve','primary',`data-approve-training="${t.id}"`):''}${isManager()&&active&&approved?btn('Assign','primary',`data-assign-training="${t.id}"`):''}${isManager()?btn(active?'Archive':'Restore / activate','ghost',`data-archive-training="${t.id}"`):''}</div></div>`
  }).join(''):'<div class="empty">No Toolbox Talks found.</div>';
}
...
async function saveTrainingAssignments(id){
  const t=state.training.find(x=>x.id===id),deps=trainingDependencyState(t);if(trainingKind(t)==='TOOLBOX_TALK'&&!['APPROVED','LEGACY'].includes(trainingApprovalStatus(t)))return toast('Approve this Toolbox Talk before assigning it.');if(!deps.ready)return toast(trainingDependencyMessage(t));
...
function renderHelp(){
  if(isStandardUser()){
    $('helpContent').innerHTML=`<div class="help-card"><h3>Keep it simple</h3><p>Your main page is <strong>My Safety</strong>. It shows anything you personally need to complete. You do not need to use the management Training catalogue.</p></div><div class="help-card"><h3>My Safety</h3><p>Open the required file, complete the training or attendance step shown, and sign only when you understand the information. If you need help or extra instruction, ask your manager before carrying out the task.</p></div><div class="help-card"><h3>Documents</h3><p>Your assigned safety documents stay available from My Safety after completion.</p></div><div class="help-card"><h3>Safety Awareness</h3><p>Complete any awareness reading shown as due. These short reviews support formal training but do not replace task-specific instruction.</p></div><div class="help-card"><h3>PPE Checks</h3><p>Complete your monthly PPE check and report anything missing, damaged or unsuitable.</p></div>`;
    return;
  }
  $('helpContent').innerHTML=`<div class="help-card"><h3>Documents</h3><p>Documents is the central controlled library. New RA, COSHH RA, SSW and other controlled versions are clearly shown as Pending approval, Approved/current, Superseded or Archived. SDS/MSDS uses Accepted/current wording.</p></div><div class="help-card"><h3>Training</h3><p>Managers/Admins use the Training catalogue to manage approved/current RA, COSHH RA, SSW and Toolbox Talk training. Standard Users work from My Safety only, keeping their interface simpler.</p></div><div class="help-card"><h3>Role access</h3><p>Users see My Safety, Safety Awareness, PPE Checks and Help. Managers additionally see Training, People, Compliance, Instructor and Reports. Admins additionally see Admin controls. Report Viewer accounts see Reports only.</p></div><div class="help-card"><h3>Pending approval & traffic lights</h3><p>New or revised controlled documents are not authorised for use or Training until a Manager/Admin opens the exact PDF and digitally approves it. Green means approved/current, amber means action such as pending approval or review due soon, red means overdue/not approved, and grey means archived or historical.</p></div><div class="help-card"><h3>Person evidence pack</h3><p>Managers/Admins open Reports → Person Evidence Pack, choose a person and select the documents/training records needed. Safety Tracker collects the matching assignments, signatures, attendance and audit evidence.</p></div><div class="help-card"><h3>Departments</h3><p>Admins create, rename and archive Departments in Admin. Each user can be assigned to a Department from People. Moving a user recalculates department-based training requirements while preserving previous evidence.</p></div><div class="help-card"><h3>Version check</h3><p>The header must show <strong>v2.3.8 CLEAN</strong>.</p></div>`
}
