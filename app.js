/* Safety Tracker v2.1.8 - SDS title cleanup + persistent Training filters + archive unlink/restore relink. */
'use strict';

const APP_VERSION='2.1.8';
const BUILD_ID='sds-filter-archive-links-v218-20260910';
...
async function toggleDocument(id){
  ...
  const restoring=d.status==='ARCHIVED',status=restoring?'ACTIVE':'ARCHIVED';
  const {error}=await sb.from('documents').update({status}).eq('id',id);if(error)return toast(error.message);
  if(!restoring){
    // v2.1.8 SQL also enforces this server-side. Do it here as well so the UI is immediate
    // even while a deployment is being completed.
    const dl=state.documentLinks.filter(l=>l.source_document_id===id||l.target_document_id===id).map(l=>l.id).filter(Boolean);
    const tl=state.trainingDocumentLinks.filter(l=>l.document_id===id).map(l=>l.id).filter(Boolean);
    try{if(dl.length)await deleteRowsByIds('document_links',dl);if(tl.length)await deleteRowsByIds('training_document_links',tl)}catch(e){console.warn('archive unlink',e)}
    await refresh('Document archived and removed from active links. Historical versions, activity and training evidence were retained.');
    return;
  }
  // Restore the auto-managed Training record for the same current version when one exists,
  // then rebuild declared relationships from the current active files rather than reviving old links.
  await loadAll();
