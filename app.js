/* Safety Tracker v2.1.9 - person-specific evidence packs + reliable archive actions. */
'use strict';

const APP_VERSION='2.1.9';
const BUILD_ID='evidence-pack-archive-fix-v219-20260910';
...
const state={
  ...
  historicalDocAssignments:[],historicalDocSignoffs:[],historicalDocConfirmations:[],documentActivity:[],settings:[],loadErrors:{},syncBusy:false,documentIndex:'ALL'
};
...
async function downloadDocument(versionId){const v=state.versions.find(x=>x.id===versionId);if(!v?.storage_path)return toast('Stored PDF not found.');const r=await sb.storage.from('safety-files').download(v.storage_path);if(r.error||!r.data)return toast(r.error?.message||'Download failed.');downloadBlob(r.data,v.file_name||'safety-document.pdf');await logDocumentActivity('DOWNLOADED',{...activityVersionSnapshot(v),source_context:'DOCUMENT_LIBRARY'});}
function showMarkDocumentReviewed(versionId){const v=state.versions.find(x=>x.id===versionId),d=state.documents.find(x=>x.id===v?.document_id);if(!v||!d)return;openModal('Mark file as reviewed',`<p><strong>${esc(d.reference?d.reference+' - '+documentDisplayTitle(d):documentDisplayTitle(d))}</strong> · v${esc(v.version_label||'—')}</p><div class="hint-box">This records that you reviewed this file. It does <strong>not</strong> complete any assigned training.</div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Confirm reviewed','primary',`data-confirm-doc-reviewed="${v.id}"`)}</div>`)}
async function confirmDocumentReviewed(versionId){const v=state.versions.find(x=>x.id===versionId);const row=await logDocumentActivity('REVIEWED',{...activityVersionSnapshot(v),source_context:'DOCUMENT_LIBRARY'},null,true);if(!row)return;closeModal();toast('File review recorded.');}
async function downloadTrainingFile(id){const f=state.trainingFiles.find(x=>x.id===id);if(!f?.storage_path)return toast('Stored training file not found.');const r=await sb.storage.from('safety-files').download(f.storage_path);if(r.error||!r.data)return toast(r.error?.message||'Download failed.');downloadBlob(r.data,f.file_name||'training-file.pdf');await logDocumentActivity('DOWNLOADED',{...activityTrainingFileSnapshot(f),source_context:'TRAINING_FILE'});}
function showMarkTrainingFileReviewed(id){const f=state.trainingFiles.find(x=>x.id===id),t=state.training.find(x=>x.id===f?.training_session_id);if(!f||!t)return;openModal('Mark file as reviewed',`<p><strong>${esc(trainingReference(t)?trainingReference(t)+' - '+t.name:t.name)}</strong></p><div class="hint-box">This records that you reviewed the file outside the formal training sign-off process. It does <strong>not</strong> complete assigned training.</div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Confirm reviewed','primary',`data-confirm-training-file-reviewed="${f.id}"`)}</div>`)}
async function confirmTrainingFileReviewed(id){const f=state.trainingFiles.find(x=>x.id===id);const row=await logDocumentActivity('REVIEWED',{...activityTrainingFileSnapshot(f),source_context:'TRAINING_FILE'},null,true);if(!row)return;closeModal();toast('File review recorded.');}
...
function cleanSdsCandidate(s){
  ...
  // Stop at metadata/regulatory text that often follows the actual Section 1.1 product name.
  t=t.replace(/\s+(?:Product code|Article(?: No\.?| number)?|UFI|REACH(?: registration)?|Relevant identified uses|Details of the supplier|1\.2\.?\b|SECTION\s+1\b|Revision(?: date)?|Version|Print date|Date of issue|According to|In accordance with|Conforms? to|COMMISSION REGULATION|REGULATION \(EU\)|REGULATION \(EC\)).*$/i,'');
  // Common PDF text-extraction artefact: the next subsection number is appended to the name.
  t=t.replace(/\s*[:;,-]?\s*1\.1(?:\.\d+)?\s*$/i,'');
  t=t.replace(/\s*[:;,-]?\s*1\.2(?:\.\d+)?\s*$/i,'');
  t=t.replace(/[\s:;|,\-–—]+$/,'').trim();
...
function extractSdsProductName(text){
  const t=clean(text);
  const patterns=[
    /\b1\.1\.?\s*Product identifier\s*(?:Product name\s*)?:?\s*([\s\S]{3,180}?)(?=\s+(?:1\.2\.?\b|Product code\b|UFI\b|REACH\b|Relevant identified uses\b|SECTION\s+1\b|Details of the supplier\b|According to\b|In accordance with\b|Conforms? to\b|COMMISSION REGULATION\b|REGULATION \(?:EU|EC\)\b))/i,
    /\b1\.1\.?\s*Product name\s*:?\s*([\s\S]{3,180}?)(?=\s+(?:1\.2\.?\b|Product code\b|UFI\b|REACH\b|Relevant identified uses\b|SECTION\b|Details of the supplier\b|According to\b|In accordance with\b|Conforms? to\b|COMMISSION REGULATION\b|REGULATION \(?:EU|EC\)\b))/i,
    /\bProduct name\s*:?\s*([\s\S]{3,180}?)(?=\s+(?:Product code\b|Product identifier\b|UFI\b|1\.2\.?\b|Relevant identified uses\b|SECTION\b|Details of the supplier\b|According to\b|In accordance with\b|Conforms? to\b|COMMISSION REGULATION\b|REGULATION \(?:EU|EC\)\b))/i,
    /\bProduct identifier\s*:?\s*([\s\S]{3,180}?)(?=\s+(?:Product code\b|UFI\b|1\.2\.?\b|Relevant identified uses\b|SECTION\b|Details of the supplier\b|According to\b|In accordance with\b|Conforms? to\b|COMMISSION REGULATION\b|REGULATION \(?:EU|EC\)\b))/i,
...
async function init(){
  ...
  $('forceSyncBtn').addEventListener('click',forceSyncFromUI);$('repairSdsTitlesBtn')?.addEventListener('click',repairSdsTitles);$('repairRaTitlesBtn')?.addEventListener('click',repairRaTitles);$('outstandingPdfBtn').addEventListener('click',()=>downloadReport('outstanding'));$('documentActivityPdfBtn')?.addEventListener('click',downloadDocumentActivityPdf);$('trainingMatrixPdfBtn').addEventListener('click',()=>downloadReport('matrix'));$('trainingSignoffsPdfBtn').addEventListener('click',()=>downloadReport('signoffs'));$('reviewDatesPdfBtn').addEventListener('click',()=>downloadReport('reviews'));$('backupBtn').addEventListener('click',downloadBackup);$('fullBackupBtn').addEventListener('click',downloadFullBackup);
...
function renderMySafety(){
  ...
  $('mySafetyList').innerHTML=rows.length?rows.map(({a,t,status})=>{
    const src=sourceForTraining(t),material=requiredTrainingMaterial(a,t),materialOpened=requiredTrainingMaterialOpened(a,t);
    const relatedDocs=linkedTrainingDocs(t.id).map(x=>x.doc).filter(d=>d.id!==t.source_document_id);
    const relatedBtns=relatedDocs.slice(0,3).map(d=>{const v=currentVersion(d.id);return v?btn(`Open ${d.reference||docTypeLabel(d.doc_type)}`,'ghost',`data-open-doc="${v.id}"`):''}).join('');
    const requiredBtn=material&&status.code!=='COMPLETED'?btn(materialOpened?'Open required file ✓':'Open required file','secondary',`data-open-required-training="${a.id}"`):'';
    const materialBadge=material&&status.code!=='COMPLETED'?`<span class="badge ${materialOpened?'complete':'due'}">${materialOpened?'Required file opened ✓':'Required file not opened'}</span>`:'';
    const traffic=status.code==='COMPLETED'?'green':status.code==='OVERDUE'?'red':'amber';
    return `<div class="item-card training-status-card traffic-${traffic}"><div class="row-between"><div><h3>${esc(trainingReference(t)&&!t.name.toUpperCase().includes(trainingReference(t))?trainingReference(t)+' - '+t.name:t.name)}</h3><div class="meta"><span class="badge">${esc(kindLabel(trainingKind(t)))}</span><span>${esc(deliveryText(status.method))}</span><span class="badge ${status.badge}">${esc(status.label)}</span>${status.due?`<span>Due ${fmtDate(status.due)}</span>`:''}${materialBadge}</div></div></div>${src?`<div class="muted">Controlled source: ${esc(src.reference||'')} ${esc(src.title)}</div>`:''}${material&&status.code!=='COMPLETED'&&!materialOpened?`<div class="request-note">Open the current required file before this training can be signed off.</div>`:''}<div class="row">${requiredBtn}${relatedBtns}${status.code!=='COMPLETED'&&status.method==='SELF_TRAINING'?btn('Complete & sign','primary',`data-sign-training="${a.id}"`):''}${status.code==='READY_TO_SIGN'?btn('Sign attendance','primary',`data-sign-training="${a.id}"`):''}${isAdmin()&&a.user_id===state.user?.id&&status.method==='INSTRUCTOR_LED'&&status.code!=='COMPLETED'&&!status.ready?btn('Complete as exception','danger',`data-training-exception="${a.id}"`):''}${status.method==='SELF_TRAINING'&&status.code!=='COMPLETED'?btn('Need further training','secondary',`data-request-instructor="${a.id}"`):''}${btn('View training','secondary',`data-view-training="${t.id}"`)}</div></div>`
...
function renderDocumentRegister(q='',type=''){
  ...
  $('documentsList').innerHTML=`<div class="register-toolbar"><div><strong>Current Document Register</strong><div class="muted">Generated live from the current Safety Tracker records.${type?' Filtered to '+esc(type==='TOOLBOX_TALK'?'Toolbox Talk':docTypeLabel(type))+'.':''}</div></div>${btn('Download register PDF','secondary','data-download-register')}</div>`+body;
}
...
function requiredTrainingMaterial(a,t){
  if(!a||!t)return null;
  if(t.source_document_id){
    const d=state.documents.find(x=>x.id===t.source_document_id);
    const v=state.versions.find(x=>x.id===t.source_document_version_id)||currentVersion(t.source_document_id);
    if(d&&v)return {kind:'DOCUMENT',document:d,version:v,label:`${d.reference||docTypeLabel(d.doc_type)} - ${documentDisplayTitle(d)}`};
  }
  const f=latestTrainingFile(t.id);
  if(f)return {kind:'TRAINING_FILE',file:f,label:f.file_name||t.name};
  return null;
}
function requiredTrainingMaterialOpened(a,t){
  const m=requiredTrainingMaterial(a,t);if(!m)return true;
  const cutoff=trainingOpenCutoff(a),uid=state.user?.id;
  return state.documentActivity.some(x=>x.user_id===uid&&x.action==='OPENED'&&new Date(x.occurred_at||0).getTime()>cutoff&&((m.kind==='DOCUMENT'&&x.document_version_id===m.version.id)||(m.kind==='TRAINING_FILE'&&x.training_file_id===m.file.id)));
}
function requiredTrainingMaterialLabel(a,t){const m=requiredTrainingMaterial(a,t);return m?.label||'required training file'}
async function openRequiredTrainingMaterial(assignmentId){
  const a=state.trainingAssignments.find(x=>x.id===assignmentId),t=state.training.find(x=>x.id===a?.training_session_id),m=requiredTrainingMaterial(a,t);if(!a||!t||!m)return toast('No required training file is attached to this assignment.');
  if(m.kind==='DOCUMENT'){
...
async function openDocument(versionId){const v=state.versions.find(x=>x.id===versionId);if(!v?.storage_path)return toast('Stored PDF not found.');const {data,error}=await sb.storage.from('safety-files').createSignedUrl(v.storage_path,300);if(error)return toast(error.message);window.open(data.signedUrl,'_blank','noopener');await logDocumentActivity('OPENED',{...activityVersionSnapshot(v),source_context:'DOCUMENT_LIBRARY'});}
function showDocDetails(id){const d=state.documents.find(x=>x.id===id),versions=state.versions.filter(v=>v.document_id===id).sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0));openModal('Document details',`<p><strong>${esc(d.reference?d.reference+' - '+documentDisplayTitle(d):documentDisplayTitle(d))}</strong></p><div class="meta"><span class="badge">${esc(docTypeLabel(d.doc_type))}</span><span>Status ${esc(d.status)}</span><span>Training ${esc(renewalText(d.default_renewal_value,d.default_renewal_unit,sourceDocTypes.has(d.doc_type)))}</span></div><h4>Version history</h4><div class="card-list">${versions.map(v=>`<div class="item-card compact"><div class="row-between"><span><strong>v${esc(v.version_label)}</strong> · ${esc(v.status)}</span><span>${fmtDate(v.issue_date)}</span></div><div class="meta"><span>Review ${fmtDate(v.review_date)}</span><span>${esc(v.file_name||'')}</span></div><div class="row">${v.storage_path?btn('Open','ghost',`data-open-doc="${v.id}"`)+btn('Download','ghost',`data-download-doc="${v.id}"`)+btn('Mark reviewed','ghost',`data-mark-doc-reviewed="${v.id}"`):''}</div></div>`).join('')}</div>`)}
function showToggleDocument(id){
  ...
  openModal(restoring?'Restore document':'Archive document',`<p><strong>${esc(d.reference?d.reference+' - '+documentDisplayTitle(d):documentDisplayTitle(d))}</strong></p><div class="${restoring?'hint-box':'danger-note'}">${restoring?'Restoring will make the document active again and rebuild its declared links from the current files.':'Archiving removes this item from active document/training links immediately. Controlled versions, access history and completed training evidence are retained.'}</div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn(restoring?'Restore':'Archive',restoring?'primary':'danger',`data-confirm-toggle-doc="${id}"`)}</div>`);
}
...
async function confirmTrainingException(id){
  if(!isAdmin())return toast('Admin access is required for a training exception.');
  const a=state.trainingAssignments.find(x=>x.id===id),t=state.training.find(x=>x.id===a?.training_session_id);
  if(!a||!t||a.user_id!==state.user?.id)return toast('This exception can only be used for your own assignment.');
  const status=assignmentStatus(a,t);
  if(status.code==='COMPLETED')return toast('This training assignment is already complete.');
...
async function saveDocumentReview(id){if(!$('reviewAck').checked)return toast('Tick the confirmation first.');const d=state.documents.find(x=>x.id===id),v=currentVersion(id),outcome=$('reviewOutcome').value,note=clean($('reviewNote').value),next=$('nextReviewDate').value||null,sig=signatureData('reviewSignaturePad'),sigName=clean($('reviewSignatureName').value);if(!sig)return toast('Please sign in the box.');if(!sigName)return toast('Enter the signature name.');if(outcome==='NO_CHANGE'&&!next)return toast('Choose the next review date.');const r=await sb.rpc('record_document_review',{p_document_version_id:v.id,p_outcome:outcome,p_review_note:note||null,p_next_review_date:next,p_signature_data:sig,p_signature_name:sigName});if(r.error)return toast(r.error.message);if(outcome==='NO_CHANGE')await sb.from('documents').update({review_required:false,review_reason:null}).eq('id',d.id);await logDocumentActivity('CONTROLLED_REVIEW',{...activityVersionSnapshot(v),source_context:'CONTROLLED_REVIEW'},{outcome,next_review_date:next},false);closeModal();await refresh('Signed document review recorded.')}
...
async function syncSourceTrainings(){
  ...
  for(const d of state.documents.filter(d=>d.status!=='ARCHIVED'&&sourceDocTypes.has(d.doc_type))){
    ...
    for(const l of linksForDocument(d.id)){const o=otherDocForLink(l,d.id);if(o)changes+=await ensureTrainingDocLink(active.id,o.id,'RELATED')}
  }
  return changes;
}
...
function showDocumentEvidencePack(docId){const d=state.documents.find(x=>x.id===docId);if(!d||!isManager())return;openModal('Person evidence pack',`<p><strong>${esc(d.reference?d.reference+' - '+documentDisplayTitle(d):documentDisplayTitle(d))}</strong></p><div class="hint-box">Creates one incident-ready PDF for one person: the selected document, their training/sign-off/access history, and the actual controlled PDFs as supporting evidence.</div><div class="form-grid"><label class="full">Person<select id="evidencePerson"><option value="">Select person…</option>${evidencePersonOptions()}</select></label><label>History<select id="evidenceScope"><option value="CURRENT">Current version only</option><option value="FULL">Full history</option></select></label><label class="check-row"><input id="evidenceLinks" type="checkbox" checked> Include directly linked documents / Toolbox Talks</label></div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Create evidence pack','primary',`data-generate-evidence-doc="${docId}"`)}</div>`)}
function showTrainingEvidencePack(trainingId){const t=state.training.find(x=>x.id===trainingId);if(!t||!isManager())return;openModal('Person evidence pack',`<p><strong>${esc(trainingReference(t)?trainingReference(t)+' - '+t.name:t.name)}</strong></p><div class="hint-box">Creates one person-specific evidence PDF containing this training item, the person's completion/access history and the actual training PDF. Directly linked controlled documents can be appended too.</div><div class="form-grid"><label class="full">Person<select id="evidencePerson"><option value="">Select person…</option>${evidencePersonOptions()}</select></label><label>History<select id="evidenceScope"><option value="CURRENT">Current only</option><option value="FULL">Full history</option></select></label><label class="check-row"><input id="evidenceLinks" type="checkbox" checked> Include directly linked documents</label></div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Create evidence pack','primary',`data-generate-evidence-training="${trainingId}"`)}</div>`)}
function evidenceSessionsForDocuments(docs,scope){const ids=new Set(docs.map(d=>d.id));return state.training.filter(t=>ids.has(t.source_document_id)&&t.auto_managed===true&&(scope==='FULL'||t.source_document_version_id===currentVersion(t.source_document_id)?.id))}
...
function makeEvidenceSummaryPdf(ctx,failed){const {jsPDF}=window.jspdf;const out=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});out.setFontSize(17);out.text('Safety Tracker - Person Evidence Pack',14,16);out.setFontSize(9);out.text(`Generated ${new Date().toLocaleString('en-GB')} · Safety Tracker v${APP_VERSION}`,14,23);out.setFontSize(10);out.text(`Person: ${personName(ctx.personId)}`,14,31);out.text(`Primary item: ${ctx.primaryLabel}`,14,37,{maxWidth:180});out.text(`History: ${ctx.scope==='FULL'?'Full history':'Current version only'} · Linked items: ${ctx.includeLinked?'Included':'Not included'}`,14,43);out.setFontSize(8);out.text('This pack records the evidence held in Safety Tracker. An OPENED event proves access to a file, not comprehension or competence on its own.',14,50,{maxWidth:180});
  const docRows=[];for(const x of ctx.documents){for(const v of documentVersionsForEvidence(x.doc,ctx.scope))docRows.push([x.role,x.doc.reference||docTypeLabel(x.doc.doc_type),documentDisplayTitle(x.doc),v.version_label||'',v.status||'',fmtDate(v.issue_date),fmtDate(v.review_date)])}for(const x of ctx.toolboxTalks){const f=latestTrainingFile(x.training.id);docRows.push([x.role,trainingReference(x.training)||'TBT',x.training.name,'—',x.training.status||'',f?fmtDate(f.created_at):'—',fmtDate(x.training.review_date)])}out.autoTable({head:[['Relationship','Reference','Item','Version','Status','Issue/file date','Review']],body:docRows.length?docRows:[['Primary','','No file details available','','','','']],startY:57,styles:{fontSize:6.6,cellPadding:1.4},headStyles:{fontSize:6.6},margin:{left:14,right:14}});
  ...
  const signed=[...ctx.evidence.signoffs.map(x=>({...x,_date:x.signed_at,_kind:'Training sign-off'})),...ctx.evidence.exceptions.map(x=>({...x,_date:x.completed_at,_kind:'Admin exception'}))].filter(x=>x.signature_data);for(const ev of signed){const t=state.training.find(x=>x.id===ev.training_session_id);out.addPage();out.setFontSize(14);out.text('Digital signature evidence',14,18);out.setFontSize(10);out.text(`${ev._kind}: ${trainingReference(t)||''} ${t?.name||ev.training_name_snapshot||''}`,14,28,{maxWidth:180});out.text(`Person: ${personName(ctx.personId)} · Signed: ${fmtDateTime(ev._date)} · Name: ${ev.signature_name||''}`,14,38,{maxWidth:180});try{out.addImage(ev.signature_data,'PNG',14,48,100,30)}catch(e){out.text('Signature image could not be rendered in this report.',14,55)}}return out.output('arraybuffer')}
async function buildEvidenceContext(primaryType,primaryId,personId,scope,includeLinked){let documents=[],toolboxTalks=[],primaryLabel='';if(primaryType==='DOCUMENT'){const d=state.documents.find(x=>x.id===primaryId);if(!d)throw new Error('Document not found.');documents.push({doc:d,role:'Primary'});primaryLabel=d.reference?`${d.reference} - ${documentDisplayTitle(d)}`:documentDisplayTitle(d);if(includeLinked){directLinkedDocumentsForEvidence(d.id).forEach(x=>documents.push({doc:x,role:'Linked'}));directToolboxTalksForDocument(d.id).forEach(x=>toolboxTalks.push({training:x,role:'Linked Toolbox Talk'}))}}else{const t=state.training.find(x=>x.id===primaryId);if(!t)throw new Error('Training item not found.');toolboxTalks.push({training:t,role:'Primary'});primaryLabel=trainingReference(t)?`${trainingReference(t)} - ${t.name}`:t.name;if(includeLinked)linkedTrainingDocs(t.id).map(x=>x.doc).filter(d=>d.status!=='ARCHIVED').forEach(x=>documents.push({doc:x,role:'Linked'}))}documents=documents.filter((x,i,a)=>a.findIndex(y=>y.doc.id===x.doc.id)===i);toolboxTalks=toolboxTalks.filter((x,i,a)=>a.findIndex(y=>y.training.id===x.training.id)===i);
  let sessions=[...evidenceSessionsForDocuments(documents.map(x=>x.doc),scope),...toolboxTalks.map(x=>x.training)];sessions=uniqueById(sessions);const evidence=evidenceRowsForPerson(sessions,personId);const docIds=new Set(documents.map(x=>x.doc.id)),sessionIds=new Set(sessions.map(x=>x.id)),fileIds=new Set(toolboxTalks.flatMap(x=>{const rows=state.trainingFiles.filter(f=>f.training_session_id===x.training.id);const use=scope==='FULL'?rows:(latestTrainingFile(x.training.id)?[latestTrainingFile(x.training.id)]:[]);return use.map(f=>f.id)}));const versionIds=new Set(documents.flatMap(x=>documentVersionsForEvidence(x.doc,scope).map(v=>v.id)));const activities=state.documentActivity.filter(a=>a.user_id===personId&&(docIds.has(a.document_id)||versionIds.has(a.document_version_id)||sessionIds.has(a.training_session_id)||fileIds.has(a.training_file_id))).sort((a,b)=>new Date(a.occurred_at||0)-new Date(b.occurred_at||0));return {primaryType,primaryId,personId,scope,includeLinked,primaryLabel,documents,toolboxTalks,sessions,evidence,activities}}
async function evidenceAttachments(ctx){const rows=[];for(const x of ctx.documents){for(const v of documentVersionsForEvidence(x.doc,ctx.scope))if(v.storage_path)rows.push({label:`${x.role}: ${x.doc.reference||docTypeLabel(x.doc.doc_type)} - ${documentDisplayTitle(x.doc)} · v${v.version_label||''}`,storage_path:v.storage_path,file_name:v.file_name||`${x.doc.reference||'document'}.pdf`})}for(const x of ctx.toolboxTalks){let files=state.trainingFiles.filter(f=>f.training_session_id===x.training.id).sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0));if(ctx.scope!=='FULL')files=files.slice(0,1);for(const f of files)if(f.storage_path)rows.push({label:`${x.role}: ${trainingReference(x.training)||'Toolbox Talk'} - ${x.training.name}`,storage_path:f.storage_path,file_name:f.file_name||'toolbox-talk.pdf'})}const seen=new Set();return rows.filter(x=>!seen.has(x.storage_path)&&seen.add(x.storage_path))}
async function generateEvidencePack(primaryType,primaryId){if(!isManager())return;const personId=$('evidencePerson')?.value,scope=$('evidenceScope')?.value||'CURRENT',includeLinked=$('evidenceLinks')?.checked!==false;if(!personId)return toast('Select the person for this evidence pack.');if(!window.PDFLib?.PDFDocument||!window.jspdf?.jsPDF)return toast('PDF libraries did not load. Refresh the page and try again.');const actionButtons=[...document.querySelectorAll('[data-generate-evidence-doc],[data-generate-evidence-training]')];actionButtons.forEach(b=>b.disabled=true);toast('Building evidence pack…');try{const ctx=await buildEvidenceContext(primaryType,primaryId,personId,scope,includeLinked),attachments=await evidenceAttachments(ctx),loaded=[],failed=[];for(const a of attachments){try{const r=await sb.storage.from('safety-files').download(a.storage_path);if(r.error||!r.data)throw new Error(r.error?.message||'Download failed');loaded.push({...a,bytes:await r.data.arrayBuffer()})}catch(e){failed.push({label:a.label,reason:e.message||'Could not load PDF'})}}const summaryBytes=makeEvidenceSummaryPdf(ctx,failed),{PDFDocument,StandardFonts,rgb}=window.PDFLib,merged=await PDFDocument.create(),summary=await PDFDocument.load(summaryBytes);const sumPages=await merged.copyPages(summary,summary.getPageIndices());sumPages.forEach(p=>merged.addPage(p));const font=await merged.embedFont(StandardFonts.Helvetica),bold=await merged.embedFont(StandardFonts.HelveticaBold);for(const a of loaded){const divider=merged.addPage([595.28,841.89]);divider.drawText('Supporting controlled document',{x:48,y:770,size:18,font:bold,color:rgb(0.08,0.16,0.24)});const lines=[];let rest=a.label;while(rest.length>80){let cut=rest.lastIndexOf(' ',80);if(cut<30)cut=80;lines.push(rest.slice(0,cut));rest=rest.slice(cut).trim()}lines.push(rest);lines.forEach((line,i)=>divider.drawText(line,{x:48,y:730-i*18,size:11,font}));divider.drawText(`File: ${a.file_name}`,{x:48,y:650,size:9,font});const src=await PDFDocument.load(a.bytes,{ignoreEncryption:true}),pages=await merged.copyPages(src,src.getPageIndices());pages.forEach(p=>merged.addPage(p))}const bytes=await merged.save(),blob=new Blob([bytes],{type:'application/pdf'}),base=safeFileName(`${personName(personId)}-${ctx.primaryLabel}`)||'evidence-pack';downloadBlob(blob,`${base}-${scope==='FULL'?'full-history':'current'}-${todayISO()}.pdf`);closeModal();toast(`Evidence pack created: ${loaded.length} PDF${loaded.length===1?'':'s'} appended${failed.length?`, ${failed.length} could not be appended`:''}.`)}catch(e){console.error(e);toast(e.message||'Could not create evidence pack.')}finally{actionButtons.forEach(b=>b.disabled=false)}}

function renderReports(){const rows=complianceRows();$('reportStats').innerHTML=[['Assignments',rows.length],['Complete',rows.filter(r=>r.code==='COMPLETED').length],['Outstanding',rows.filter(r=>r.code!=='COMPLETED').length],['Completion evidence',state.trainingSignoffs.length+state.trainingExceptions.length]].map(([l,n])=>`<div class="stat"><strong>${n}</strong><span>${l}</span></div>`).join('');renderDocumentActivityReport()}
...
function renderHelp(){$('helpContent').innerHTML=`<div class="help-card"><h3>Documents</h3><p>Documents opens with a live Register first, followed by dedicated indexes for Risk Assessments, COSHH Risk Assessments, Safe Systems of Work, MSDS/Safety Data Sheets and Toolbox Talks. The Register lists every current item under its correct section and can be downloaded as a PDF. Toolbox Talk files are indexed here for access, while assignment and sign-off remain in Training.</p></div><div class="help-card"><h3>Training</h3><p>All completion/sign-off happens in Training. RA and COSHH default to self-training with a 12-month refresher. Admin can change the refresher to 6 months, 12 months, 24 months or On change only. SSW and Toolbox Talks default to instructor-led. A new controlled version creates a fresh training requirement immediately. Where a current controlled source or training PDF exists, that exact required file must be opened and audit-recorded before sign-off. Related/supporting links do not block completion.</p></div><div class="help-card"><h3>New document versions</h3><p>If the same controlled reference is uploaded again, v2 compares the actual PDF text. Unchanged content is skipped; changed content becomes the next version and the old version is superseded.</p></div><div class="help-card"><h3>Combined PDF packs</h3><p>Bulk Import reads the PDF page by page and splits combined RA, COSHH, SSW and Toolbox Talk packs into individual controlled records using the reference printed on each document. It also splits combined manufacturer SDS/MSDS packs by detecting each genuine SDS first page (Section 1 / 1.1 Product identifier), so an SDS pack imports as separate product records. Shield COSHH forms are grouped by the footer reference such as COSHH-014 | Version 1.2, so a multi-assessment pack does not import as one record. Review the detected page ranges before importing.</p></div><div class="help-card"><h3>Force Sync & Review</h3><p>Force Sync first clears the existing link tables, then re-reads every current stored PDF and rebuilds relationships only from references/titles stated in linked, related, supporting or reference sections. Before rebuilding links, v2 repairs Risk Assessment titles from the first page and aligns SDS names from the manufacturer Section 1.1 Product identifier/name. COSHH then matches its stated substance/SDS wording against those aligned product names and aliases. SDS cleanup removes trailing regulation/subsection text such as “According to …” or “: 1.1”. Missing references are flagged instead of guessed.</p></div><div class="help-card"><h3>Document access & review audit</h3><p>Opening, downloading or explicitly marking a controlled file as reviewed creates a timestamped audit event against the signed-in user and exact document/version. Training completion is also reflected in the audit. Marking a file reviewed does not count as training completion.</p></div><div class="help-card"><h3>Archiving documents</h3><p>Archiving a controlled document immediately removes its active document-to-document and Training-document links while retaining versions, access history and completion evidence. Restoring a document re-reads the current files and rebuilds only relationships declared by the documents.</p></div><div class="help-card"><h3>Self-training support</h3><p>A self-training user can ask ordinary questions without changing the assignment. If they select Need further training and confirm that they require extra instruction/demonstration, only their individual assignment changes to instructor-led. The master RA/COSHH method and other users remain self-training.</p></div><div class="help-card"><h3>Administrator training exception</h3><p>An administrator can complete only their own outstanding instructor-led assignment as an exception when independent instructor confirmation is not available. The required current file must have been opened first. A reason of at least 10 characters, confirmation and digital signature are mandatory and retained as separate compliance evidence. Normal users cannot use this route.</p></div><div class="help-card"><h3>Person evidence packs</h3><p>Managers/Admins can create an incident-ready evidence pack from a controlled document or Toolbox Talk. Choose a person, current or full history, and whether to include directly linked documents. The PDF includes that person's assignments, sign-offs/signatures, instructor confirmations, admin exceptions and document access/review history, then appends the actual controlled PDFs as supporting evidence.</p></div><div class="help-card"><h3>Version check</h3><p>The header must show <strong>v2.1.9 CLEAN</strong>. Admin → Build diagnostics also checks version.json and service-worker registrations.</p></div>`}

async function globalClick(e){const el=e.target.closest('button');if(!el)return;if(el.dataset.docIndex){state.documentIndex=el.dataset.docIndex;if($('documentTypeFilter'))$('documentTypeFilter').value=state.documentIndex==='REGISTER'?'':state.documentIndex;renderDocuments();return}if(el.dataset.downloadRegister!==undefined)return documentRegisterPdf();if(el.dataset.closeModal!==undefined)return closeModal();if(el.dataset.createDocument!==undefined)return createDocumentRecord();if(el.dataset.openRequiredTraining)return openRequiredTrainingMaterial(el.dataset.openRequiredTraining);if(el.dataset.openDoc)return openDocument(el.dataset.openDoc);if(el.dataset.downloadDoc)return downloadDocument(el.dataset.downloadDoc);if(el.dataset.markDocReviewed)return showMarkDocumentReviewed(el.dataset.markDocReviewed);if(el.dataset.confirmDocReviewed)return confirmDocumentReviewed(el.dataset.confirmDocReviewed);if(el.dataset.docActivity)return showDocumentActivity(el.dataset.docActivity);if(el.dataset.evidenceDoc)return showDocumentEvidencePack(el.dataset.evidenceDoc);if(el.dataset.generateEvidenceDoc)return generateEvidencePack('DOCUMENT',el.dataset.generateEvidenceDoc);if(el.dataset.evidenceTraining)return showTrainingEvidencePack(el.dataset.evidenceTraining);if(el.dataset.generateEvidenceTraining)return generateEvidencePack('TRAINING',el.dataset.generateEvidenceTraining);if(el.dataset.docDetails)return showDocDetails(el.dataset.docDetails);if(el.dataset.docLinks)return showDocumentLinks(el.dataset.docLinks);if(el.dataset.addDocLink)return addDocumentLink(el.dataset.addDocLink);if(el.dataset.removeDocLink)return removeDocumentLink(el.dataset.removeDocLink);if(el.dataset.newVersion)return showNewVersion(el.dataset.newVersion);if(el.dataset.publishVersion)return publishVersionFromModal(el.dataset.publishVersion);if(el.dataset.toggleDoc)return showToggleDocument(el.dataset.toggleDoc);if(el.dataset.confirmToggleDoc)return toggleDocument(el.dataset.confirmToggleDoc);if(el.dataset.reviewDoc)return showDocumentReview(el.dataset.reviewDoc);if(el.dataset.saveDocReview)return saveDocumentReview(el.dataset.saveDocReview);if(el.dataset.saveTraining!==undefined)return saveTraining();if(el.dataset.viewTraining)return showTrainingDetails(el.dataset.viewTraining);if(el.dataset.openTrainingFile)return openTrainingFile(el.dataset.openTrainingFile);if(el.dataset.downloadTrainingFile)return downloadTrainingFile(el.dataset.downloadTrainingFile);if(el.dataset.markTrainingFileReviewed)return showMarkTrainingFileReviewed(el.dataset.markTrainingFileReviewed);if(el.dataset.confirmTrainingFileReviewed)return confirmTrainingFileReviewed(el.dataset.confirmTrainingFileReviewed);if(el.dataset.trainingFileActivity)return showTrainingFileActivity(el.dataset.trainingFileActivity);if(el.dataset.assignTraining)return showAssignTraining(el.dataset.assignTraining);if(el.dataset.saveTrainingAssignments)return saveTrainingAssignments(el.dataset.saveTrainingAssignments);if(el.dataset.editTraining)return showEditTraining(el.dataset.editTraining);if(el.dataset.saveTrainingEdit)return saveTrainingEdit(el.dataset.saveTrainingEdit);if(el.dataset.archiveTraining)return showArchiveTraining(el.dataset.archiveTraining);if(el.dataset.confirmArchiveTraining)return archiveTraining(el.dataset.confirmArchiveTraining);if(el.dataset.signTraining)return signTraining(el.dataset.signTraining);if(el.dataset.confirmTrainingSign)return confirmTrainingSign(el.dataset.confirmTrainingSign);if(el.dataset.trainingException)return showTrainingException(el.dataset.trainingException);if(el.dataset.confirmTrainingException)return confirmTrainingException(el.dataset.confirmTrainingException);if(el.dataset.requestInstructor)return requestInstructor(el.dataset.requestInstructor);if(el.dataset.confirmInstructorRequest)return confirmInstructorRequest(el.dataset.confirmInstructorRequest);if(el.dataset.saveGroupAttendance)return saveGroupAttendance(el.dataset.saveGroupAttendance);if(el.dataset.singleAttendance)return showSingleAttendance(el.dataset.singleAttendance);if(el.dataset.saveSingleAttendance)return saveSingleAttendance(el.dataset.saveSingleAttendance);if(el.dataset.sendInvite!==undefined)return sendInvite();if(el.dataset.setRole)return showSetRole(el.dataset.setRole);if(el.dataset.saveRole)return saveRole(el.dataset.saveRole);if(el.dataset.toggleUser)return toggleUser(el.dataset.toggleUser);if(el.id==='saveNewPassword'){const r=await sb.auth.updateUser({password:$('newPassword').value});if(r.error)return toast(r.error.message);closeModal();toast('Password updated.')}}

window.SafetyTrackerV2={APP_VERSION,BUILD_ID,state,sb,loadAll,refresh,runSafetySync,hashPdf,sha256Text,pdfTextFromBlob,currentVersion,ensureTrainingDocLink,createSourceTraining,syncSourceTrainings,safeFileName,productWords,similarity,refsInText,canonicalRef,declaredDocumentMatches,nextVersionLabel,publishFileAsNewVersion,existingDocMatch,extractSdsProductName,extractCoshhProductCandidates,productNameMatchScore,documentDisplayTitle,repairSdsTitles,extractRiskAssessmentTitle,repairRaTitles,renderDocumentRegister,documentRegisterPdf,logDocumentActivity,requiredTrainingMaterial,requiredTrainingMaterialOpened,openRequiredTrainingMaterial,showTrainingException,showDocumentEvidencePack,showTrainingEvidencePack,generateEvidencePack,classifySafetyPdfText,looksLikeCoshhAssessment,looksLikeSafetyDataSheet,toast};
init();
