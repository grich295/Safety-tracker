/* Safety Tracker v2.1.5 - RA title repair + required-file-open enforcement + admin exception fix. */
'use strict';

const APP_VERSION='2.1.5';
const BUILD_ID='open-before-sign-v215-20260910';
...
function cleanRaTitleCandidate(value,ref=''){
  let t=clean(value);
  if(!t)return '';
  t=t.replace(/^RISK\s+ASSESSMENT\s*/i,'').replace(new RegExp(`\\b${String(ref||'').replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&')}\\b.*$`,'i'),'').trim();
  t=t.replace(/\s+(?:Marriott\s+Portsmouth|Adopted\s+on|Department\s*\/\s*job\s+title|RA\s+Reference|Location|Persons\s+at\s+risk|Task|Linked\s+controls)\b.*$/i,'').trim();
  ...
  // Long prose is almost certainly a control or description, not a document title.
...
function extractRiskAssessmentTitle(text,ref=''){
  ...
  // Shield Safety maintenance RAs put the actual title immediately before
  // "Marriott Portsmouth" on page 1. Join up to three wrapped title lines.
  ...
  if(hotelIndex>0){
    ...
    for(let i=hotelIndex-1;i>=0&&parts.length<3;i--){
      ...
      if(joined.length>=35)break;
    }
    const c=cleanRaTitleCandidate(parts.join(' '),ref);if(c)return c;
  }
  // Older Shield forms explicitly print RISK ASSESSMENT then the title.
  const idx=lines.findIndex(x=>/^RISK\s+ASSESSMENT$/i.test(x));
  if(idx>=0){
    const parts=[];
  ...
  // pdf.js can flatten the first-page title and hotel onto one line.
  const m=clean(raw).match(/(?:^|Page\s+1\s+)(.{4,180}?)\s+Marriott\s+Portsmouth\b/i);
...
async function repairRaTitles(options={}){
  ...
  for(let i=0;i<docs.length;i++){
    ...
    try{
      ...
      const first=await pdfFirstPageTextFromBlob(r.data),title=extractRiskAssessmentTitle(first,d.reference||'');
  ...
  if(!silent&&status){status.hidden=false;status.textContent=`Risk Assessment title repair complete: ${changed} updated${failed?`, ${failed} could not be read`:''}.`}
  if(!silent)toast(`${changed} Risk Assessment title${changed===1?'':'s'} updated.`);
