/* Safety Tracker v2.6.1 - button action recovery on top of v2.6.0 traffic-light UI. */
'use strict';

const APP_VERSION='2.6.2';
const BUILD_ID='v262-cache-button-recovery-20260912';
...
async function openDocument(versionId){const v=state.versions.find(x=>x.id===versionId);if(!v?.storage_path)return toast('Stored PDF not found.');if(!isManager()&&(!isVersionApproved(v)||v.status!=='CURRENT'))return toast('Only the approved current version is available for use.');let popup=null;try{popup=window.open('about:blank','_blank')}catch{}const {data,error}=await sb.storage.from('safety-files').createSignedUrl(v.storage_path,300);if(error){try{popup?.close()}catch{}return toast(error.message)}const url=data?.signedUrl;if(!url){try{popup?.close()}catch{}return toast('Could not create a secure link for this PDF.')}try{if(popup&&!popup.closed){popup.opener=null;popup.location.href=url}else{window.location.href=url}}catch{window.location.href=url}await logDocumentActivity('OPENED',{...activityVersionSnapshot(v),source_context:versionApprovalStatus(v)==='PENDING'?'VERSION_APPROVAL':'DOCUMENT_LIBRARY'});if($('approvalOpenStatus')&&$('approvalOpenStatus').dataset.versionId===v.id){$('approvalOpenStatus').textContent='Opened ✓';$('approvalOpenStatus').className='badge complete';}}
