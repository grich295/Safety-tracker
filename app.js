/* Safety Tracker v2.3.1 - QA fixes for self-department assignment and approved-document audiences. */
'use strict';

const APP_VERSION='2.3.1';
const BUILD_ID='qa-self-department-approved-audience-v231-20260911';
...
function renderPeople(){
  $('peopleList').innerHTML=state.people.length?state.people.map(p=>`<div class="item-card"><div class="row-between"><div><strong>${esc(p.display_name||p.email)}</strong><div class="meta"><span>${esc(p.email||'')}</span><span class="badge">${esc(profileAccessLabel(p))}</span><span class="badge">${esc(userDepartmentName(p.id))}</span><span>${p.active===false?'Disabled':'Active'}</span>${p.report_only===true?'<span>Reports/download only</span>':''}</div></div>${isAdmin()?`<div class="row">${btn('Role / department','secondary',`data-set-role="${p.id}"`)}${btn(p.active===false?'Enable':'Disable',p.active===false?'primary':'danger',`data-toggle-user="${p.id}"`)}</div>`:''}</div></div>`).join(''):'<div class="empty">No users found.</div>'
}
...
function showSetRole(id){
  ...
  openModal('Set role / department',`<div class="form-grid"><label>Role / access<select id="roleSelect"><option value="user" ${selected==='user'?'selected':''}>User</option><option value="manager" ${selected==='manager'?'selected':''}>Manager</option><option value="admin" ${selected==='admin'?'selected':''}>Admin</option><option value="report_viewer" ${selected==='report_viewer'?'selected':''}>Report Viewer — reports/download only</option></select></label><label>Department<select id="roleDepartment"><option value="">No department</option>${deptOpts}</select></label></div><div class="hint-box">${own?'<strong>Your own account:</strong> you can change your Department here, but you cannot demote, disable or change your own access level. ':''}Changing department immediately recalculates department-based training requirements. Previous sign-offs/evidence are retained. Changing someone to Report Viewer deactivates current Training, Safety Awareness and PPE assignments but keeps previous evidence/sign-offs.</div><div class="actions">${btn('Save','primary',`data-save-role="${id}"`)}</div>`)
}
async function saveRole(id){
  ...
  try{
    if(own&&requested!==currentAccess)throw new Error('You can change your own Department here, but you cannot change your own role/access level.');
    if(requested==='report_viewer'){
    ...
    closeModal();await refresh(own?'Your department has been updated and automatic training recalculated.':requested==='report_viewer'?'Report Viewer access applied.':'Role / department updated and automatic training recalculated.')
