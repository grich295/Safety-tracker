...
function renderToolboxTalkDocumentIndex(q,status){
  ...
  $('documentsList').innerHTML=rows.length?rows.map(t=>{
    const f=latestTrainingFile(t.id),assigns=state.trainingAssignments.filter(a=>a.training_session_id===t.id&&a.active!==false),traffic=trainingCatalogueTraffic(t),active=t.status!=='ARCHIVED';
    return `<div class="item-card training-catalogue-card traffic-${traffic}"><div class="row-between"><div><h3>${esc(trainingReference(t)&&!t.name.toUpperCase().includes(trainingReference(t))?trainingReference(t)+' - '+t.name:t.name)}</h3><div class="meta"><span class="badge">Toolbox Talk</span><span class="badge ${active?'complete':''}">${active?'Active':'Archived'}</span><span>Review ${fmtDate(t.review_date)}</span><span>${esc(deliveryText(t.delivery_method||'INSTRUCTOR_LED'))}</span></div></div><div class="meta"><span class="badge">${assigns.length} assigned</span></div></div><div class="muted">${active?'Active Toolbox Talk — available for assignment, attendance and sign-off in Training.':'Archived Toolbox Talk — retained for history and not available for new assignments.'}</div><div class="row">${f?btn('Open file','secondary',`data-open-training-file="${f.id}"`)+btn('Download','secondary',`data-download-training-file="${f.id}"`):''}${btn('Activity','ghost',`data-training-file-activity="${t.id}"`)}${btn('View training','secondary',`data-view-training="${t.id}"`)}${isManager()&&active?btn('Assign','primary',`data-assign-training="${t.id}"`):''}${isManager()?btn(active?'Archive':'Restore / activate','ghost',`data-archive-training="${t.id}"`):''}</div></div>`
