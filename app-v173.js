...
const state = { user:null, profile:null, people:[], documents:[], versions:[], docAssignments:[], docSignoffs:[], docConfirmations:[], documentLinks:[], documentReviews:[], documentApprovals:[], training:[], trainingAssignments:[], trainingSignoffs:[], trainingConfirmations:[], trainingFiles:[], trainingApprovals:[] };
...
const yearsFromNow = n => { const d=new Date(); d.setFullYear(d.getFullYear()+n); return localISODate(d); };
const daysFromNow = n => { const d=new Date(); d.setDate(d.getDate()+n); return localISODate(d); };
...
function approvalLabel(s){
  return ({PENDING:'Pending approval',APPROVED:'Approved',LEGACY:'Existing / pre-approval',NOT_REQUIRED:'Approval not required'})[s]||'Pending approval';
}
function versionIsApproved(v,d=null){
  if(!v)return false;
  const doc=d||state.documents.find(x=>x.id===v.document_id);
  if(doc?.doc_type==='SDS')return true;
  return ['APPROVED','LEGACY'].includes(v.approval_status);
}
function pendingVersions(docId){
  return state.versions.filter(v=>v.document_id===docId&&v.approval_status==='PENDING').sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));
}
function currentPublishedVersion(docId){
  return state.versions.find(v=>v.document_id===docId&&v.status==='CURRENT'&&['APPROVED','LEGACY','NOT_REQUIRED',null,undefined].includes(v.approval_status))
    ||state.versions.filter(v=>v.document_id===docId&&['APPROVED','LEGACY','NOT_REQUIRED'].includes(v.approval_status)).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0]
    ||null;
}
function trainingIsApproved(t){
  if(!t)return false;
  if(t.session_type!=='TOOLBOX_TALK')return true;
  return ['APPROVED','LEGACY','NOT_REQUIRED'].includes(t.approval_status);
}
function documentApprovalForVersion(versionId){
  return state.documentApprovals.find(a=>a.document_version_id===versionId)||null;
}
function trainingApprovalForSession(trainingId){
  return state.trainingApprovals.find(a=>a.training_session_id===trainingId)||null;
}
function renewalPresetValue(value,unit){
  if(!value||!unit)return '';
  const exact=['3|MONTHS','6|MONTHS','12|MONTHS','24|MONTHS'];
  const v=`${value}|${unit}`;
  return exact.includes(v)?v:'CUSTOM';
}
...
function autoLinkWords(s){
  const stop=new Set([
    'safety','data','sheet','sds','msds','coshh','assessment','risk','safe','system','work',
    'toolbox','talk','procedure','policy','maintenance','marriott','portsmouth','manufacturer',
    'product','current','section','using','use','the','and','for','with','from','into','general',
    'replacement','removal','application','document'
  ]);
  return String(s||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').split(' ')
    .filter(x=>x.length>2&&!stop.has(x));
}
function autoLinkSimilarity(a,b){
  const A=autoLinkWords(a),B=autoLinkWords(b);if(!A.length||!B.length)return 0;
  const as=new Set(A),bs=new Set(B);
  const common=[...as].filter(x=>bs.has(x)).length;
  return common/Math.max(1,Math.min(as.size,bs.size));
}
function linkExistsBetween(a,b){
...
function currentVersionForDisplay(docId){
  return currentPublishedVersion(docId)||pendingVersions(docId)[0]||
    state.versions.filter(v=>v.document_id===docId).sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0))[0];
}
function isGenericSdsTitle(s){
  const t=String(s||'').trim().toLowerCase();
  return !t||t==='manufacturer safety data sheet'||t==='safety data sheet'||t==='sds'||t==='msds';
}
function stripPdfName(s){
  return String(s||'').replace(/^[0-9a-f-]{20,}-/i,'').replace(/\.pdf$/i,'').trim();
}
function documentPickerLabel(d){
  const v=currentVersionForDisplay(d.id);
  const ref=d.reference?`${d.reference} · `:'';
  let title=d.title||'Untitled';
  const fileName=v?.file_name?stripPdfName(v.file_name):'';
  if(d.doc_type==='SDS'&&isGenericSdsTitle(title)&&fileName&&!/manufacturer safety data sheet/i.test(fileName))title=fileName;
  const fileExtra=fileName&&fileName.toLowerCase()!==String(title).toLowerCase()?` · File: ${fileName}`:'';
  return `${docTypeLabel(d.doc_type)} · ${ref}${title}${fileExtra}`;
}
async function pdfTextAndLinesFromFileLike(fileLike,maxPages=4){
  if(!window.pdfjsLib||!fileLike)return {text:'',lines:[]};
  try{
    ...
    for(let p=1;p<=Math.min(pdf.numPages,maxPages);p++){
      ...
      for(const it of content.items||[]){
        const t=String(it.str||'').trim();if(t)line+=(line?' ':'')+t;
        if(it.hasEOL){if(line.trim())lines.push(line.replace(/\s+/g,' ').trim());line='';}
      }
      if(line.trim())lines.push(line.replace(/\s+/g,' ').trim());
      allLines.push(...lines);
    }
    return {text:allLines.join(' '),lines:allLines};
  }catch(e){console.warn('PDF text read failed',e);return {text:'',lines:[]}}
}
function extractSdsTitleFromLines(lines){
  const cleanLine=s=>String(s||'').replace(/\s+/g,' ').trim();
  // 1. Explicit "Product name" field, same line or following line.
  for(let i=0;i<Math.min(lines.length,80);i++){
    const l=cleanLine(lines[i]);
    let m=l.match(/^Product name\s*:?\s*(.+?)\s*:?\s*$/i);
    if(m&&m[1]&&!/^product name$/i.test(m[1])){
      const v=cleanLine(m[1]).replace(/\s+:\s*$/,'');
      if(v.length>2)return v;
    }
    if(/^Product name\s*:?\s*$/i.test(l)){
      for(let j=i+1;j<Math.min(lines.length,i+5);j++){
        const v=cleanLine(lines[j]).replace(/^:\s*/,'').replace(/\s+:\s*$/,'');
        if(v&&!/^\d|^1\.\d|supplier|identifier/i.test(v)&&v.length>2)return v;
      }
    }
  }
  // 2. Product name immediately before the "SAFETY DATA SHEET" heading.
  const i=lines.findIndex(x=>/^\s*SAFETY DATA SHEET\s*$/i.test(cleanLine(x)));
  if(i>=0){
    for(let j=i-1;j>=Math.max(0,i-6);j--){
      const v=cleanLine(lines[j]);
      if(!v||/SHIELD SAFETY|SECTION\s+3\.4|MAINTENANCE|SUPPORTING NON-COSHH/i.test(v))continue;
      if(v.length>3)return v;
    }
  }
  // 3. Common SDS top-page pattern: heading then a strong product line.
  if(i>=0){
    for(let j=i+1;j<Math.min(lines.length,i+12);j++){
      const v=cleanLine(lines[j]);
      if(!v||/Product name|SECTION 1|Identification|Conforms to|Regulation|Version|Date of/i.test(v))continue;
      if(v.length>4)return v.replace(/\s+:\s*$/,'');
    }
  }
  return '';
}
async function repairGenericSdsTitles(){
  if(!isManager()||!window.pdfjsLib)return false;
  let changed=false;
  const targets=state.documents.filter(d=>d.doc_type==='SDS'&&d.status!=='ARCHIVED'&&isGenericSdsTitle(d.title));
  for(const d of targets){
    const v=currentVersionForDisplay(d.id);if(!v?.storage_path)continue;
    try{
      const {data,error}=await sb.storage.from('safety-files').download(v.storage_path);
      if(error||!data)continue;
      const parsed=await pdfTextAndLinesFromFileLike(data,3);
      const title=extractSdsTitleFromLines(parsed.lines);
      if(title&&!isGenericSdsTitle(title)){
        const {error:updateError}=await sb.from('documents').update({title}).eq('id',d.id);
        if(!updateError){d.title=title;changed=true;}
      }
    }catch(e){console.warn('SDS title repair failed',d.id,e)}
  }
  return changed;
}
async function insertDiscoveredLink(a,b){
  if(!a||!b||a.id===b.id||linkExistsBetween(a.id,b.id))return false;
  const rel=inferLinkType(a,b);
  const {error}=await sb.from('document_links').insert({
    source_document_id:rel.source.id,target_document_id:rel.target.id,
    link_type:rel.type,created_by:state.user.id
  });
  if(error&&!String(error.message||'').toLowerCase().includes('duplicate')){
    console.warn('Automatic document link failed',error);return false;
  }
  if(!error)state.documentLinks.push({
    source_document_id:rel.source.id,target_document_id:rel.target.id,link_type:rel.type
  });
  return !error;
}
function automaticPairAllowed(a,b){
  const t=new Set([a?.doc_type,b?.doc_type]);
  return (t.has('SDS')&&t.has('COSHH'))||
         (t.has('COSHH')&&t.has('SSW'))||
         (t.has('RISK_ASSESSMENT')&&t.has('SSW'));
}
function autoPairThreshold(a,b){
  const t=new Set([a?.doc_type,b?.doc_type]);
  if(t.has('SDS')&&t.has('COSHH'))return 0.55;
  if(t.has('COSHH')&&t.has('SSW'))return 0.62;
  if(t.has('RISK_ASSESSMENT')&&t.has('SSW'))return 0.68;
  return 1;
}
async function autoDiscoverDocumentLinks(docId,fileLike=null){
  if(!isManager())return 0;
  await loadAll();
  const d=state.documents.find(x=>x.id===docId);if(!d)return 0;
  let added=0;

  // First use explicit references written inside the newly uploaded PDF.
  if(fileLike&&window.pdfjsLib){
    const parsed=await pdfTextAndLinesFromFileLike(fileLike,20);
    const refs=[...new Set((parsed.text.match(/\b(?:RA|COSHH|SSW|PROC)-\d{3}\b/gi)||[]).map(x=>x.toUpperCase()))];
    for(const ref of refs){
      if(ref===String(d.reference||'').toUpperCase())continue;
      const other=state.documents.find(x=>String(x.reference||'').toUpperCase()===ref&&x.status!=='ARCHIVED');
      if(other&&await insertDiscoveredLink(d,other))added++;
    }
  }

  // Then find strong title/product matches in the recognised relationship chains.
  const candidates=state.documents.filter(x=>x.id!==d.id&&x.status!=='ARCHIVED'&&automaticPairAllowed(d,x));
  for(const other of candidates){
    if(linkExistsBetween(d.id,other.id))continue;
    const score=autoLinkSimilarity(d.title,other.title);
    if(score>=autoPairThreshold(d,other)){
      if(await insertDiscoveredLink(d,other))added++;
    }
  }
  return added;
}
async function autoDiscoverAllDocumentLinks(options={}){
  if(!isManager())return {added:0,explicit:0,matched:0,scanned:0,repaired:0,unmatched:[]};
  const scanFiles=options.scanFiles===true;
  const progress=typeof options.onProgress==='function'?options.onProgress:()=>{};
  await loadAll();

  let repaired=0;
  if(scanFiles){
    const before=state.documents.filter(d=>d.doc_type==='SDS'&&isGenericSdsTitle(d.title)).length;
    if(await repairGenericSdsTitles()){
      await loadAll();
      const after=state.documents.filter(d=>d.doc_type==='SDS'&&isGenericSdsTitle(d.title)).length;
      repaired=Math.max(0,before-after);
    }
  }

  let added=0,explicit=0,matched=0,scanned=0;
  const docs=state.documents.filter(d=>d.status!=='ARCHIVED');

  if(scanFiles&&window.pdfjsLib){
    for(let i=0;i<docs.length;i++){
      const d=docs[i],v=currentVersionForDisplay(d.id);
      progress(`Scanning stored PDFs ${i+1} of ${docs.length}: ${d.reference||d.title}…`);
      if(!v?.storage_path)continue;
      try{
        const {data,error}=await sb.storage.from('safety-files').download(v.storage_path);
        if(error||!data)continue;
        scanned++;
        const parsed=await pdfTextAndLinesFromFileLike(data,40);
        const refs=[...new Set((parsed.text.match(/\b(?:RA|COSHH|SSW|PROC)-\d{3}\b/gi)||[]).map(x=>x.toUpperCase()))];
        for(const ref of refs){
          if(ref===String(d.reference||'').toUpperCase())continue;
          const other=docs.find(x=>String(x.reference||'').toUpperCase()===ref);
          if(other&&!linkExistsBetween(d.id,other.id)){
            if(await insertDiscoveredLink(d,other)){added++;explicit++;}
          }
        }
      }catch(e){console.warn('Forced link scan failed',d.id,e)}
    }
    await loadAll();
  }

  progress('Checking document titles and product names for missing links…');
  for(let i=0;i<docs.length;i++){
    for(let j=i+1;j<docs.length;j++){
      const a=docs[i],b=docs[j];
      if(!automaticPairAllowed(a,b)||linkExistsBetween(a.id,b.id))continue;
      if(autoLinkSimilarity(a.title,b.title)>=autoPairThreshold(a,b)){
        if(await insertDiscoveredLink(a,b)){added++;matched++;}
      }
    }
  }

  await loadAll();
  const refreshed=state.documents.filter(d=>d.status!=='ARCHIVED');
  const unmatched=refreshed.filter(d=>
    ['SDS','COSHH','SSW','RISK_ASSESSMENT'].includes(d.doc_type) &&
    !linksForDocument(d.id).length
  ).map(d=>({
    id:d.id,
    type:d.doc_type,
  ...
  progress('');
  return {added,explicit,matched,scanned,repaired,unmatched};
}
...
async function loadAll(){
  const tables=['profiles','documents','document_versions','document_assignments','document_signoffs','document_delivery_confirmations','document_links','document_reviews','document_approvals','training_sessions','training_assignments','training_signoffs','training_delivery_confirmations','training_files','training_approvals'];
  const results=await Promise.all(tables.map(t=>sb.from(t).select('*')));
  const map=['people','documents','versions','docAssignments','docSignoffs','docConfirmations','documentLinks','documentReviews','documentApprovals','training','trainingAssignments','trainingSignoffs','trainingConfirmations','trainingFiles','trainingApprovals'];
...
function currentVersion(docId){
  return currentPublishedVersion(docId)
    ||pendingVersions(docId)[0]
    ||state.versions.filter(v=>v.document_id===docId).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0];
}
...
function renderDocuments(){
  ...
  if(!isManager())docs=docs.filter(d=>d.doc_type==='SDS'||!!currentPublishedVersion(d.id));
  docs.sort((a,b)=>a.title.localeCompare(b.title));
  $('documentsList').innerHTML=docs.length?docs.map(d=>{
    const v=currentPublishedVersion(d.id)||currentVersion(d.id),
          pending=pendingVersions(d.id)[0],
          assigns=state.docAssignments.filter(a=>a.document_version_id===v?.id&&a.active!==false),
          reviewOverdue=v?.review_date&&v.review_date<todayISO(),links=linksForDocument(d.id),
          canAssign=d.doc_type!=='SDS'&&v&&versionIsApproved(v,d);
    return `<div class="item-card">
      <div class="row-between"><div><h3>${esc(d.title)}</h3><div class="meta">
        <span class="badge">${esc(docTypeLabel(d.doc_type))}</span><span>${esc(d.reference||'No reference')}</span>
        ${v?`<span>Current version ${esc(v.version_label)}</span>`:''}
        ${pending?'<span class="badge due">New upload pending approval</span>':''}
        ${reviewOverdue?'<span class="badge overdue">Review overdue</span>':''}
        ${d.review_required?'<span class="badge overdue">Linked document changed - review required</span>':''}
      </div></div><span class="badge">${esc(d.status)}</span></div>
      <div class="meta"><span>Review: ${fmtDate(v?.review_date)}</span><span>${assigns.length} assigned</span><span>${deliveryText(effectiveDocumentMethod(d,null,v))}</span><span>${renewalText(d.default_renewal_value,d.default_renewal_unit)}</span><span>${links.length} linked</span></div>
      <div class="row">
        ${v?btn('View current','secondary',`data-open-doc="${v.id}"`):''}
        ${pending&&isManager()?btn('View pending','secondary',`data-open-doc="${pending.id}"`)+btn('Sign & approve','primary',`data-approve-doc-version="${pending.id}"`):''}
        ${btn('Links','ghost',`data-doc-links="${d.id}"`)}
        ${isManager()?(canAssign?btn('Assign','secondary',`data-assign-doc="${d.id}"`):'')+(!pending||d.doc_type==='SDS'?btn('New version','secondary',`data-new-version="${d.id}"`):'')+((d.doc_type!=='SDS'&&(reviewOverdue||d.review_required))?btn('Review','primary',`data-record-doc-review="${d.id}"`):'')+btn('Details','ghost',`data-doc-details="${d.id}"`):btn('Details','ghost',`data-doc-details="${d.id}"`)}
      </div>
    </div>`;
  }).join(''):'<div class="empty">No documents found.</div>';
}
function showNewDocument(){
  openModal('New document',`<div class="form-grid"><label>Title<input id="docTitle"></label><label>Reference<input id="docRef" placeholder="e.g. RA-014"></label><label>Type<select id="docType"><option value="RISK_ASSESSMENT">Risk Assessment</option><option value="COSHH">COSHH Assessment</option><option value="SDS">SDS / MSDS</option><option value="SSW">Safe System of Work (SSW)</option><option value="POLICY">Policy</option><option value="PROCEDURE">Procedure</option><option value="OTHER">Other</option></select></label><label>Delivery method<select id="docDelivery"><option value="SELF_TRAINING">Self-training</option><option value="INSTRUCTOR_LED">Instructor-led</option></select></label><div class="full hint-box"><strong>Choose for each item:</strong> Risk Assessments and COSHH default to self-training. SSW defaults to instructor-led. SDS/MSDS can be stored and linked without assigning it for sign-off.</div><label>Version<input id="docVersion" value="1"></label><label>Issue date<input id="docIssue" type="date" value="${todayISO()}"></label><label>Review date<input id="docReview" type="date" value="${yearsFromNow(1)}"></label><label>Default sign-off frequency<select id="docRenewalPreset"><option value="">One-off only</option><option value="3|MONTHS">Every 3 months</option><option value="6|MONTHS">Every 6 months</option><option value="12|MONTHS">Every 12 months</option><option value="24|MONTHS">Every 24 months</option><option value="CUSTOM">Custom</option></select></label><label>Re-sign on new version<select id="docResign"><option value="true">Yes</option><option value="false">No</option></select></label><div id="docCustomRenewal" class="full" hidden><div class="form-grid"><label>Every<input id="docRenewalValue" type="number" min="1"></label><label>Unit<select id="docRenewalUnit"><option>DAYS</option><option>MONTHS</option><option>YEARS</option></select></label></div></div><label class="full">Upload finished file<input id="docFile" type="file"></label><label class="full">Notes<textarea id="docNotes"></textarea></label><div id="docCreateStatus" class="full muted" aria-live="polite"></div></div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Create document','primary','id="createDocumentBtn" data-create-document')}</div>`);
...
async function createDocumentRecord(){
  ...
  try{
    ...
    if(!title){
      setStatus('Enter a document title.');toast('Enter a document title.');$('docTitle')?.focus();return;
    }
    if(!file){
      setStatus('Choose the finished document file first.');toast('Choose the finished document file first.');return;
    }

    if(saveBtn){saveBtn.dataset.busy='1';saveBtn.disabled=true;saveBtn.textContent='Creating…';}
    ...
    const {data:doc,error}=await sb.from('documents').insert({
      ...
      resign_on_new_version:isSds?false:($('docResign').value==='true'),
    ...
    if(up.error){await sb.from('documents').delete().eq('id',doc.id);throw up.error;}

    setStatus(isSds?'Saving SDS/MSDS…':'Saving as pending approval…');
    const {error:ve}=await sb.from('document_versions').insert({
      document_id:doc.id,
      version_label:$('docVersion').value.trim()||'1',
      issue_date:$('docIssue').value||null,
      review_date:isSds?null:($('docReview').value||null),
      delivery_method:$('docDelivery').value,
      storage_path:path,
      file_name:file.name,
      notes:$('docNotes').value.trim()||null,
      status:isSds?'CURRENT':'SUPERSEDED',
      approval_status:isSds?'NOT_REQUIRED':'PENDING',
      created_by:state.user.id
    });
    if(ve){await sb.storage.from('safety-files').remove([path]);await sb.from('documents').delete().eq('id',doc.id);throw ve;}

    const autoLinks=await autoDiscoverDocumentLinks(doc.id,file);
    closeModal();
    await refresh((isSds?'SDS/MSDS saved as reference-only.':'Document uploaded and is awaiting signed approval before it becomes current or can be assigned.')+(autoLinks?` ${autoLinks} related document link${autoLinks===1?'':'s'} added automatically.`:''));
  }catch(err){
    console.error('Create document failed',err);
    const msg=err?.message||'Could not create document.';
    setStatus(`Error: ${msg}`);toast(msg);
    if(saveBtn){saveBtn.dataset.busy='0';saveBtn.disabled=false;saveBtn.textContent='Create document';}
  }
}
async function showAssignDocument(docId){
  const d=state.documents.find(x=>x.id===docId),v=currentPublishedVersion(docId);if(!d||!v)return toast('This document has no approved current version yet.');
  if(d.doc_type==='SDS')return toast('SDS/MSDS is reference-only and does not require employee sign-off.');
  if(!versionIsApproved(v,d))return toast('This document must be signed and approved before it can be assigned.');
...
async function showNewVersion(docId){
  const d=state.documents.find(x=>x.id===docId),old=currentPublishedVersion(docId);
  if(d.doc_type!=='SDS'&&pendingVersions(docId).length){
    return toast('A new version is already awaiting approval. Approve that version before uploading another.');
  }
  const currentMethod=effectiveDocumentMethod(d,null,old);
  const isSds=d.doc_type==='SDS';
  openModal('Upload new version',`<p><strong>${esc(d.title)}</strong> · Current version ${esc(old?.version_label||'—')}</p>
  ${!isSds?`<div class="hint-box"><strong>Approval required.</strong> The existing approved version remains current until you digitally sign and approve this upload. No reassignment happens until approval.</div>`:'<div class="hint-box"><strong>SDS/MSDS reference document.</strong> No approval signature is required.</div>'}
  <div class="form-grid">
    <label>New version<input id="newVerLabel"></label>
    <label>Issue date<input id="newVerIssue" type="date" value="${todayISO()}"></label>
    ${!isSds?`<label>Review date<input id="newVerReview" type="date" value="${yearsFromNow(1)}"></label>`:''}
    <label>Delivery method for new version<select id="newVerDelivery"><option value="KEEP" selected>Keep current — ${deliveryText(currentMethod)}</option><option value="SELF_TRAINING">Self-training</option><option value="INSTRUCTOR_LED">Instructor-led</option></select></label>
    ${!isSds?`<div class="full hint-box"><strong>This applies to the new version only.</strong> Existing completed versions keep their recorded delivery method.</div>
    <label>Require fresh sign-off after approval?<select id="newVerReassign"><option value="true" ${d.resign_on_new_version?'selected':''}>Yes — reassign current users after approval</option><option value="false" ${!d.resign_on_new_version?'selected':''}>No — do not reassign</option></select></label>
    <label>New sign-off due date<input id="newVerDue" type="date" value="${daysFromNow(14)}"></label>`:''}
    <label class="full">Upload file<input id="newVerFile" type="file"></label>
    <label class="full">Notes<textarea id="newVerNotes"></textarea></label>
  </div>
  <div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn(isSds?'Publish SDS/MSDS':'Upload for approval','primary',`data-publish-version="${docId}"`)}</div>`);
}
async function publishVersion(docId){
  const file=$('newVerFile').files[0];
  if(!$('newVerLabel').value.trim()||!file)return toast('Version and file are required.');
  const d=state.documents.find(x=>x.id===docId),old=currentPublishedVersion(docId),isSds=d.doc_type==='SDS';
  if(!isSds&&pendingVersions(docId).length)return toast('A version is already awaiting approval.');
  ...
  const reassign=!isSds&&$('newVerReassign')?.value==='true';
  ...
  const {data:nv,error}=await sb.from('document_versions').insert({
    ...
    review_date:isSds?null:($('newVerReview')?.value||null),
    ...
    status:isSds?'CURRENT':'SUPERSEDED',
    approval_status:isSds?'NOT_REQUIRED':'PENDING',
    reassign_on_approval:reassign,
    reassign_due_date:!isSds?($('newVerDue')?.value||daysFromNow(14)):null,
    created_by:state.user.id
  }).select().single();
  if(error)return toast(error.message);
  ...
  const autoLinks=await autoDiscoverDocumentLinks(docId,file);
  closeModal();
  await refresh((isSds?'New SDS/MSDS version published. No approval signature required.':'New version uploaded. The existing approved version remains current until this upload is digitally signed and approved.')+(autoLinks?` ${autoLinks} related document link${autoLinks===1?'':'s'} added automatically.`:''));
}

function showDocumentApproval(versionId){
  const v=state.versions.find(x=>x.id===versionId),d=v&&state.documents.find(x=>x.id===v.document_id);
  if(!d||!v)return toast('Document version not found.');
  if(d.doc_type==='SDS')return toast('SDS/MSDS does not require signed approval.');
  if(!isManager())return toast('Manager or Admin access required.');
  if(v.approval_status!=='PENDING')return toast('This version is not awaiting approval.');

  openModal('Sign and approve document',`
    <div class="stack">
      <div><strong>${esc(d.title)}</strong><div class="meta"><span>${esc(d.reference||'No reference')}</span><span>Version ${esc(v.version_label)}</span><span>Uploaded by ${esc(personName(v.created_by))}</span><span>${fmtDateTime(v.created_at)}</span></div></div>
      <div class="hint-box"><strong>This is an approval, not employee training sign-off.</strong> Approval makes this version current and allows it to be assigned. The previous current version remains live until approval.</div>
      <div class="row">${btn('View uploaded file','secondary',`data-open-doc="${v.id}"`)}</div>
      <label>Approval note (optional)<textarea id="docApprovalNote" placeholder="Any approval note or checks completed"></textarea></label>
      <p>I confirm I have reviewed this uploaded document version and approve it as the controlled current version.</p>
      ${signatureBlock('docApproval')}
      <label class="check-row"><input id="docApprovalAck" type="checkbox"> I approve this document version and confirm this digital signature.</label>
      <div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Sign & approve','primary',`data-save-doc-approval="${v.id}"`)}</div>
    </div>`);
  setupSignaturePad('docApprovalSignaturePad','docApprovalClearSignature');
}
async function saveDocumentApproval(versionId){
  if(!$('docApprovalAck')?.checked)return toast('Tick the approval confirmation first.');
  const sig=signatureData('docApprovalSignaturePad'),sigName=$('docApprovalSignatureName')?.value.trim(),note=$('docApprovalNote')?.value.trim()||null;
  if(!sig)return toast('Please sign in the digital signature box.');
  if(!sigName)return toast('Enter the signature name.');
  const {error}=await sb.rpc('approve_document_version',{p_document_version_id:versionId,p_signature_data:sig,p_signature_name:sigName,p_approval_note:note});
  if(error)return toast(error.message);
  closeModal();
  await refresh('Document digitally signed and approved. It is now the current controlled version.');
}
function showDocumentApprovalEvidence(approvalId){
  const a=state.documentApprovals.find(x=>x.id===approvalId);if(!a)return toast('Approval record not found.');
  const src=reviewSignatureSrc(a);
  openModal('Signed document approval',`<div class="stack">
    <strong>${esc(a.title_snapshot)}</strong>
    <div class="meta"><span>${esc(a.reference_snapshot||'No reference')}</span><span>Version ${esc(a.version_snapshot)}</span><span>Approved by ${esc(personName(a.approver_id))}</span><span>${fmtDateTime(a.approved_at)}</span></div>
    ${a.approval_note?`<p>${esc(a.approval_note)}</p>`:''}
    <p class="muted">${esc(a.statement_snapshot||'')}</p>
    <div><strong>Signature name:</strong> ${esc(a.signature_name||'')}</div>
    ${src?`<div class="review-signature-evidence"><img src="${src}" alt="Approver digital signature"></div>`:'<div class="message">Signature image is not available.</div>'}
  </div>`);
}
...
function showDocDetails(docId){
  const d=state.documents.find(x=>x.id===docId),
        ...
        approvals=state.documentApprovals.filter(a=>vs.some(v=>v.id===a.document_version_id)).sort((a,b)=>new Date(b.approved_at)-new Date(a.approved_at)),
        current=currentPublishedVersion(docId)||currentVersion(docId),isSds=d.doc_type==='SDS';

  openModal('Document details',`
    <div class="stack">
      <div><strong>${esc(d.title)}</strong><div class="meta"><span>${esc(docTypeLabel(d.doc_type))}</span><span>${esc(d.reference||'No reference')}</span>${!isSds?`<span>${deliveryText(effectiveDocumentMethod(d,null,current))}</span>`:''}</div></div>
      ${isSds?`<div class="hint-box"><strong>Reference-only SDS/MSDS.</strong> No employee assignment or signed approval is required. Upload/check attribution and manufacturer revision are retained.</div>`:'<div class="hint-box"><strong>Controlled document approval.</strong> Every newly uploaded version must be digitally signed and approved before it becomes current or can be assigned.</div>'}
      ...
      <div><strong>Linked documents</strong><div class="card-list">${links.length?links.map(l=>{const other=otherDocForLink(l,docId),v=other&&currentPublishedVersion(other.id);return `<div class="item-card compact"><div class="row-between"><span>${esc(other?.title||'Unknown document')}</span><span class="badge">${esc(linkTypeLabel(l.link_type))}</span></div><div class="row">${v?btn('View','secondary',`data-open-doc="${v.id}"`):''}${isManager()?btn('Remove link','danger',`data-remove-doc-link="${l.id}"`):''}</div></div>`}).join(''):'<span class="muted">No linked documents.</span>'}</div>${isManager()?`<div class="row">${btn('Add / manage links','secondary',`data-doc-links="${d.id}"`)}</div>`:''}</div>
      <div><strong>Version history / upload evidence</strong><div class="card-list">${vs.map(v=>{
        const pending=v.approval_status==='PENDING',approval=documentApprovalForVersion(v.id);
        const displayStatus=pending?'PENDING APPROVAL':v.status;
        return `<div class="item-card"><div class="row-between"><strong>Version ${esc(v.version_label)}</strong><span class="badge ${pending?'due':versionIsApproved(v,d)?'complete':''}">${esc(displayStatus)}</span></div>
        <div class="meta"><span>${esc(approvalLabel(v.approval_status))}</span><span>Issued ${fmtDate(v.issue_date)}</span>${!isSds?`<span>Review ${fmtDate(v.review_date)}</span>`:''}<span>${esc(v.file_name)}</span><span>Uploaded by ${esc(personName(v.created_by))}</span><span>Uploaded ${fmtDateTime(v.created_at)}</span>${approval?`<span>Approved ${fmtDateTime(approval.approved_at)}</span>`:''}</div>
        <div class="row">${btn('View','secondary',`data-open-doc="${v.id}"`)}${pending&&isManager()?btn('Sign & approve','primary',`data-approve-doc-version="${v.id}"`):''}${approval?btn('View signed approval','ghost',`data-view-doc-approval="${approval.id}"`):''}</div></div>`;
      }).join('')}</div></div>
      ${!isSds?`<div><strong>Signed approval history</strong><div class="card-list">${approvals.length?approvals.map(a=>`<div class="item-card compact"><div class="row-between"><div><strong>Version ${esc(a.version_snapshot)}</strong><div class="meta"><span>Approved by ${esc(personName(a.approver_id))}</span><span>${fmtDateTime(a.approved_at)}</span></div></div><span class="badge complete">Approved & signed</span></div>${a.approval_note?`<p class="muted">${esc(a.approval_note)}</p>`:''}<div class="row">${btn('View signed approval','secondary',`data-view-doc-approval="${a.id}"`)}</div></div>`).join(''):'<span class="muted">No signed approvals recorded yet. Existing pre-v1.7 documents remain identified as pre-approval records.</span>'}</div></div>`:''}
      ${!isSds?`<div><strong>Signed periodic review history</strong><div class="card-list">${reviews.length?reviews.map(r=>`<div class="item-card compact"><div class="row-between"><div><strong>Version ${esc(r.version_snapshot)}</strong><div class="meta"><span>${esc(reviewOutcomeLabel(r.outcome))}</span><span>Reviewed by ${esc(personName(r.reviewer_id))}</span><span>${fmtDateTime(r.reviewed_at)}</span>${r.next_review_date?`<span>Next review ${fmtDate(r.next_review_date)}</span>`:''}</div></div><span class="badge complete">Signed</span></div>${r.review_note?`<p class="muted">${esc(r.review_note)}</p>`:''}<div class="row">${btn('View signed review','secondary',`data-view-doc-review="${r.id}"`)}</div></div>`).join(''):'<span class="muted">No signed periodic review records yet.</span>'}</div></div>`:''}
...
async function forceSyncAndReviewLinks(){
  if(!isManager())return toast('Manager or Admin access required.');
  if(!navigator.onLine)return toast('Link sync requires an internet connection.');
  const btnEl=$('forceLinkSyncBtn');
  if(btnEl){btnEl.disabled=true;btnEl.textContent='Syncing…';}
  const status=$('forceLinkSyncStatus');
  const setStatus=msg=>{if(status)status.textContent=msg||'';};

  try{
    setStatus('Starting full document-link sync…');
    const result=await autoDiscoverAllDocumentLinks({
      scanFiles:true,
      onProgress:setStatus
    });
    await refresh();
    setStatus(`Link sync complete. ${result.added} link${result.added===1?'':'s'} added.`);
    showLinkSyncReview(result);
  }catch(e){
    console.error('Force link sync failed',e);
    setStatus(`Sync failed: ${e.message||e}`);
    toast('Could not complete link sync.');
  }finally{
    if(btnEl){btnEl.disabled=false;btnEl.textContent='Force sync & review links';}
  }
}

function showLinkSyncReview(result){
  const standardLinks=state.documentLinks.filter(l=>
    ['SDS_TO_COSHH','COSHH_TO_SSW','RA_TO_SSW'].includes(l.link_type)
  );
  const linkRows=standardLinks.map(l=>{
    const a=state.documents.find(d=>d.id===l.source_document_id),
          b=state.documents.find(d=>d.id===l.target_document_id);
    if(!a||!b)return '';
    return `<div class="item-card compact">
      <div class="row-between">
        <span>${esc(documentPickerLabel(a))}<br>↳ ${esc(documentPickerLabel(b))}</span>
        <span class="badge">${esc(linkTypeLabel(l.link_type))}</span>
      </div>
    </div>`;
  }).join('');

  const unmatched=(result.unmatched||[]).map(d=>
    `<div class="item-card compact">
      <div class="row-between">
        <span>${esc(docTypeLabel(d.type))} · ${esc(d.reference||'')} · ${esc(d.title)}</span>
        ${btn('Review links','secondary',`data-doc-links="${d.id}"`)}
      </div>
    </div>`
  ).join('');

  openModal('Link sync review',`
    <div class="stack">
      <div class="hint-box">
        <strong>Full link sync completed.</strong><br>
        Stored PDFs scanned: ${result.scanned||0}<br>
        Generic SDS/MSDS titles repaired: ${result.repaired||0}<br>
        Links added from explicit references: ${result.explicit||0}<br>
        Links added from strong title/product matching: ${result.matched||0}<br>
        <strong>Total new links: ${result.added||0}</strong>
      </div>

      <div>
        <strong>Documents still needing link review</strong>
        <div class="card-list" style="margin-top:8px">
          ${unmatched||'<span class="muted">None. All RA/COSHH/SSW/SDS documents have at least one link.</span>'}
        </div>
      </div>

      <div>
        <strong>Current automatic relationship links</strong>
        <div class="card-list" style="margin-top:8px;max-height:320px;overflow:auto">
          ${linkRows||'<span class="muted">No standard relationship links are currently recorded.</span>'}
        </div>
      </div>

      <div class="hint-box">
        Automatic linking is deliberately conservative. If a document remains above, use <strong>Review links</strong> to add the correct relationship manually rather than creating a weak match.
      </div>
    </div>
  `);
}

async function showDocumentLinks(docId){
  if(isManager()){
    const repaired=await repairGenericSdsTitles();
    if(repaired)await loadAll();
  }
  const d=state.documents.find(x=>x.id===docId),
        links=linksForDocument(docId),
        available=state.documents.filter(x=>x.id!==docId&&x.status!=='ARCHIVED')
          .sort((a,b)=>documentPickerLabel(a).localeCompare(documentPickerLabel(b)));

  const options=available.map(x=>`<option value="${x.id}">${esc(documentPickerLabel(x))}</option>`).join('');
  openModal('Linked documents',`<div class="stack">
    <p><strong>${esc(d.title)}</strong> · ${esc(docTypeLabel(d.doc_type))}</p>
    <div><strong>Current links</strong><div class="card-list">${
      links.length?links.map(l=>{
        const other=otherDocForLink(l,docId);
        return `<div class="item-card compact"><div class="row-between">
          <span>${esc(other?documentPickerLabel(other):'Unknown')}</span>
          <span class="badge">${esc(linkTypeLabel(l.link_type))}</span>
        </div>${isManager()?`<div class="row">${btn('Remove','danger',`data-remove-doc-link="${l.id}"`)}</div>`:''}</div>`;
      }).join(''):'<span class="muted">No links yet.</span>'
    }</div></div>
    ${isManager()?`<div class="section-card"><strong>Add link</strong>
      <label>Search documents<input id="relatedDocumentSearch" placeholder="Type product, title, reference or filename"></label>
      <label>Related document<select id="relatedDocumentId"><option value="">Choose document</option>${options}</select></label>
      <p class="muted">Options show document type, reference, title and filename where useful. New uploads also automatically scan for missing SDS/MSDS → COSHH, COSHH → SSW and RA → SSW links.</p>
      <div class="row">${btn('Add link','primary',`data-add-doc-link="${docId}"`)}</div>
    </div>`:''}
  </div>`);

  const search=$('relatedDocumentSearch'),select=$('relatedDocumentId');
  if(search&&select){
    const all=available.map(x=>({id:x.id,label:documentPickerLabel(x)}));
    const render=()=>{
      const q=search.value.trim().toLowerCase();
      const filtered=!q?all:all.filter(x=>x.label.toLowerCase().includes(q));
      select.innerHTML='<option value="">Choose document</option>'+filtered.map(x=>`<option value="${x.id}">${esc(x.label)}</option>`).join('');
      if(filtered.length===1)select.value=filtered[0].id;
    };
...
function renderTraining(){
  ...
  let rows=state.training.filter(t=>(!q||`${t.name} ${t.description||''}`.toLowerCase().includes(q))&&(!type||t.session_type===type)&&t.status!=='ARCHIVED');
  if(!isManager())rows=rows.filter(t=>trainingIsApproved(t));
  rows.sort((a,b)=>new Date(b.delivered_date||b.created_at)-new Date(a.delivered_date||a.created_at));
  $('trainingList').innerHTML=rows.length?rows.map(t=>{
    const assigns=state.trainingAssignments.filter(a=>a.training_session_id===t.id&&a.active!==false),
          signs=state.trainingSignoffs.filter(s=>assigns.some(a=>a.id===s.training_assignment_id)),
          done=new Set(signs.map(s=>s.user_id)).size,
          reviewOverdue=t.review_date&&t.review_date<todayISO(),
          pending=t.session_type==='TOOLBOX_TALK'&&t.approval_status==='PENDING',
          canAssign=trainingIsApproved(t);
    return `<div class="item-card">
      <div class="row-between"><div><h3>${esc(t.name)}</h3><div class="meta"><span class="badge">${esc(t.session_type.replaceAll('_',' '))}</span><span>${fmtDate(t.delivered_date)}</span><span>Trainer: ${esc(t.trainer_name||'—')}</span>${pending?'<span class="badge due">Pending signed approval</span>':''}${reviewOverdue?'<span class="badge overdue">Review overdue</span>':''}</div></div><span class="badge">${done}/${assigns.length} signed</span></div>
      <div class="meta"><span>${deliveryText(t.delivery_method||defaultTrainingDelivery(t.session_type))}</span><span>${renewalText(t.renewal_value,t.renewal_unit)}</span><span>Review: ${fmtDate(t.review_date)}</span>${t.session_type==='TOOLBOX_TALK'?`<span>${esc(approvalLabel(t.approval_status))}</span>`:''}</div>
      <div class="row">${btn('View','secondary',`data-view-training="${t.id}"`)}${isManager()?btn('Edit','ghost',`data-edit-training="${t.id}"`)+(pending?btn('Sign & approve','primary',`data-approve-training="${t.id}"`):'')+(canAssign?btn('Assign','secondary',`data-assign-training="${t.id}"`):'')+btn('Archive','ghost',`data-archive-training="${t.id}"`):''}</div>
    </div>`;
  }).join(''):'<div class="empty">No training sessions found.</div>';
}
function showNewTraining(){
  openModal('New training session',`<div class="form-grid">
    ...
    <label>Review date<input id="trainReview" type="date"></label>
    <label>Sign-off / refresher frequency<select id="trainRenewalPreset"><option value="">One-off only</option><option value="3|MONTHS">Every 3 months</option><option value="6|MONTHS">Every 6 months</option><option value="12|MONTHS">Every 12 months</option><option value="24|MONTHS">Every 24 months</option><option value="CUSTOM">Custom</option></select></label>
    <div id="trainCustomRenewal" class="full" hidden><div class="form-grid"><label>Every<input id="trainRenewalValue" type="number" min="1"></label><label>Unit<select id="trainRenewalUnit"><option>DAYS</option><option>MONTHS</option><option>YEARS</option></select></label></div></div>
    ...
    <div id="trainApprovalHint" class="full hint-box" hidden><strong>Toolbox Talk approval:</strong> after upload this will be pending until a Manager/Admin reviews the file and digitally signs approval. It cannot be assigned before approval.</div>
  </div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Create training','primary','id="saveTraining"')}</div>`);
  const syncDelivery=()=>{
    $('trainDelivery').value=defaultTrainingDelivery($('trainType').value);
    $('trainApprovalHint').hidden=$('trainType').value!=='TOOLBOX_TALK';
  };
...
async function saveTraining(){
  if(!$('trainName').value.trim())return toast('Training name is required.');
  const r=renewalFromPreset('train'),type=$('trainType').value,isTbt=type==='TOOLBOX_TALK';
  const {data:t,error}=await sb.from('training_sessions').insert({
    name:$('trainName').value.trim(),
    session_type:type,
    delivery_method:$('trainDelivery').value,
    description:$('trainDesc').value.trim()||null,
    delivered_date:$('trainDate').value||null,
    trainer_name:$('trainTrainer').value.trim()||null,
    trainer_user_id:state.user.id,
    review_date:$('trainReview').value||null,
    default_due_date:$('trainDefaultDue').value||null,
    renewal_value:r.value,
    renewal_unit:r.unit,
    status:'ACTIVE',
    approval_status:isTbt?'PENDING':'NOT_REQUIRED',
    created_by:state.user.id
  }).select().single();
  if(error)return toast(error.message);
  for(const file of $('trainFiles').files){
  ...
  closeModal();
  await refresh(isTbt?'Toolbox Talk uploaded and awaiting signed approval before assignment.':'Training created.');
}

function showEditTraining(id){
  const t=state.training.find(x=>x.id===id);if(!t||!isManager())return;
  const preset=renewalPresetValue(t.renewal_value,t.renewal_unit);
  openModal('Edit training',`<p><strong>${esc(t.name)}</strong></p><div class="form-grid">
    <label>Delivery method<select id="editTrainDelivery"><option value="SELF_TRAINING" ${t.delivery_method==='SELF_TRAINING'?'selected':''}>Self-training</option><option value="INSTRUCTOR_LED" ${t.delivery_method==='INSTRUCTOR_LED'?'selected':''}>Instructor-led</option></select></label>
    <label>Default completion due date<input id="editTrainDefaultDue" type="date" value="${t.default_due_date||''}"></label>
    <label>Date delivered<input id="editTrainDate" type="date" value="${t.delivered_date||''}"></label>
    <label>Trainer / presenter<input id="editTrainTrainer" value="${esc(t.trainer_name||'')}"></label>
    <label>Review date<input id="editTrainReview" type="date" value="${t.review_date||''}"></label>
    <label>Sign-off / refresher frequency<select id="editTrainRenewalPreset">
      <option value="" ${preset===''?'selected':''}>One-off only</option>
    ...
    <div id="editTrainCustomRenewal" class="full" ${preset==='CUSTOM'?'':'hidden'}><div class="form-grid"><label>Every<input id="editTrainRenewalValue" type="number" min="1" value="${preset==='CUSTOM'?(t.renewal_value||''):''}"></label><label>Unit<select id="editTrainRenewalUnit"><option ${t.renewal_unit==='DAYS'?'selected':''}>DAYS</option><option ${t.renewal_unit==='MONTHS'?'selected':''}>MONTHS</option><option ${t.renewal_unit==='YEARS'?'selected':''}>YEARS</option></select></label></div></div>
    <label class="full">Details<textarea id="editTrainDesc">${esc(t.description||'')}</textarea></label>
  </div><div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Save changes','primary',`data-save-edit-training="${id}"`)}</div>`);
...
async function saveEditTraining(id){
  const t=state.training.find(x=>x.id===id);if(!t||!isManager())return;
  const p=$('editTrainRenewalPreset').value;let rv=null,ru=null;
  if(p==='CUSTOM'){rv=Number($('editTrainRenewalValue').value)||null;ru=$('editTrainRenewalUnit').value;}
  else if(p){[rv,ru]=p.split('|');rv=Number(rv);}
  const payload={
    delivery_method:$('editTrainDelivery').value,
    default_due_date:$('editTrainDefaultDue').value||null,
    delivered_date:$('editTrainDate').value||null,
    trainer_name:$('editTrainTrainer').value.trim()||null,
    review_date:$('editTrainReview').value||null,
    renewal_value:rv,renewal_unit:ru,
    description:$('editTrainDesc').value.trim()||null
  };
  const {error}=await sb.from('training_sessions').update(payload).eq('id',id);if(error)return toast(error.message);
  const {error:ae}=await sb.from('training_assignments').update({renewal_value:rv,renewal_unit:ru}).eq('training_session_id',id).eq('active',true);
  if(ae)return toast(ae.message);
  closeModal();await refresh('Training settings updated.');
}
function showTrainingApproval(id){
  const t=state.training.find(x=>x.id===id);if(!t)return;
  if(!isManager())return toast('Manager or Admin access required.');
  if(t.session_type!=='TOOLBOX_TALK')return toast('Signed upload approval is only required for Toolbox Talk training files.');
  if(t.approval_status!=='PENDING')return toast('This Toolbox Talk is not awaiting approval.');
  const files=state.trainingFiles.filter(f=>f.training_session_id===id);
  openModal('Sign and approve Toolbox Talk',`<div class="stack">
    <div><strong>${esc(t.name)}</strong><div class="meta"><span>${renewalText(t.renewal_value,t.renewal_unit)}</span><span>Uploaded/created by ${esc(personName(t.created_by))}</span><span>${fmtDateTime(t.created_at)}</span></div></div>
    <div class="hint-box"><strong>Approval required before training assignment.</strong> Review the uploaded Toolbox Talk and its training frequency, then sign below.</div>
    <div class="row">${files.map(f=>btn(`View ${f.file_name}`,'secondary',`data-open-training-file="${f.id}"`)).join('')||'<span class="muted">No file attached.</span>'}</div>
    <label>Approval note (optional)<textarea id="trainingApprovalNote"></textarea></label>
    <p>I confirm I have reviewed this Toolbox Talk and approve it for controlled use and training assignment.</p>
    ${signatureBlock('trainingApproval')}
    <label class="check-row"><input id="trainingApprovalAck" type="checkbox"> I approve this Toolbox Talk and confirm this digital signature.</label>
    <div class="actions">${btn('Cancel','ghost','data-close-modal')}${btn('Sign & approve','primary',`data-save-training-approval="${id}"`)}</div>
  </div>`);
  setupSignaturePad('trainingApprovalSignaturePad','trainingApprovalClearSignature');
}
async function saveTrainingApproval(id){
  if(!$('trainingApprovalAck')?.checked)return toast('Tick the approval confirmation first.');
  const sig=signatureData('trainingApprovalSignaturePad'),sigName=$('trainingApprovalSignatureName')?.value.trim(),note=$('trainingApprovalNote')?.value.trim()||null;
  if(!sig)return toast('Please sign in the digital signature box.');
  if(!sigName)return toast('Enter the signature name.');
  const {error}=await sb.rpc('approve_training_session',{p_training_session_id:id,p_signature_data:sig,p_signature_name:sigName,p_approval_note:note});
  if(error)return toast(error.message);
  closeModal();await refresh('Toolbox Talk digitally signed and approved. It can now be assigned.');
}
function showTrainingApprovalEvidence(id){
  const a=state.trainingApprovals.find(x=>x.id===id);if(!a)return toast('Approval record not found.');
  const src=reviewSignatureSrc(a);
  openModal('Signed Toolbox Talk approval',`<div class="stack"><strong>${esc(a.training_name_snapshot)}</strong><div class="meta"><span>Approved by ${esc(personName(a.approver_id))}</span><span>${fmtDateTime(a.approved_at)}</span></div>${a.approval_note?`<p>${esc(a.approval_note)}</p>`:''}<p class="muted">${esc(a.statement_snapshot||'')}</p><div><strong>Signature name:</strong> ${esc(a.signature_name||'')}</div>${src?`<div class="review-signature-evidence"><img src="${src}" alt="Approver digital signature"></div>`:'<div class="message">Signature image is not available.</div>'}</div>`);
}
...
function showTrainingDetails(id){
  ...
  const approval=trainingApprovalForSession(id),pending=t.session_type==='TOOLBOX_TALK'&&t.approval_status==='PENDING';
  openModal('Training details',`<div class="stack">
    <div><strong>${esc(t.name)}</strong><div class="meta"><span>${esc(t.session_type.replaceAll('_',' '))}</span><span>Default: ${deliveryText(baseMethod)}</span><span>Trainer ${esc(t.trainer_name||'—')}</span><span>Default due ${fmtDate(t.default_due_date)}</span><span>${renewalText(t.renewal_value,t.renewal_unit)}</span>${t.session_type==='TOOLBOX_TALK'?`<span>${esc(approvalLabel(t.approval_status))}</span>`:''}</div></div>
    ${pending?`<div class="message"><strong>Pending signed approval.</strong> This Toolbox Talk cannot be assigned yet.${isManager()?`<div class="row">${btn('Sign & approve','primary',`data-approve-training="${id}"`)}</div>`:''}</div>`:''}
    ${approval?`<div class="hint-box"><strong>Signed approval:</strong> ${esc(personName(approval.approver_id))} · ${fmtDateTime(approval.approved_at)} <div class="row">${btn('View signed approval','ghost',`data-view-training-approval="${approval.id}"`)}</div></div>`:''}
    ${t.description?`<p>${esc(t.description)}</p>`:''}
    ${refs.length?`<div><strong>Relevant documents</strong><div class="row">${relatedDocs.length?relatedDocs.map(d=>{const v=currentPublishedVersion(d.id);return v?btn(`${d.reference} · ${d.title}`,'secondary',`data-open-doc="${v.id}"`):''}).join(''):'<span class="muted">Referenced documents are not in the tracker yet.</span>'}</div></div>`:''}
    <div><strong>Files</strong><div class="row">${files.length?files.map(f=>btn(f.file_name,'secondary',`data-open-training-file="${f.id}"`)).join(''):'<span class="muted">No supporting files.</span>'}</div></div>
    ${isManager()?`<div class="row">${btn('Edit training settings','secondary',`data-edit-training="${id}"`)}</div>`:''}
    ${isManager()&&trainingIsApproved(t)?`<div><strong>Attendance / sign-off</strong><p class="muted">People who request instructor-led support are handled individually even if the training default is self-training.</p><div class="card-list">${assigns.length?assigns.map(a=>{const method=effectiveTrainingMethod(t,a),s=latestTrainingSignoff(a),c=latestTrainingConfirmation(a),event=latestTrainingEvent(a),ready=freshConfirmation(c,s),due=trainingAssignmentDue(a,s),done=!!s&&!(a.renewal_value&&new Date(due)<=new Date());let label=done?'Completed':method==='INSTRUCTOR_LED'?(ready?'Ready for trainee sign-off':event?.attendance_status==='ABSENT'?'Absent / not attended':'Awaiting instructor confirmation'):'Outstanding self-training';return `<div class="item-card compact"><div class="row-between"><label class="check-row">${method==='INSTRUCTOR_LED'&&!done?`<input type="checkbox" class="attendance-person" value="${a.id}">`:''}<span>${esc(personName(a.user_id))}</span></label><span class="badge ${done?'complete':'due'}">${label}</span></div><div class="meta"><span>${deliveryText(method)}</span><span>Due ${fmtDate(a.due_date)}</span>${methodOverrideNote(a)?`<span>${esc(methodOverrideNote(a))}</span>`:''}${event?.delivery_date?`<span>Last attendance date ${fmtDate(event.delivery_date)}</span>`:''}${s?`<span>Signed ${fmtDateTime(s.signed_at)}</span>`:''}</div></div>`}).join(''):'<span class="muted">No one assigned.</span>'}</div>${assigns.some(a=>effectiveTrainingMethod(t,a)==='INSTRUCTOR_LED')?`<div class="actions">${btn('Record selected attendance','primary',`data-bulk-training-attendance="${id}"`)}</div>`:''}</div>`:''}
  </div>`);
}
...
function showAssignTraining(id){
  const t=state.training.find(x=>x.id===id);if(!t)return;
  if(!trainingIsApproved(t))return toast('This Toolbox Talk must be digitally signed and approved before it can be assigned.');
  const current=state.trainingAssignments.filter(a=>a.training_session_id===id&&a.active!==false).map(a=>a.user_id);
      ...
      <div class="help-grid">
        ...
        <div class="help-card">
          ...
          <ul>
            ...
            <li>Use new versions instead of overwriting historical evidence. After each new document/version upload, Safety Tracker automatically checks for missing SDS/MSDS → COSHH, COSHH → SSW and RA → SSW links. Admin/Manager can also use Force sync & review links to rescan all stored PDFs and review anything still unmatched.</li>
...
async function downloadBackup(){const payload={exported_at:new Date().toISOString(),profiles:state.people,documents:state.documents,document_versions:state.versions,document_assignments:state.docAssignments,document_signoffs:state.docSignoffs,document_delivery_confirmations:state.docConfirmations,document_links:state.documentLinks,document_reviews:state.documentReviews,document_approvals:state.documentApprovals,training_sessions:state.training,training_assignments:state.trainingAssignments,training_signoffs:state.trainingSignoffs,training_delivery_confirmations:state.trainingConfirmations,training_files:state.trainingFiles,training_approvals:state.trainingApprovals};downloadBlob(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),`safety-tracker-backup-${todayISO()}.json`)}
...
function documentApprovalEvidenceRows(){
  return state.documentApprovals.map(a=>({
    Document:a.title_snapshot||'',
    Reference:a.reference_snapshot||'',
    Version:a.version_snapshot||'',
    Approved_by:personName(a.approver_id),
    Approved_at:fmtDateTime(a.approved_at),
    Signature_name:a.signature_name||'',
    Note:a.approval_note||''
  }));
}
function trainingApprovalEvidenceRows(){
  return state.trainingApprovals.map(a=>({
    Toolbox_Talk:a.training_name_snapshot||'',
    Approved_by:personName(a.approver_id),
    Approved_at:fmtDateTime(a.approved_at),
    Signature_name:a.signature_name||'',
    Note:a.approval_note||''
...
function approvalSignatureRows(){
  return [
    ...state.documentApprovals.map(a=>({kind:'Document approval',person:personName(a.approver_id),title:a.title_snapshot||'',reference:a.reference_snapshot||'',version:a.version_snapshot||'',training_date:'',instructor:'',signed_at:a.approved_at,signature_name:a.signature_name||'',signature_data:a.signature_data||null})),
    ...state.trainingApprovals.map(a=>({kind:'Toolbox Talk approval',person:personName(a.approver_id),title:a.training_name_snapshot||'',reference:'',version:'',training_date:'',instructor:'',signed_at:a.approved_at,signature_name:a.signature_name||'',signature_data:a.signature_data||null}))
  ];
}
...
function downloadEvidencePDF(){
  ...
  const sections=[
    ...
    ['Signed Upload Approvals - Documents',documentApprovalEvidenceRows()],
    ['Signed Upload Approvals - Toolbox Talks',trainingApprovalEvidenceRows()],
  ...
  addSignatureEvidencePages(doc,'Upload Approval Signature Evidence',approvalSignatureRows());
